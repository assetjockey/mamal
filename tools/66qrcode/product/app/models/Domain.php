...
class Domain extends Model {
    ...
    public function delete($domain_id) {
        ...
        $links_ids = db()->where('domain_id', $domain->domain_id)->where('user_id', $domain->user_id)->getValue('links', 'link_id', null);

        /* Delete all process to make sure everything is removed */
        if($links_ids) {
            (new \Altum\Models\Link())->bulk_delete($links_ids, $domain->user_id, true);
        }
