...
class Filters {
    ...
    public function __construct($allowed_filters = [], $allowed_search_by = [], $allowed_order_by = [], $allowed_results_per_page = [], $filters_types = [], $search_types = [], $allowed_datetime_fields = []) {
    ...
    public function process() {
        ...
        foreach($this->allowed_filters as $filter) {
            ...
            if(
				isset($_GET[$filter])
				&& !is_array($_GET[$filter]) // No array filters for now
				&& $_GET[$filter] != ''
			) {
        ...
        /* Search */
        if(
			count($this->allowed_search_by)
			&& isset($_GET['search'])
			&& !is_array($_GET['search']) // No array filters for now
			&& isset($_GET['search_by'])
			&& !is_array($_GET['search_by']) // No array filters for now
			&& in_array($_GET['search_by'], $this->allowed_search_by)
		) {
    ...
    public function get_sql_where($table_prefix = null) {
        ...
        if($this->search && $this->search_by) {
			if(array_key_exists($this->search_by, $this->search_types)) {

				switch($this->search_types[$this->search_by]['type']) {
					/* Search JSON keys */
					case 'json_keys':
						$where .= " AND JSON_KEYS({$table_prefix}`{$this->search_types[$this->search_by]['column']}`) LIKE '%{$this->search}%'";
						break;

					/* Search JSON values from any keys */
					case 'json_search':
						$where .= " AND JSON_SEARCH({$table_prefix}`{$this->search_types[$this->search_by]['column']}`, 'one', '%{$this->search}%')";
						break;
				}
