...
class Uploads {
    ...
    public static function download_image_from_url($image_url, $uploads_file_key, $save_file_name, $allowed_mime_types = [], $error_response_type = 'error', $error_field = null) {
		...
		$remote_content_length = curl_getinfo($curl_handle, defined('CURLINFO_CONTENT_LENGTH_DOWNLOAD_T') ? CURLINFO_CONTENT_LENGTH_DOWNLOAD_T : CURLINFO_CONTENT_LENGTH_DOWNLOAD);
		$curl_error = curl_error($curl_handle);
		$curl_handle = null;

		if($curl_exec_result === false) {
			$return_error('Failed to fetch the image headers' . ($curl_error ? ': ' . $curl_error : '.'));
			return null;
		}
		...
		if($remote_content_length > $max_file_size) {
			$return_error('Remote image is too large.');
			return null;
		}

		$remote_content_type = $remote_content_type ? explode(';', $remote_content_type)[0] : null;
		...
		if($temp_file_handle === false) {
			$return_error('Failed to save the image to temp location.');
			return null;
		}

		$curl_handle = curl_init($image_url);
		curl_setopt($curl_handle, CURLOPT_FILE, $temp_file_handle);
		curl_setopt($curl_handle, CURLOPT_FOLLOWLOCATION, true);
		curl_setopt($curl_handle, CURLOPT_TIMEOUT, 15);
		curl_setopt($curl_handle, CURLOPT_USERAGENT, 'Mozilla/5.0');
		curl_setopt($curl_handle, CURLOPT_NOPROGRESS, false);
		curl_setopt($curl_handle, CURLOPT_PROGRESSFUNCTION, function($curl_handle, $download_size, $downloaded_size) use ($max_file_size, &$downloaded_file_size) {
			$downloaded_file_size = $downloaded_size;

			return $downloaded_size > $max_file_size ? 1 : 0;
		});
		...
		if($curl_exec_result === false || $http_status !== 200) {
			unlink($temp_file_path);

			if($downloaded_file_size > $max_file_size) {
				$return_error('Remote image is too large.');
			} else {
				$return_error('Failed to download the image' . ($curl_error ? ': ' . $curl_error : '.'));
			}

			return null;
		}

		if(filesize($temp_file_path) > $max_file_size) {
			unlink($temp_file_path);
			$return_error('Remote image is too large.');
			return null;
		}
    ...
    static function clean_svg($svg_content, $allow_embedded_images = false) {
        ...
        foreach ($xpath->query('//*[local-name()="image" or local-name()="feImage"]') as $node) {
            $embedded_image_is_valid = false;

            if ($allow_embedded_images && strtolower($node->localName) === 'image') {
                $image_href = $node->getAttribute('href');
                if (!$image_href) {
                    $image_href = $node->getAttributeNS('http://www.w3.org/1999/xlink', 'href');
                }

                if (preg_match('~^data:image/(png|jpe?g|gif);base64,([a-zA-Z0-9+/]+={0,2})$~iD', $image_href, $matches)) {
                    $image_data = base64_decode($matches[2], true);
                    $image_details = $image_data !== false ? @getimagesizefromstring($image_data) : false;
                    $image_type = mb_strtolower($matches[1]) == 'jpg' ? 'jpeg' : mb_strtolower($matches[1]);
                    $embedded_image_is_valid = $image_details && isset($image_details['mime']) && $image_details['mime'] == 'image/' . $image_type;
                }
            }

            if (!$embedded_image_is_valid) {
                $node->parentNode->removeChild($node);
            }
        }
        ...
        foreach ($xpath->query('//@*[local-name()="href"]') as $attr) {
            ...
            $embedded_image_is_valid = $allow_embedded_images && strtolower($attr->ownerElement->localName) === 'image' && preg_match('~^data:image/(png|jpe?g|gif);base64,[a-zA-Z0-9+/]+={0,2}$~iD', $value);

            if (!$embedded_image_is_valid && (preg_match('~^(?:[a-z][a-z0-9+\-.]*:)?//~i', $value) || preg_match('~^(data|file|ftp|javascript):~i', $value))) {
