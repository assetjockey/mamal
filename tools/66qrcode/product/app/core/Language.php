...
class Language {
	...
	public static function initialize() {
		...
		foreach(self::get_language_file_paths() as $file_path) {
...
ALTUM;
	...
	public static function get_language_file_paths() {
		$cache_path = self::$path . 'cache/language_file_paths.php';

		if(ALTUMCODE == 66 && is_file($cache_path)) {
			return require $cache_path;
		}

		$language_file_paths = glob(self::$path . '*.php');

		if(ALTUMCODE == 66 && is_writable(self::$path . 'cache/')) {
			file_put_contents($cache_path, '<?php return ' . var_export($language_file_paths, true) . ';', LOCK_EX);
		}
