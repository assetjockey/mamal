...
class QrCodeGenerator extends Controller {
    ...
    public function index() {
        ...
        if($_POST['type'] == 'file' && empty($_POST['api_key'])) {
            throw_404();
        }
        ...
        if(!isset($_POST['api_key']) || (isset($_POST['api_key']) && empty($_POST['api_key']))) {
            ...
            if(!isset($this->user->plan_settings->enabled_qr_codes->{$_POST['type']}) || !$this->user->plan_settings->enabled_qr_codes->{$_POST['type']}) {
                throw_404();
            }
        } else {
        ...
        /* Limit QR code generation bursts */
        $rate_limit_key = ((isset($_POST['api_key']) && !empty($_POST['api_key'])) || is_logged_in()) ? 'qr-code-generator-user-' . $this->user->user_id : 'qr-code-generator-ip-' . md5(get_ip());
        ...
        if($rate_limit_remaining < 0) {
            Response::json(l('qr_codes.error_message.rate_limit'), 'error');
        }
        ...
        /* Limit stored image requests */
        $stored_image_context = stream_context_create([
            'http' => [
                'follow_location' => 0,
                'max_redirects' => 0,
                'timeout' => 5,
        ...
        switch($_POST['type']) {
            ...
            case 'file':
                $_POST['file_url'] = get_url($_POST['file_url'] ?? SITE_URL);
                $data = $_POST['file_url'];
                break;
            ...
            case 'sms':
                ...
                /* Build the standard SMS URI */
                $data = 'sms:' . $_POST['sms'];
                if($_POST['sms_body'] !== '') $data .= '?body=' . rawurlencode($_POST['sms_body']);
                break;
            ...
            case 'email':
                ...
                /* Build the standard email URI */
                $email_query_parameters = [];
                if($_POST['email_subject'] !== '') $email_query_parameters[] = 'subject=' . rawurlencode($_POST['email_subject']);
                if($_POST['email_body'] !== '') $email_query_parameters[] = 'body=' . rawurlencode($_POST['email_body']);

                $data = 'mailto:' . $_POST['email'];
                if($email_query_parameters) $data .= '?' . implode('&', $email_query_parameters);
                break;
            ...
            case 'facetime':
                $_POST['facetime'] = input_clean($_POST['facetime'], $available_qr_codes['facetime']['max_length']);

                /* Normalize a valid FaceTime recipient */
                if(filter_var($_POST['facetime'], FILTER_VALIDATE_EMAIL)) {
                    $_POST['facetime'] = input_clean_email($_POST['facetime']);
                } else {
                    $facetime_phone = preg_replace('/[\s().-]/', '', $_POST['facetime']);
                    if(preg_match('/^\+?[\d\s().-]+$/', $_POST['facetime']) && preg_match('/^\+[1-9]\d{6,14}$/', $facetime_phone)) {
                        $_POST['facetime'] = $facetime_phone;
                    } elseif($_POST['facetime'] !== '') {
                        Response::json(l('qr_codes.error_message.invalid_facetime'), 'error');
                    }
                }
            ...
            /* FaceTime Audio */
            case 'facetime_audio':
                $_POST['facetime_audio'] = input_clean($_POST['facetime_audio'], $available_qr_codes['facetime_audio']['max_length']);

                /* Normalize a valid FaceTime recipient */
                if(filter_var($_POST['facetime_audio'], FILTER_VALIDATE_EMAIL)) {
                    $_POST['facetime_audio'] = input_clean_email($_POST['facetime_audio']);
                } else {
                    $facetime_audio_phone = preg_replace('/[\s().-]/', '', $_POST['facetime_audio']);
                    if(preg_match('/^\+?[\d\s().-]+$/', $_POST['facetime_audio']) && preg_match('/^\+[1-9]\d{6,14}$/', $facetime_audio_phone)) {
                        $_POST['facetime_audio'] = $facetime_audio_phone;
                    } elseif($_POST['facetime_audio'] !== '') {
                        Response::json(l('qr_codes.error_message.invalid_facetime'), 'error');
                    }
                }

                $data = 'facetime-audio:' . $_POST['facetime_audio'];
                break;
            ...
            case 'wifi':
                ...
                /* Escape special WiFi characters */
                $wifi_ssid = str_replace(['\\', ';', ',', '"', ':'], ['\\\\', '\;', '\,', '\"', '\:'], $_POST['wifi_ssid']);
                $wifi_password = str_replace(['\\', ';', ',', '"', ':'], ['\\\\', '\;', '\,', '\"', '\:'], $_POST['wifi_password']);

                $data_to_be_rendered = 'WIFI:S:' . $wifi_ssid . ';';
                $data_to_be_rendered .= 'T:' . $_POST['wifi_encryption'] . ';';
                if($_POST['wifi_password']) $data_to_be_rendered .= 'P:' . $wifi_password . ';';
                if($_POST['wifi_is_hidden']) $data_to_be_rendered .= 'H:true;';
                $data_to_be_rendered .= ';';
            ...
            case 'event':
                ...
                /* Escape special calendar characters */
                $event = str_replace(['\\', "\r\n", "\r", "\n", ';', ','], ['\\\\', '\n', '\n', '\n', '\;', '\,'], $_POST['event']);
                $event_location = str_replace(['\\', "\r\n", "\r", "\n", ';', ','], ['\\\\', '\n', '\n', '\n', '\;', '\,'], $_POST['event_location']);
                $event_note = str_replace(['\\', "\r\n", "\r", "\n", ';', ','], ['\\\\', '\n', '\n', '\n', '\;', '\,'], $_POST['event_note']);
                $event_url = str_replace(["\r", "\n"], '', $_POST['event_url']);

                $data_to_be_rendered = 'BEGIN:VEVENT' . "\n";
                $data_to_be_rendered .= 'UID:' . string_generate(32) . "\n";
                $data_to_be_rendered .= 'SUMMARY:' . $event . "\n";
                $data_to_be_rendered .= 'LOCATION:' . $event_location . "\n";
                $data_to_be_rendered .= 'URL:' . $event_url . "\n";
                $data_to_be_rendered .= 'DESCRIPTION:' . $event_note . "\n";
            ...
            case 'pix':
                ...
                function generatePixCode($key, $name, $city, $amount = null, $description = "", $transactionID = "***") {
...
//                    }
                    ...
                    $name = mb_strcut($name, 0, 25, 'UTF-8');
                    $city = mb_strcut($city, 0, 15, 'UTF-8');
                    $transactionID = mb_strcut($transactionID, 0, 25, 'UTF-8');
                    ...
                    if($description !== "") {
                        $description_max_length = max(0, 73 - strlen($key));
                        $merchant_account_information[2] = mb_strcut($description, 0, $description_max_length, 'UTF-8');
                    }
                ...
                $_POST['pix_currency'] = isset($_POST['pix_currency']) && in_array($_POST['pix_currency'], ['BRL']) ? $_POST['pix_currency'] : 'BRL';
                $_POST['pix_amount'] = isset($_POST['pix_amount']) && $_POST['pix_amount'] !== '' ? (float) $_POST['pix_amount'] : null;
        ...
        if(($_POST['qr_code_foreground'] || $qr_code_foreground) && !isset($_POST['qr_code_foreground_remove'])) {
            ...
            if($_POST['qr_code_foreground']) {
                ...
                if(!preg_match('/^[a-zA-Z0-9._-]+$/D', $qr_code_foreground_name) || !in_array($qr_code_foreground_extension, Uploads::get_whitelisted_file_extensions('qr_code_foreground'))) {
                    Response::json(l('global.error_message.invalid_file_type'), 'error');
                }

                /* Use the configured uploads location */
                $qr_code_foreground_link = !\Altum\Plugin::is_active('offload') || (\Altum\Plugin::is_active('offload') && !settings()->offload->uploads_url) ? Uploads::get_full_path('qr_code_foreground') . $qr_code_foreground_name : Uploads::get_full_url('qr_code_foreground') . $qr_code_foreground_name;
            }
            ...
            $qr_code_foreground_extension = mb_strtolower(pathinfo($qr_code_foreground_name, PATHINFO_EXTENSION));
            $foreground = file_get_contents($qr_code_foreground_link, false, $stored_image_context, 0, settings()->codes->background_size_limit * 1000000 + 1);

            /* Validate the foreground image */
            if($foreground === false || !@getimagesizefromstring($foreground)) {
                Response::json(l('global.error_message.invalid_file_type'), 'error');
            }

            if(strlen($foreground) > settings()->codes->background_size_limit * 1000000) {
                Response::json(sprintf(l('global.error_message.file_size_limit'), settings()->codes->background_size_limit), 'error');
            }
        ...
        if(($_POST['qr_code_logo'] || $qr_code_logo) && !isset($_POST['qr_code_logo_remove'])) {
            ...
            if($_POST['qr_code_logo']) {
                ...
                /* Validate the stored file name */
                if(!preg_match('/^[a-zA-Z0-9._-]+$/D', $qr_code_logo_name) || !in_array($qr_code_logo_extension, Uploads::get_whitelisted_file_extensions('qr_codes/logo'))) {
                    Response::json(l('global.error_message.invalid_file_type'), 'error');
                }

                /* Use the configured uploads location */
                $qr_code_logo_link = !\Altum\Plugin::is_active('offload') || (\Altum\Plugin::is_active('offload') && !settings()->offload->uploads_url) ? Uploads::get_full_path('qr_codes/logo') . $qr_code_logo_name : Uploads::get_full_url('qr_codes/logo') . $qr_code_logo_name;
            }
            ...
            $qr_code_logo_extension = mb_strtolower(pathinfo($qr_code_logo_name, PATHINFO_EXTENSION));
            $logo = file_get_contents($qr_code_logo_link, false, $stored_image_context, 0, settings()->codes->logo_size_limit * 1000000 + 1);
            $logo_details = $logo !== false ? @getimagesizefromstring($logo) : false;
            ...
            if(strlen($logo) > settings()->codes->logo_size_limit * 1000000) {
                Response::json(sprintf(l('global.error_message.file_size_limit'), settings()->codes->logo_size_limit), 'error');
            }
            ...
            /* Size of the logo */
            list($logo_width, $logo_height) = $logo_details;
        ...
        if(($_POST['qr_code_background'] || $qr_code_background) && !isset($_POST['qr_code_background_remove'])) {
            ...
            if($_POST['qr_code_background']) {
                ...
                /* Validate the stored file name */
                if(!preg_match('/^[a-zA-Z0-9._-]+$/D', $qr_code_background_name) || !in_array($qr_code_background_extension, Uploads::get_whitelisted_file_extensions('qr_code_background'))) {
                    Response::json(l('global.error_message.invalid_file_type'), 'error');
                }

                /* Use the configured uploads location */
                $qr_code_background_link = !\Altum\Plugin::is_active('offload') || (\Altum\Plugin::is_active('offload') && !settings()->offload->uploads_url) ? Uploads::get_full_path('qr_code_background') . $qr_code_background_name : Uploads::get_full_url('qr_code_background') . $qr_code_background_name;
            }
            ...
            $qr_code_background_extension = mb_strtolower(pathinfo($qr_code_background_name, PATHINFO_EXTENSION));
            $background = file_get_contents($qr_code_background_link, false, $stored_image_context, 0, settings()->codes->background_size_limit * 1000000 + 1);

            /* Validate the background image */
            if($background === false || !@getimagesizefromstring($background)) {
                Response::json(l('global.error_message.invalid_file_type'), 'error');
            }

            if(strlen($background) > settings()->codes->background_size_limit * 1000000) {
                Response::json(sprintf(l('global.error_message.file_size_limit'), settings()->codes->background_size_limit), 'error');
            }
        ...
        if($frame) {
            ...
            if($frame_text) {
                ...
                $text_svg_object = SVG::fromString('<svg><text x="' . $frame_text_x . '%" y="' . $frame_text_y . '%" dominant-baseline="middle" text-anchor="middle" style="font-weight:100;font-size: ' . $frame_text_px . 'px;fill:' . $frame_text_color . ';font-family:' . $frame_text_font . ';">' . $frame_text_escaped . '</text></svg>');
