...
class AiQrCodes extends Controller {
    ...
    public function index() {
        ...
        $paginator = (new \Altum\Paginator($total_rows, $filters->get_results_per_page(), $_GET['page'] ?? 1, url('ai-qr-codes?' . $filters->get_get() . '&page={{PAGE}}')));
        ...
        if(!Alerts::has_field_errors() && !Alerts::has_errors()) {
            ...
            switch($_POST['type']) {
                case 'delete':
                    ...
                    (new AiQrCode())->bulk_delete($_POST['selected'], $this->user->user_id);
