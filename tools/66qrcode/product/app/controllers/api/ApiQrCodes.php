...
class ApiQrCodes extends Controller {
    ...
    private function post() {
        ...
        if(!isset($this->user->plan_settings->enabled_qr_codes->{$_POST['type']}) || !$this->user->plan_settings->enabled_qr_codes->{$_POST['type']}) {
            $this->response_error(l('global.info_message.plan_feature_no_access'), 401);
        }
        ...
        switch ($_POST['type']) {
            ...
            case 'url':
                ...
                if($_POST['url_dynamic'] && $_POST['url_dynamic_existing_link']) {
                    ...
                    $_POST['url'] = $settings['url'] = (new \Altum\Models\Link())->get_link_full_url($link, $this->user);
                } else {
                    if(isset($_POST['link_id'])) {
                        $link = db()->where('link_id', $_POST['link_id'])->where('user_id', $this->user->user_id)->where('type', 'file', '<>')->getOne('links', ['link_id']);
            ...
            case 'facetime':
                ...
                $settings['facetime'] = $_POST['facetime'] = input_clean(isset($_POST['facetime']) ? $_POST['facetime'] : null, $available_qr_codes['facetime']['max_length']);
                ...
                if($_POST['facetime'] !== '') {
                    if(filter_var($_POST['facetime'], FILTER_VALIDATE_EMAIL)) {
                        ...
                        if(preg_match('/^\+?[\d\s().-]+$/', $_POST['facetime']) && preg_match('/^\+[1-9]\d{6,14}$/', $facetime_phone)) {
                            ...
                            $this->response_error(l('qr_codes.error_message.invalid_facetime'), 401);
                        }
                    }
                }
                break;
            ...
            case 'facetime_audio':
                ...
                $settings['facetime_audio'] = $_POST['facetime_audio'] = input_clean(isset($_POST['facetime_audio']) ? $_POST['facetime_audio'] : null, $available_qr_codes['facetime_audio']['max_length']);
        ...
        if($_POST['is_bulk'] && $_POST['type'] == 'text') {
            ...
            $data = [
                ...
                'name' => $_POST['name'],
                'type' => $_POST['type'],
                'qr_code' => \Altum\Uploads::get_full_url('qr_codes') . $qr_code,
                'qr_code_logo' => \Altum\Uploads::get_full_url('qr_codes/logo') . $qr_code_logo,
    ...
    private function patch() {
        ...
        if($_POST['type'] == 'file' || $qr_code->type == 'file') {
            $this->response_error(l('qr_codes.error_message.file_api_not_available'), 400);
        }
        ...
        switch($_POST['type']) {
            ...
            case 'url':
                ...
                $_POST['url_dynamic'] = isset($_POST['url_dynamic']) ? (int) (bool) $_POST['url_dynamic'] : (int) $qr_code->link_id;
                $_POST['url_dynamic_existing_link'] = $_POST['url_dynamic'] ? (isset($_POST['url_dynamic_existing_link']) ? (int) (bool) $_POST['url_dynamic_existing_link'] : (int) ($qr_code->link_id && (!isset($qr_code->settings->url_dynamic_existing_link) || $qr_code->settings->url_dynamic_existing_link))) : 0;
                $settings['url_dynamic_existing_link'] = $_POST['url_dynamic_existing_link'];

                if($_POST['url_dynamic'] && $_POST['url_dynamic_existing_link']) {
                    $link_id = $_POST['link_id'] ?? $qr_code->link_id;
                    $link = db()->where('link_id', $link_id)->where('user_id', $this->user->user_id)->where('type', 'file', '<>')->getOne('links', ['link_id', 'domain_id', 'url']);
            ...
            case 'facetime':
                $settings['facetime'] = $_POST['facetime'] = input_clean(isset($_POST['facetime']) ? $_POST['facetime'] : $qr_code->settings->facetime, $available_qr_codes['facetime']['max_length']);
            ...
            /* FaceTime Audio */
            case 'facetime_audio':
                $settings['facetime_audio'] = $_POST['facetime_audio'] = input_clean(isset($_POST['facetime_audio']) ? $_POST['facetime_audio'] : $qr_code->settings->facetime_audio, $available_qr_codes['facetime_audio']['max_length']);
        ...
        if($_POST['type'] == 'url' && ($_POST['url_dynamic'] ?? false) && !($_POST['url_dynamic_existing_link'] ?? false)) {
            if($qr_code->link_id && isset($qr_code->settings->url_dynamic_existing_link) && !$qr_code->settings->url_dynamic_existing_link) {
                if($link = db()->where('link_id', $qr_code->link_id)->where('user_id', $this->user->user_id)->getOne('links', ['link_id', 'url'])) {
                    ...
                    $_POST['link_id'] = $link->link_id;
                    $qr_code_settings['url'] = SITE_URL . $link->url . '/';
                }
            } else {
                $total_links_rows = database()->query("SELECT COUNT(*) AS `total` FROM `links` WHERE `user_id` = {$this->user->user_id}")->fetch_object()->total ?? 0;
                ...
                $qr_code_settings['url'] = SITE_URL . $url . '/';
            }

            if(isset($qr_code_settings['url'])) {
                $settings['url_dynamic_existing_link'] = 1;
                $settings['url'] = $qr_code_settings['url'];
            }
        }
