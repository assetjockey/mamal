...
class ApiBarcodes extends Controller {
    ...
    private function get_all() {
        ...
        $paginator = (new \Altum\Paginator($total_rows, $filters->get_results_per_page(), $_GET['page'] ?? 1, url('api/barcodes?' . $filters->get_get() . '&page={{PAGE}}')));
    ...
    private function get() {
        ...
        $barcode = db()->where('barcode_id', $barcode_id)->where('user_id', $this->user->user_id)->getOne('barcodes');
    ...
    private function post() {
        ...
        $total_rows = db()->where('user_id', $this->user->user_id)->getValue('barcodes', 'count(`barcode_id`)');
        ...
        $_POST['value'] = input_clean($_POST['value'], $_POST['is_bulk'] ? 10000 : 64);
        ...
        /* Check the bulk plan access */
        if($_POST['is_bulk'] && !$this->user->plan_settings->barcodes_bulk_limit) {
            $this->response_error(l('global.info_message.plan_feature_no_access'), 401);
        }
        ...
        if($_POST['is_bulk']) {
            ...
            foreach($data_rows as $data_row) {
                ...
                if($this->user->plan_settings->barcodes_bulk_limit != -1 && $i >= $this->user->plan_settings->barcodes_bulk_limit) {
                    break;
                }
                $i++;
            }
            ...
            $data = [
                ...
                'type' => $_POST['type'],
                'value' => $_POST['value'],
                'barcode' => \Altum\Uploads::get_full_url('barcodes') . $barcode,
    ...
    private function patch() {
        ...
        if($this->user->plan_settings->barcodes_limit != -1 && $total_rows > $this->user->plan_settings->barcodes_limit) {
            $this->response_error(sprintf(settings()->payment->is_enabled ? l('global.info_message.plan_feature_limit_removal_with_upgrade') : l('global.info_message.plan_feature_limit_removal'), $total_rows - $this->user->plan_settings->barcodes_limit, mb_strtolower(l('barcodes.title')), l('global.info_message.plan_upgrade')), 401);
        }
