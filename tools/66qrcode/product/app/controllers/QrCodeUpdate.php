...
class QrCodeUpdate extends Controller {
    ...
    public function index() {
        ...
        /* Get the linked file */
        $file_link = $qr_code->link_id ? db()->where('link_id', $qr_code->link_id)->where('user_id', $this->user->user_id)->where('type', 'file')->getOne('links') : null;
        if($file_link) {
            $file_link->full_url = (new \Altum\Models\Link())->get_link_full_url($file_link, $this->user);
        }
        ...
        if(!empty($_POST)) {
            ...
            switch($_POST['type']) {
                ...
                case 'url':
                    ...
                    if($_POST['url_dynamic'] && $_POST['url_dynamic_existing_link']) {
                        $link = db()->where('link_id', $_POST['link_id'] ?? null)->where('user_id', $this->user->user_id)->where('type', 'file', '<>')->getOne('links', ['link_id']);
                        ...
                        if($_POST['url_dynamic']) {
                            $_POST['url'] = $settings['url'] = get_url($_POST['url']);

                            if(empty(trim($_POST['url']))) {
                                Alerts::add_field_error('url', l('global.error_message.empty_fields'));
                            }

                            $url_details = parse_url($_POST['url']);

                            if(!isset($url_details['scheme'])) {
                                Alerts::add_field_error('url', l('links.error_message.invalid_location_url'));
                            }

                            $domain = get_domain_from_url($_POST['url']);

                            if($domain && in_array($domain, settings()->links->blacklisted_domains)) {
                                Alerts::add_field_error('url', l('links.error_message.blacklisted_domain'));
                            }
                            ...
                            if(settings()->links->google_safe_browsing_is_enabled) {
                                if(google_safe_browsing_check($_POST['url'], settings()->links->google_safe_browsing_api_key)) {
                                    Alerts::add_field_error('url', l('links.error_message.blacklisted_location_url'));
                                }
                            }
                        }
                    }
                ...
                case 'file':
                    if(!$this->user->plan_settings->enabled_qr_codes->file) {
                        Alerts::add_field_error('file', l('global.info_message.plan_feature_no_access'));
                    }

                    if(empty($_FILES['file']['name']) && !$file_link) {
                        Alerts::add_field_error('file', l('global.error_message.empty_field'));
                    }

                    if(empty($_FILES['file']['name']) && $file_link && isset($_POST['file_remove'])) {
                        Alerts::add_field_error('file', l('global.error_message.empty_field'));
                    }

                    if(!empty($_FILES['file']['name'])) {
                        unset($_POST['file_remove']);
                        $file_size_limit = $this->user->plan_settings->qr_code_file_size_limit == -1 ? get_max_upload() : min(get_max_upload(), $this->user->plan_settings->qr_code_file_size_limit);

                        if(!$file_size_limit) {
                            Alerts::add_field_error('file', l('global.info_message.plan_feature_no_access'));
                        } else {
                            Uploads::validate_upload('qr_code_files', 'file', $file_size_limit);
                        }

                    }

                    $settings['file_url'] = $file_link->full_url ?? SITE_URL;
                    break;
                ...
                case 'facetime':
                    ...
                    /* Validate and normalize the FaceTime recipient */
                    if($_POST['facetime'] !== '') {
                        if(filter_var($_POST['facetime'], FILTER_VALIDATE_EMAIL)) {
                            $settings['facetime'] = $_POST['facetime'] = input_clean_email($_POST['facetime']);
                        } else {
                            $facetime_phone = preg_replace('/[\s().-]/', '', $_POST['facetime']);

                            if(preg_match('/^\+?[\d\s().-]+$/', $_POST['facetime']) && preg_match('/^\+[1-9]\d{6,14}$/', $facetime_phone)) {
                                $settings['facetime'] = $_POST['facetime'] = $facetime_phone;
                            } else {
                                Alerts::add_field_error('facetime', l('qr_codes.error_message.invalid_facetime'));
                            }
                        }
                    }
                    break;
                ...
                /* FaceTime Audio */
                case 'facetime_audio':
                    $required_fields[] = 'facetime_audio';
                    $settings['facetime_audio'] = $_POST['facetime_audio'] = input_clean($_POST['facetime_audio'], $available_qr_codes['facetime_audio']['max_length']);

                    /* Validate and normalize the FaceTime recipient */
                    if($_POST['facetime_audio'] !== '') {
                        if(filter_var($_POST['facetime_audio'], FILTER_VALIDATE_EMAIL)) {
                            $settings['facetime_audio'] = $_POST['facetime_audio'] = input_clean_email($_POST['facetime_audio']);
                        } else {
                            ...
                            if(preg_match('/^\+?[\d\s().-]+$/', $_POST['facetime_audio']) && preg_match('/^\+[1-9]\d{6,14}$/', $facetime_audio_phone)) {
                                $settings['facetime_audio'] = $_POST['facetime_audio'] = $facetime_audio_phone;
                            } else {
                                Alerts::add_field_error('facetime_audio', l('qr_codes.error_message.invalid_facetime'));
                            }
                        }
                    }
                    break;
            ...
            /* Clean the QR code image */
            if(!Alerts::has_field_errors() && !Alerts::has_errors() && $_POST['qr_code']) {
                $qr_code_prefix = 'data:image/svg+xml;base64,';
                $qr_code_svg = mb_substr($_POST['qr_code'], 0, mb_strlen($qr_code_prefix)) == $qr_code_prefix ? base64_decode(mb_substr($_POST['qr_code'], mb_strlen($qr_code_prefix)), true) : false;

                if($qr_code_svg !== false) {
                    $svg_sanitizer = new \enshrined\svgSanitize\Sanitizer();
                    $svg_sanitizer->removeRemoteReferences(true);
                    $qr_code_svg = $svg_sanitizer->sanitize($qr_code_svg);
                    $qr_code_svg = $qr_code_svg ? Uploads::clean_svg($qr_code_svg, true) : false;
                }

                if(!$qr_code_svg) {
                    Alerts::add_error(l('global.error_message.invalid_file_type'));
                } else {
                    $_POST['qr_code'] = $qr_code_prefix . base64_encode($qr_code_svg);
                }
            }
            ...
            if(!Alerts::has_field_errors() && !Alerts::has_errors()) {
                ...
                $created_file_link = false;

                if($_POST['type'] == 'file') {
                    if(!empty($_FILES['file']['name'])) {
                        $new_file_name = input_clean(basename($_FILES['file']['name']), 256);
                        $new_file_size = (int) $_FILES['file']['size'];
                        $new_file_mime_type = mime_content_type($_FILES['file']['tmp_name']);
                        $new_file = Uploads::process_upload(null, 'qr_code_files', 'file', 'file_remove', $file_size_limit);
                    }
                    ...
                    if(!$file_link) {
                        ...
                        if($this->user->plan_settings->links_limit != -1 && $total_links_rows >= $this->user->plan_settings->links_limit) {
                            Alerts::add_field_error('file', l('global.info_message.plan_feature_limit'));
                        } else if($new_file) {
                            ...
                            $file_link = db()->where('link_id', $_POST['link_id'])->getOne('links');
                            $file_link->full_url = SITE_URL . $url . '/';
                            $new_file = null;
                            $created_file_link = true;
                        ...
                        $_POST['link_id'] = $file_link->link_id;
                    }

                    if((Alerts::has_field_errors() || Alerts::has_errors()) && $new_file) {
                        Uploads::delete_uploaded_file($new_file, 'qr_code_files');
                        $new_file = null;
                    }

                    if(!Alerts::has_field_errors() && !Alerts::has_errors() && $file_link) {
                        $settings['file_url'] = $file_link->full_url;
                        ...
                        /* Attach stored image names */
                        if($qr_code->qr_code_logo) $request_data['qr_code_logo'] = $qr_code->qr_code_logo;
                        if($qr_code->qr_code_background) $request_data['qr_code_background'] = $qr_code->qr_code_background;
                        if($qr_code->qr_code_foreground) $request_data['qr_code_foreground'] = $qr_code->qr_code_foreground;
                        ...
                        if(Alerts::has_field_errors() || Alerts::has_errors()) {
                            if($created_file_link) {
                                (new \Altum\Models\Link())->delete($file_link->link_id);
                                $_POST['link_id'] = null;
                                $file_link = null;
                            } else if($new_file) {
                                Uploads::delete_uploaded_file($new_file, 'qr_code_files');
                            ...
                            $_POST['qr_code'] = $response->body->details->data ?? $_POST['qr_code'];
                            $_POST['embedded_data'] = $file_link->full_url;
                        }
                    }
                }

                if($_POST['type'] == 'url' && ($_POST['url_dynamic'] ?? false) && !($_POST['url_dynamic_existing_link'] ?? false)) {
                    if($qr_code->link_id && isset($qr_code->settings->url_dynamic_existing_link) && !$qr_code->settings->url_dynamic_existing_link) {
                        if($link = db()->where('link_id', $qr_code->link_id)->where('user_id', $this->user->user_id)->getOne('links', ['link_id', 'url'])) {
                            db()->where('link_id', $qr_code->link_id)->where('user_id', $this->user->user_id)->update('links', [
                                'project_id' => $_POST['project_id'],
                            ...
                            $dynamic_link_full_url = SITE_URL . $link->url . '/';
                        }
                    } else {
                        $total_links_rows = database()->query("SELECT COUNT(*) AS `total` FROM `links` WHERE `user_id` = {$this->user->user_id}")->fetch_object()->total ?? 0;
                        ...
                        if($this->user->plan_settings->links_limit != -1 && $total_links_rows >= $this->user->plan_settings->links_limit) {
                            ...
                            $dynamic_link_full_url = SITE_URL . $url . '/';
                        }
                    }

                    if(!Alerts::has_field_errors() && !Alerts::has_errors() && isset($dynamic_link_full_url)) {
                        $settings['url_dynamic_existing_link'] = 1;
                        $settings['url'] = $dynamic_link_full_url;
                        $qr_code_settings = $settings;
                        $qr_code_settings['url'] = $dynamic_link_full_url;
                        ...
                        $_POST['qr_code'] = $response->body->details->data ?? $_POST['qr_code'];
                        $_POST['embedded_data'] = $dynamic_link_full_url;
                    }
                }
                ...
                if((Alerts::has_field_errors() || Alerts::has_errors()) && $_POST['type'] == 'file') {
                    if($created_file_link && $file_link) {
                        (new \Altum\Models\Link())->delete($file_link->link_id);
                        $_POST['link_id'] = null;
                        $file_link = null;
                    } else if($new_file) {
                ...
                if(!Alerts::has_field_errors() && !Alerts::has_errors()) {
                    ...
                    if($_POST['type'] == 'file' && $file_link) {
                        if($new_file) {
                            db()->where('link_id', $file_link->link_id)->update('links', [
                                'project_id' => $_POST['project_id'],
                                'file' => $new_file,
                                'file_name' => $new_file_name,
                                'file_size' => $new_file_size,
                            ...
                            Uploads::delete_uploaded_file($file_link->file, 'qr_code_files');
                        } else {
                            db()->where('link_id', $file_link->link_id)->update('links', [
                                'project_id' => $_POST['project_id'],
                                'name' => $_POST['name'],
                        ...
                        cache()->deleteItemsByTag('link_id=' . $file_link->link_id);
                        cache()->deleteItem('links?user_id=' . $this->user->user_id);
                        cache()->deleteItem('links_total?user_id=' . $this->user->user_id);
                        cache()->deleteItem('links_dashboard?user_id=' . $this->user->user_id);
                    }

                    if($qr_code->type == 'file' && $_POST['type'] != 'file' && $file_link) {
                        (new \Altum\Models\Link())->delete($file_link->link_id);
                    }
