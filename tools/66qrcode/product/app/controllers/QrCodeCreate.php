                ...
                if(!Alerts::has_field_errors() && !Alerts::has_errors()) {
                    ...
                    if($_POST['type'] == 'file') {
                        ...
                        if($this->user->plan_settings->links_limit != -1 && $total_links_rows >= $this->user->plan_settings->links_limit) {
                            ...
                            $file_name = input_clean(basename($_FILES['file']['name']), 256);
                            $file_size = (int) $_FILES['file']['size'];
                            $file_mime_type = mime_content_type($_FILES['file']['tmp_name']);
                            $file = Uploads::process_upload(null, 'qr_code_files', 'file', 'file_remove', $file_size_limit);

                            if(!Alerts::has_field_errors() && !Alerts::has_errors() && $file) {
                                ...
                                $file_link_full_url = SITE_URL . $url . '/';
                                $settings['file_url'] = $file_link_full_url;
                                ...
                                /* Attach uploaded image names */
                                if($qr_code_logo) $request_data['qr_code_logo'] = $qr_code_logo;
                                if($qr_code_background) $request_data['qr_code_background'] = $qr_code_background;
                                if($qr_code_foreground) $request_data['qr_code_foreground'] = $qr_code_foreground;
                                ...
                                if(Alerts::has_field_errors() || Alerts::has_errors()) {
                                    (new \Altum\Models\Link())->delete($file_link_id);
                                    $file_link_id = null;
                                    $_POST['link_id'] = null;
                                } else {
                                    $_POST['qr_code'] = $response->body->details->data ?? $_POST['qr_code'];
                                    $_POST['embedded_data'] = $file_link_full_url;
                                }
                            }
                        }
                    }
                    ...
                    if((Alerts::has_field_errors() || Alerts::has_errors()) && $file_link_id) {
                        (new \Altum\Models\Link())->delete($file_link_id);
                        $file_link_id = null;
                        $_POST['link_id'] = null;
                    }
