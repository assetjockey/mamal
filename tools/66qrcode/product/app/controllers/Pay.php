...
class Pay extends Controller {
		...
		if(!empty($_POST) && !$this->return_type) {
			...
			if(settings()->payment->codes_is_enabled && isset($_POST['code'])) {
				...
				if($this->code) {
					...
					if($this->code && !is_array($this->code->plans_ids)) {
						$this->code->plans_ids = [];
					}

					if($this->code && !in_array($this->plan_id, $this->code->plans_ids)) {
						$this->code = null;
					}
				} else {
					$this->code = null;
				...
				try {
					$razorpay_payment_link_response = $razorpay_api->paymentLink->create([
						...
						'notes' => [
							...
							'discount_amount' => $discount_amount,
							'taxes_ids' => json_encode($this->applied_taxes_ids),
							'webhook_identifier' => settings()->razorpay->webhook_identifier,
						],
						'callback_url' => url('pay/' . $this->plan_id . $this->return_url_parameters('success', $base_amount, $formatted_price, $code, $discount_amount)),
						'callback_method' => 'get'
					]);
	...
	private function onepay() {
		...
		/* 1pay expects the amount in the smallest currency unit */
		$onepay_amount = in_array(currency(), get_zero_decimal_currencies_array()) ? (int) round($price_with_taxes) : (int) round($price_with_taxes * 100);
		$formatted_price = in_array(currency(), get_zero_decimal_currencies_array()) ? number_format($price_with_taxes, 0, '.', '') : number_format($price_with_taxes, 2, '.', '');
		...
		/* Keep the reference short as 1pay sends it back in the webhook */
		$payment_frequency = [
			'monthly' => 'm',
			'quarterly' => 'q',
			'biannual' => 'b',
			'annual' => 'a',
			'lifetime' => 'l',
		][$_POST['payment_frequency']] ?? $_POST['payment_frequency'];
		...
		$reference_id = implode('|', [
			...
			rawurlencode($code),
			$discount_amount,
			base64_encode(json_encode($this->applied_taxes_ids)),
		]);

		/* Send known customer details */
		$billing = is_object($this->user->billing) ? $this->user->billing : new \StdClass();
		$onepay_customer_fields = [
			'email' => [
		...
		if(!empty($billing->name)) {
			$billing_name_parts = explode(' ', trim($billing->name), 2);

			if(isset($billing->type) && $billing->type == 'business') {
				$onepay_customer_fields['company'] = [
					'value' => $billing->name,
				];
			} else {
				$onepay_customer_fields['forename'] = [
					'value' => $billing_name_parts[0],
				];

				if(isset($billing_name_parts[1])) {
					$onepay_customer_fields['surname'] = [
						'value' => $billing_name_parts[1],
					];
				}
			}
		} else {
			$user_name_parts = explode(' ', trim($this->user->name), 2);

			$onepay_customer_fields['forename'] = [
				'value' => $user_name_parts[0],
			];

			if(isset($user_name_parts[1])) {
				$onepay_customer_fields['surname'] = [
					'value' => $user_name_parts[1],
				];
			}
		}

		if(!empty($billing->address)) {
			$onepay_customer_fields['street'] = [
				'value' => $billing->address,
			];
		}

		if(!empty($billing->zip)) {
			$onepay_customer_fields['postcode'] = [
				'value' => $billing->zip,
			];
		}

		if(!empty($billing->city)) {
			$onepay_customer_fields['place'] = [
				'value' => $billing->city,
			];
		}

		if(!empty($billing->country)) {
			$onepay_customer_fields['country'] = [
				'value' => $billing->country,
			];
		}

		if(!empty($billing->phone)) {
			$onepay_customer_fields['phone'] = [
				'value' => $billing->phone,
			];
		}

		if(!empty($billing->tax_id)) {
			$onepay_customer_fields['custom_field_1'] = [
				'name' => 'Tax ID',
		...
		$onepay_gateway_payload = [
			'amount' => $onepay_amount,
			'currency' => currency(),
			'purpose' => settings()->business->brand_name . ' - ' . $this->plan->name . ' - ' . l('plan.custom_plan.' . $_POST['payment_frequency']),
			'referenceId' => $reference_id,
			'fields' => $onepay_customer_fields,
			'language' => \Altum\Language::$code,
			'successRedirectUrl' => url('pay/' . $this->plan_id . $this->return_url_parameters('success', $base_amount, $formatted_price, $code, $discount_amount)),
			'failedRedirectUrl' => url('pay/' . $this->plan_id . $this->return_url_parameters('cancel', $base_amount, $formatted_price, $code, $discount_amount)),
			'cancelRedirectUrl' => url('pay/' . $this->plan_id . $this->return_url_parameters('cancel', $base_amount, $formatted_price, $code, $discount_amount)),
			'skipResultPage' => true,
		];

		/* Apply a custom payment theme */
		if(!empty(settings()->onepay->look_and_feel_profile)) {
			$onepay_gateway_payload['lookAndFeelProfile'] = settings()->onepay->look_and_feel_profile;
		}

		/* Add managed subscription details */
		if($_POST['payment_type'] === 'recurring') {
			/* 1pay uses ISO 8601 subscription intervals */
			$subscription_interval = match ($_POST['payment_frequency']) {
				'monthly' => 'P1M',
				'quarterly' => 'P3M',
				'biannual' => 'P6M',
				'annual' => 'P1Y',
			...
			/* Use custom cancellation notice */
			$subscription_cancellation_interval = $subscription_interval;

			if(!empty(settings()->onepay->subscription_cancellation_interval)) {
				$subscription_cancellation_interval = settings()->onepay->subscription_cancellation_interval;
			}

			$onepay_gateway_payload['subscriptionState'] = true;
			$onepay_gateway_payload['subscriptionInterval'] = $subscription_interval;
			$onepay_gateway_payload['subscriptionPeriod'] = $subscription_interval;
			$onepay_gateway_payload['subscriptionCancellationInterval'] = $subscription_cancellation_interval;
		}

		/* Generate the payment link */
		try {
			$onepay_response = \Unirest\Request::post(
				Onepay::$api_url . 'Gateway/?instance=' . rawurlencode(settings()->onepay->instance_name),
				Onepay::get_headers(),
				\Unirest\Request\Body::json($onepay_gateway_payload)
			);
		} catch (\Exception $exception) {
			$onepay_error_message = (DEBUG || \Altum\Authentication::is_admin()) ? $exception->getMessage() : l('pay.error_message.failed_payment');
			Alerts::add_error($onepay_error_message);
			redirect('pay/' . $this->plan_id . '?' . $trial_skip_parameter . $discount_code_parameter);
		}

		/* Get the checkout url from the 1pay response */
		$onepay_payment_data = $onepay_response->body ?? null;

		if(isset($onepay_response->body->data)) {
			$onepay_payment_data = is_array($onepay_response->body->data) ? ($onepay_response->body->data[0] ?? null) : $onepay_response->body->data;
		}

		$onepay_payment_url = $onepay_payment_data->link ?? ($onepay_payment_data->url ?? null);

		if(!$onepay_payment_url && isset($onepay_payment_data->hash)) {
			$onepay_payment_url = 'https://' . settings()->onepay->instance_name . '.1pay.ch/?payment=' . $onepay_payment_data->hash;
		}

		if($onepay_response->code >= 400 || !$onepay_payment_url) {
			$onepay_error_message = (DEBUG || \Altum\Authentication::is_admin()) ? ($onepay_response->raw_body ?: print_r($onepay_response->body, true)) : l('pay.error_message.failed_payment');
			Alerts::add_error($onepay_error_message);
			redirect('pay/' . $this->plan_id . '?' . $trial_skip_parameter . $discount_code_parameter);
		}

		/* Redirect to 1pay checkout */
		header('Location: ' . $onepay_payment_url); die();
