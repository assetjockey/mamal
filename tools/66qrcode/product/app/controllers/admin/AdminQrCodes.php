...
class AdminQrCodes extends Controller {
    ...
    public function index() {
        ...
        $filters = (new \Altum\Filters(['qr_code_id', 'user_id', 'project_id', 'type'], ['name'], ['qr_code_id', 'type', 'last_datetime', 'name', 'datetime'], allowed_datetime_fields: ['datetime', 'last_datetime']));
        ...
        if(!Alerts::has_field_errors() && !Alerts::has_errors()) {
            ...
            switch($_POST['type']) {
                case 'delete':
                    ...
                    (new QrCode())->bulk_delete($_POST['selected'], null, true);
    ...
    public function transfer() {
        ...
        if(!$qr_code = db()->where('qr_code_id', $qr_code_id)->getOne('qr_codes', ['qr_code_id', 'link_id', 'user_id', 'name', 'type'])) {
            throw_404();
        }
        ...
        if(!Alerts::has_field_errors() && !Alerts::has_errors()) {
            ...
            if($qr_code->type == 'file' && $qr_code->link_id) {
                db()->where('link_id', $qr_code->link_id)->update('links', [
                    'user_id' => $new_user->user_id,
                ]);
            }
