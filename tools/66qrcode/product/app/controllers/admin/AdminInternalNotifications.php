...
class AdminInternalNotifications extends Controller {
    ...
    public function index() {
        ...
        $filters = (new \Altum\Filters(['internal_notification_id', 'user_id', 'from_who', 'for_who', 'is_read'], ['title', 'description'], ['internal_notification_id', 'datetime', 'read_datetime'], allowed_datetime_fields: ['datetime', 'read_datetime']));
    ...
    public function get_segment_count() {
        ...
        switch($segment) {
            ...
            case 'subscribers':
                ...
                $count = db()->where('is_broadcast_subscribed', 1)->getValue('users', 'COUNT(*)');
            ...
            case 'filter':
                ...
                /* Is subscribed */
                $_GET['filters_is_broadcast_subscribed'] = isset($_GET['filters_is_broadcast_subscribed']) ? (bool) $_GET['filters_is_broadcast_subscribed'] : 0;

                if($_GET['filters_is_broadcast_subscribed']) {
                    $has_filters = true;
                    $query->where('is_broadcast_subscribed', 1);
                }
