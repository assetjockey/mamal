...
class AdminStatistics extends Controller {
    ...
    protected function email_reports() {
        ...
        $email_reports_chart = [];
        $result = database()->query("SELECT COUNT(*) AS `total`, DATE_FORMAT({$convert_tz_sql}, '{$this->datetime['query_date_format']}') AS `formatted_date` FROM `email_reports` WHERE `datetime` >= '{$this->datetime['query_start_date']}' AND `datetime` < '{$this->datetime['query_end_date']}' GROUP BY `formatted_date`");
        while($row = $result->fetch_object()) {
            $row->formatted_date = $this->datetime['process']($row->formatted_date, true);
        ...
        /* Empty chart */
        if($email_reports_chart['is_empty']) {
            $email_reports_chart['email_reports'] = '[]';
        }
    ...
    protected function teams() {
        ...
        $teams_chart = [];
        $result = database()->query("SELECT COUNT(*) AS `total`, DATE_FORMAT({$convert_tz_sql}, '{$this->datetime['query_date_format']}') AS `formatted_date` FROM `teams` WHERE `datetime` >= '{$this->datetime['query_start_date']}' AND `datetime` < '{$this->datetime['query_end_date']}' GROUP BY `formatted_date`");
        while($row = $result->fetch_object()) {
    ...
    protected function teams_members() {
        ...
        $teams_members_chart = [];
        $result = database()->query("SELECT COUNT(*) AS `total`, DATE_FORMAT({$convert_tz_sql}, '{$this->datetime['query_date_format']}') AS `formatted_date` FROM `teams_members` WHERE `datetime` >= '{$this->datetime['query_start_date']}' AND `datetime` < '{$this->datetime['query_end_date']}' GROUP BY `formatted_date`");
        while($row = $result->fetch_object()) {
            $row->formatted_date = $this->datetime['process']($row->formatted_date);
