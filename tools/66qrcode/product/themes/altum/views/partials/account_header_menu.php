...
<div class="d-lg-none mb-4">
    ...
    <?php if(settings()->payment->is_enabled): ?>
        ...
        <?php if(\Altum\Plugin::is_active('affiliate') && settings()->affiliate->is_enabled): ?>
            <option value="<?= url('referrals') ?>" <?= in_array(\Altum\Router::$controller_key, ['referrals', 'referrals-users']) ? 'selected="selected"' : null ?>>🤝 <?= l('referrals.menu') ?></option>
        <?php endif ?>
    <?php endif ?>
...
<div class="row d-none d-lg-flex mb-4 mx-n2">
    ...
    <?php if(settings()->payment->is_enabled): ?>
        ...
        <?php if(\Altum\Plugin::is_active('affiliate') && settings()->affiliate->is_enabled): ?>
            <div class="col-lg-4 p-2 text-truncate">
                <a class="btn btn-block btn-custom text-truncate  <?= in_array(\Altum\Router::$controller_key, ['referrals', 'referrals-users']) ? 'active' : null ?>" href="<?= url('referrals') ?>">
