...
<div class="container mt-5">
    <div id="qr_reader_notifications"></div>
...
<script>
    ...
    const qr_reader_notifications = document.querySelector('#qr_reader_notifications');

    const display_qr_reader_error = message => {
        const alert = document.createElement('div');
        const icon = document.createElement('i');
        const text = document.createTextNode(message);
        const close_button = document.createElement('button');

        qr_reader_notifications.innerHTML = '';
        ...
        qr_reader_notifications.appendChild(alert);
    };
    ...
    if(navigator.mediaDevices && typeof navigator.mediaDevices.getUserMedia === 'function') {
        ...
        document.querySelector('#start_scanner').addEventListener('click', async event => {
            ...
            try {
                ...
                display_qr_reader_error(<?= json_encode(l('qr_reader.camera_error_message')) ?>);
            }
    ...
    <script type="application/ld+json">
        {
            ...
            "itemListElement": [
                ...
                },
                {
                    "@type": "ListItem",
                    "position": 2,
                    "name": <?= json_encode(l('qr_reader.title')) ?>,
                    "item": <?= json_encode(url('qr-reader')) ?>
                }
            ]
        }
    </script>
