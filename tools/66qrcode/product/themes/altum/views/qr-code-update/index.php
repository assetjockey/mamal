...
<?php $qr_code_file_size_limit = ($this->user->plan_settings->qr_code_file_size_limit ?? 0) == -1 ? get_max_upload() : min(get_max_upload(), $this->user->plan_settings->qr_code_file_size_limit ?? 0) ?>
...
<div class="container">
    ...
    <div class="d-print-none">
        ...
        <div class="d-flex justify-content-between mb-4">
            ...
            <?= include_view(THEME_PATH . 'views/qr-codes/qr_code_dropdown_button.php', ['id' => $data->qr_code->qr_code_id, 'resource_name' => $data->qr_code->name, 'type' => $data->qr_code->type]) ?>
        </div>
    </div>
    ...
    <form id="form" action="" method="post" role="form" enctype="multipart/form-data">
        ...
        <input type="hidden" name="file_url" value="<?= $data->file_link->full_url ?? SITE_URL ?>" />
        ...
        <?php if(!empty($data->qr_code->qr_code_logo)): ?>
            <input type="hidden" name="qr_code_logo" value="<?= $data->qr_code->qr_code_logo ?>" />
        <?php endif ?>

        <?php if(!empty($data->qr_code->qr_code_background)): ?>
            <input type="hidden" name="qr_code_background" value="<?= $data->qr_code->qr_code_background ?>" />
        <?php endif ?>

        <?php if(!empty($data->qr_code->qr_code_foreground)): ?>
            <input type="hidden" name="qr_code_foreground" value="<?= $data->qr_code->qr_code_foreground ?>" />
        <?php endif ?>

        <?php /* QR code type selector */ ?>
        <div class="qr-code-types position-relative mb-4 d-none d-lg-block d-print-none">
            <div class="qr-code-types-wrapper-left"></div>
            <div class="qr-code-types-wrapper-right"></div>

            <div class="btn-group-toggle d-flex align-items-center qr-code-types-wrapper" data-toggle="buttons">
                <?php foreach($data->available_qr_codes as $key => $value): ?>
                    <label class="mr-3 my-1 btn btn-light font-size-small font-weight-500 qr-code-type-button <?= $data->qr_code->type == $key ? 'active' : null ?> <?= $this->user->plan_settings->enabled_qr_codes->{$key} ? null : 'disabled' ?>" data-toggle="tooltip" <?= $this->user->plan_settings->enabled_qr_codes->{$key} ? 'title="' . l('qr_codes.type.' . $key . '_description') . '"' : 'title="' . l('global.info_message.plan_feature_no_access') . '"' ?> data-tooltip-hide-on-click>
                ...
                <div class="card">
                    <div class="card-body">
                        ...
                        <div>
                            ...
                            <div class="form-group" data-type="url" data-url-dynamic-existing-link>
                                <div class="custom-control custom-checkbox">
                                    <input id="url_dynamic_existing_link" name="url_dynamic_existing_link" type="checkbox" class="custom-control-input" <?= ($data->qr_code->link_id ?? null) && (!isset($data->qr_code->settings->url_dynamic_existing_link) || $data->qr_code->settings->url_dynamic_existing_link) ? 'checked="checked"' : null ?> data-reload-qr-code />
                                    <label class="custom-control-label" for="url_dynamic_existing_link"><?= l('qr_codes.input.url_dynamic_existing_link') ?></label>
                                    <small class="form-text text-muted"><?= l('qr_codes.input.url_dynamic_existing_link_help') ?></small>
                                </div>
                            </div>
                        </div>

                        <div>
                            <div class="form-group" data-type="file" data-file-input-wrapper-size-limit="<?= $qr_code_file_size_limit ?>" data-file-input-wrapper-size-limit-error="<?= sprintf(l('global.error_message.file_size_limit'), $qr_code_file_size_limit) ?>">
                                <label for="file"><i class="fas fa-fw fa-file fa-sm text-muted mr-1"></i> <?= l('qr_codes.input.file') ?></label>

                                <?= include_view(THEME_PATH . 'views/partials/file_input.php', [
                                    'uploads_file_key' => 'qr_code_files',
                                    'file_key' => 'file',
                                    'already_existing_file' => $data->file_link->file ?? null,
                                    'custom_file_full_url' => $data->file_link->full_url ?? null,
                                    'is_required' => empty($data->file_link->file),
                                    'input_data' => 'data-reload-qr-code',
                                ]) ?>

                                <?= \Altum\Alerts::output_field_error('file') ?>
                                <small class="form-text text-muted"><?= l('qr_codes.input.file_help') ?></small>
                                <small class="form-text text-muted"><?= sprintf(l('global.accessibility.whitelisted_file_extensions'), \Altum\Uploads::get_whitelisted_file_extensions_accept('qr_code_files')) . ' ' . sprintf(l('global.accessibility.file_size_limit'), $qr_code_file_size_limit) ?></small>
                            </div>
                        </div>
                        ...
                        <div>
                            <div class="form-group" data-type="facetime_audio">
                                ...
                                <input type="text" id="facetime_audio" name="facetime_audio" class="form-control <?= \Altum\Alerts::has_field_errors('facetime_audio') ? 'is-invalid' : null ?>" value="<?= isset($data->qr_code->settings->facetime_audio) ? $data->qr_code->settings->facetime_audio : null ?>" maxlength="<?= $data->available_qr_codes['facetime_audio']['max_length'] ?>" required="required" data-reload-qr-code />
                                <?= \Altum\Alerts::output_field_error('facetime_audio') ?>
                            </div>
                        </div>
                        ...
                        <div>
                            ...
                            <button class="btn btn-block btn-gray-200 font-size-little-small font-weight-450 my-4" type="button" data-toggle="collapse" data-target="#vcard_phone_numbers_container" aria-expanded="false" aria-controls="vcard_phone_numbers_container" data-type="vcard">
                            ...
                            <button class="btn btn-block btn-gray-200 font-size-little-small font-weight-450 my-4" type="button" data-toggle="collapse" data-target="#vcard_socials_container" aria-expanded="false" aria-controls="vcard_socials_container" data-type="vcard">
    ...
    <?php endforeach ?>
                        ...
                        <?php $qr_code_themes = require APP_PATH . 'includes/qr_codes_themes.php' ?>

                        <?php /* QR code themes */ ?>
                        <button class="btn btn-block btn-gray-200 font-size-little-small font-weight-450 my-4" type="button" data-toggle="collapse" data-target="#themes_container" aria-expanded="false" aria-controls="themes_container">
                            <i class="fas fa-fw fa-palette fa-sm mr-1"></i> <?= l('qr_codes.input.themes') ?>
                        </button>

                        <div class="collapse" id="themes_container" data-parent="#form">
                            <div class="row mx-n2 mb-4">
                                <?php foreach($qr_code_themes as $key => $theme): ?>
                                    <label class="col-12 col-sm-6 p-2 custom-radio-box m-0">
                                        <input type="radio" name="qr_code_theme" value="<?= $key ?>" class="custom-control-input" data-qr-code-theme />

                                        <div class="card h-100" data-toggle="tooltip" title="<?= l('qr_codes.theme_apply') ?>" data-tooltip-hide-on-click>
                                            <div class="card-body d-flex flex-column">
                                                <div class="d-flex align-items-center mb-3">
                                                    <h3 class="h6 mb-0"><?= $theme['name'] ?></h3>
                                                </div>

                                                <div class="qr-code-theme-preview rounded flex-grow-1" style="background-color: <?= $theme['preview_background_color'] ?>;">
                                                    <img src="<?= ASSETS_FULL_URL . 'images/qr-code-themes/' . $theme['preview_image'] . '?v=' . PRODUCT_CODE ?>" class="qr-code-theme-preview-image" alt="<?= $theme['name'] ?>" loading="lazy" />
                                                </div>
                                            </div>
                                        </div>
                                    </label>
                                ...
                                <div class="row btn-group-toggle m-n2" data-toggle="buttons">
                                    <?php foreach($data->frames_fonts as $font_key => $font): ?>
                                        <div class="p-2 col-12 col-lg-4 p-2 h-100">
                                            <label class="btn btn-light btn-block font-size-small mb-0 text-truncate <?= ($data->qr_code->settings->frame_text_font ?? array_key_first($data->frames_fonts)) == $font_key ? 'active"' : null?>" style="font-family: <?= $font['font-family'] ?> !important;">
