...
<div class="container text-center d-print-none">
    ...
    <div class="qr-code-types position-relative">
        <div class="qr-code-types-wrapper-left"></div>
        <div class="qr-code-types-wrapper-right"></div>

        <div class="d-flex align-items-center qr-code-types-wrapper">
            <?php foreach($data->available_qr_codes as $key => $value): ?>
                <div class="mr-3 my-1" data-toggle="tooltip" <?= $this->user->plan_settings->enabled_qr_codes->{$key} ? 'title="' . l('qr_codes.type.' . $key . '_description') . '"' : 'title="' . l('global.info_message.plan_feature_no_access') . '"' ?>>
                    <a
                            href="<?= url('qr/' . $key) ?>"
                            class="btn <?= $data->type == $key ? 'active' : null ?> <?= $this->user->plan_settings->enabled_qr_codes->{$key} ? null : 'disabled' ?> qr-code-type-button"
                    >
    ...
    <script type="application/ld+json">
        {
            ...
            "itemListElement": [
                ...
                },
                {
                    "@type": "ListItem",
                    "position": 2,
                    "name": <?= json_encode(l('qr.title')) ?>,
                    "item": <?= json_encode(url('qr')) ?>
                },
                {
                    "@type": "ListItem",
                    "position": 3,
...
<?php else: ?>
    ...
    <?php ob_start() ?>
        <script>
            ...
            /* QR code type slider */
            let qr_index_code_types_wrapper = document.querySelector('.qr-code-types-wrapper');
            let qr_index_code_types_wrapper_left = document.querySelector('.qr-code-types-wrapper-left');
            let qr_index_code_types_wrapper_right = document.querySelector('.qr-code-types-wrapper-right');

            let qr_index_code_types_update_fades = () => {
                if(!qr_index_code_types_wrapper || !qr_index_code_types_wrapper_left || !qr_index_code_types_wrapper_right) {
                    return;
                }

                qr_index_code_types_wrapper_left.style.opacity = qr_index_code_types_wrapper.scrollLeft ? 1 : 0;
                qr_index_code_types_wrapper_right.style.opacity = (qr_index_code_types_wrapper.scrollLeft + qr_index_code_types_wrapper.clientWidth + 1 >= qr_index_code_types_wrapper.scrollWidth) ? 0 : 1;
            };

            if(qr_index_code_types_wrapper) {
                qr_index_code_types_wrapper.addEventListener('scroll', qr_index_code_types_update_fades);
                window.addEventListener('resize', qr_index_code_types_update_fades);

                qr_index_code_types_update_fades();
            }
        </script>
    <?php \Altum\Event::add_content(ob_get_clean(), 'javascript') ?>
