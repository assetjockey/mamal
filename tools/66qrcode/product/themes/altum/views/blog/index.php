...
<div class="container <?= settings()->content->blog_columns == 1 ? 'col-lg-8' : null ?>">
    ...
    <div class="row mt-4">
        <div class="<?= settings()->content->blog_columns == 1 ? 'col-12 mb-5' : 'col-12 col-lg-8 mb-lg-0' ?>">
            <?php if($data->blog_post_featured || !empty($data->blog_posts)): ?>
                <?php /* Featured post */ ?>
                <?php if($data->blog_post_featured): ?>
                    <div class="card mb-4 border-primary" style="border-width: 2px;">
                        <div class="card-body">
                            <small>
                                <span class="badge badge-primary mb-3"><i class="fas fa-fw fa-sm fa-star mr-1"></i> <?= l('blog.featured') ?></span>
                            </small>

                            <a href="<?= SITE_URL . ($data->blog_post_featured->language ? \Altum\Language::$active_languages[$data->blog_post_featured->language] . '/' : null) . 'blog/' . $data->blog_post_featured->url ?>" class="text-decoration-none">
                                <h2 class="h4 mb-2"><?= $data->blog_post_featured->title ?></h2>
                            </a>

                            <p class="small text-muted mb-4">
                                <span data-toggle="tooltip" title="<?= sprintf(l('global.last_datetime_tooltip'), \Altum\Date::get($data->blog_post_featured->last_datetime, 2)) ?>">
                                    <i class="fas fa-fw fa-xs fa-calendar-alt mr-1"></i> <?= \Altum\Date::get($data->blog_post_featured->datetime, 2) ?>
                                </span>

                                <?php if($data->blog_post_featured->blog_posts_category_id && isset($data->blog_posts_categories[$data->blog_post_featured->blog_posts_category_id])): ?>
                                    • <a href="<?= SITE_URL . ($data->blog_posts_categories[$data->blog_post_featured->blog_posts_category_id]->language ? \Altum\Language::$active_languages[$data->blog_posts_categories[$data->blog_post_featured->blog_posts_category_id]->language] . '/' : null) . 'blog/category/' . $data->blog_posts_categories[$data->blog_post_featured->blog_posts_category_id]->url ?>" class="text-muted"><?= $data->blog_posts_categories[$data->blog_post_featured->blog_posts_category_id]->title ?></a>
                                <?php endif ?>

                                <?php if(settings()->content->blog_views_is_enabled): ?>
                                    <span> • <?= sprintf(l('blog.total_views'), nr($data->blog_post_featured->total_views)) ?></span>
                                <?php endif ?>
                            </p>

                            <?php if($data->blog_post_featured->image): ?>
                                <a href="<?= SITE_URL . ($data->blog_post_featured->language ? \Altum\Language::$active_languages[$data->blog_post_featured->language] . '/' : null) . 'blog/' . $data->blog_post_featured->url ?>">
                                    <img src="<?= \Altum\Uploads::get_full_url('blog') . $data->blog_post_featured->image ?>" class="blog-post-image img-fluid w-100 rounded" alt="<?= $data->blog_post_featured->image_description ?>" />
                                </a>
                            <?php endif ?>

                            <?php if($data->blog_post_featured->description): ?>
                                <p class="m-0 mt-3"><?= $data->blog_post_featured->description ?></p>
                            <?php endif ?>
                        </div>
                    </div>
                <?php endif ?>
