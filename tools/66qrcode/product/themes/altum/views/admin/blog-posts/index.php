...
<?php if(!empty($data->blog_posts) || $data->filters->has_applied_filters): ?>
    ...
    <div class="d-flex flex-column flex-md-row justify-content-between align-items-md-end mb-4">
        ...
        <div class="d-flex position-relative">
            ...
            <div class="ml-3">
                <button id="bulk_enable" type="button" class="btn btn-gray-300 <?= !empty($data->blog_posts) ? null : 'disabled' ?>" data-toggle="tooltip" title="<?= l('global.bulk_actions') ?>"><i class="fas fa-fw fa-sm fa-list"></i></button>
        ...
        <div class="table-responsive table-custom-container">
            <table class="table table-custom">
                ...
                <tr>
                    ...
                    <th><?= l('admin_blog.is_published') ?></th>
                    <th></th>
                    <th></th>
                    <th></th>
                </tr>
                ...
                <?php foreach($data->blog_posts as $row): ?>
                    <tr>
                        ...
                        <td class="text-nowrap">
                            <div class="d-flex">
                                <a href="<?= url('admin/blog-post-update/' . $row->blog_post_id) ?>" class="mr-3">
                                    <?php if($row->image): ?>
                                        <img src="<?= \Altum\Uploads::get_full_url('blog') . $row->image ?>" class="table-avatar" alt="" loading="lazy" />
                                    <?php else: ?>
                                ...
                                <div class="d-flex flex-column text-truncate">
                                    <div>
                                        ...
                                        <?php if($row->is_featured): ?>
                                            <span class="badge badge-primary mr-1"><i class="fas fa-fw fa-sm fa-star mr-1"></i> <?= l('admin_blog.featured') ?></span>
                                        <?php endif ?>
                                    ...
                                    <div class="small text-muted">
                                        ...
                                        <a href="<?= $row->is_published ? SITE_URL . ($row->language ? \Altum\Language::$active_languages[$row->language] . '/' : null) . 'blog/' . $row->url : url('admin/blog-post-preview/' . $row->blog_post_id) ?>" target="_blank" rel="noreferrer">
                        ...
                        <td class="text-nowrap">
                            <?php // Published status ?>
                            <?php if($row->is_published): ?>
                                <span class="badge badge-success"><i class="fas fa-fw fa-sm fa-check mr-1"></i> <?= l('global.yes') ?></span>
                            <?php else: ?>
                                <span class="badge badge-warning"><i class="fas fa-fw fa-sm fa-eye-slash mr-1"></i> <?= l('global.no') ?></span>
                            <?php endif ?>
                        </td>
                        ...
                        <td class="text-nowrap">
                            <div class="d-flex align-items-center">
                                <?php // Category title ?>
                                <?php $blog_posts_category_title = $row->blog_posts_category_id && isset($data->blog_posts_categories[$row->blog_posts_category_id]) ? $data->blog_posts_categories[$row->blog_posts_category_id]->title : l('global.none') ?>

                                <span class="mr-2" data-toggle="tooltip" data-html="true" title="<?= l('admin_blog.blog_posts_category_id') . '<br />' . $blog_posts_category_title ?>">
                                    <i class="fas fa-fw fa-book text-muted"></i>
                                </span>

                                <span class="mr-2" data-toggle="tooltip" data-html="true" title="<?= l('global.language') . '<br />' . ($row->language ? $row->language : l('global.all')) ?>">
                        ...
                        <td>
                            <div class="d-flex justify-content-end">
                                <?= include_view(THEME_PATH . 'views/admin/blog-posts/admin_blog_post_dropdown_button.php', ['id' => $row->blog_post_id, 'resource_name' => $row->title, 'url' => $row->url, 'language' => $row->language, 'is_published' => $row->is_published]) ?>
                            </div>
                        </td>
                    </tr>
                <?php endforeach ?>
