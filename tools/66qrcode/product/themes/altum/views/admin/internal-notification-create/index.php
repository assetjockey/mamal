...
<div class="card <?= \Altum\Alerts::has_field_errors() ? 'border-danger' : null ?>">
    <div class="card-body">
        ...
        <form id="form" action="" method="post" role="form">
            ...
            <div class="form-group custom-control custom-checkbox" data-segment="filter">
                <input id="<?= 'filters_is_broadcast_subscribed' ?>" name="filters_is_broadcast_subscribed" type="checkbox" class="custom-control-input" <?= $data->values['filters_is_broadcast_subscribed'] ? 'checked="checked"' : null ?>>
                <label class="custom-control-label" for="<?= 'filters_is_broadcast_subscribed' ?>"><?= l('admin_broadcasts.segment.filter.is_broadcast_subscribed') ?></label>
            </div>
...
<script>
    ...
    document.querySelectorAll('#filters_is_broadcast_subscribed,[name^="filters_"]').forEach(element => element.addEventListener('change', async event => {
        await get_segment_count();
    }));
    ...
    let get_segment_count = async () => {
        ...
        if(segment == 'filter') {
            ...
            ['title', 'description', 'url', 'icon'].forEach(field => query.delete(field));
        }
