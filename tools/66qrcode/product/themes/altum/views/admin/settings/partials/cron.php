...
<?php foreach(['reset', 'broadcasts', 'email_reports'] as $cron): ?>
    <div class="form-group">
        <label for="cron_<?= $cron ?>"><i class="fas fa-fw fa-sm <?= $cron == 'broadcasts' ? 'fa-envelopes-bulk' : 'fa-clock' ?> text-muted mr-1"></i> <?= l('admin_settings.cron.' . $cron) ?></label>
        ...
        <?php if(isset(settings()->cron->{$cron . '_datetime'})): ?>
            <small class="form-text text-muted"><?= sprintf(l('admin_settings.cron.last_processing_time'), isset(settings()->cron->{$cron . '_datetime_processing'}) ? nr(settings()->cron->{$cron . '_datetime_processing'}, 2) . ' ' . l('global.date.seconds') : l('global.na')) ?></small>
        <?php endif ?>
    </div>
<?php endforeach ?>
