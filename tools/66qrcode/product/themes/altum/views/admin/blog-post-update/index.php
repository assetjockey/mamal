...
<div class="d-flex justify-content-between align-items-end mb-4">
    ...
    <?= include_view(THEME_PATH . 'views/admin/blog-posts/admin_blog_post_dropdown_button.php', ['id' => $data->blog_post->blog_post_id, 'resource_name' => $data->blog_post->title, 'url' => $data->blog_post->url, 'language' => $data->blog_post->language, 'is_published' => $data->blog_post->is_published]) ?>
</div>
...
<div class="card <?= \Altum\Alerts::has_field_errors() ? 'border-danger' : null ?>">
    <div class="card-body">
        ...
        <form id="blog_post_update_form" action="" method="post" role="form" enctype="multipart/form-data">
            ...
            <?php /* Blog author */ ?>
            <?php if($data->blog_authors_is_enabled && count($data->blog_authors)): ?>
                <div class="form-group">
                    <label for="author_id"><i class="fas fa-fw fa-sm fa-user-pen text-muted mr-1"></i> <?= l('admin_blog.author_id') ?></label>
                    <select id="author_id" name="author_id" class="custom-select">
                        <?php foreach($data->blog_authors as $blog_author): ?>
                            <option value="<?= $blog_author->id ?>" <?= isset($data->blog_post->author_id) && $data->blog_post->author_id == $blog_author->id ? 'selected="selected"' : null ?>><?= $blog_author->name ?></option>
                        <?php endforeach ?>
                        <option value="" <?= !isset($data->blog_post->author_id) || !$data->blog_post->author_id ? 'selected="selected"' : null ?>><?= l('global.none') ?></option>
                    </select>
                </div>
            <?php endif ?>
            ...
            <?php /* Featured post */ ?>
            <div class="form-group custom-control custom-switch">
                <input id="is_featured" name="is_featured" type="checkbox" class="custom-control-input" <?= $data->blog_post->is_featured ? 'checked="checked"' : null ?>>
                <label class="custom-control-label" for="is_featured"><?= l('admin_blog.is_featured') ?></label>
            </div>
