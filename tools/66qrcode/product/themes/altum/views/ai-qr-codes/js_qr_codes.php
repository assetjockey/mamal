...
<script>
    ...
    let url_dynamic_handler = () => {
        ...
        if(url_dynamic) {
            ...
            if(url_dynamic_existing_link) {
                content_element.removeAttribute('required');
                ...
                if(link_id_element.options.length) {
                    if(!content_element.dataset.dynamicUrlValue) {
                        content_element.dataset.dynamicUrlValue = content_element.value;
                    }

                    content_element.value = link_id_element.options[link_id_element.selectedIndex].getAttribute('data-url');
                }
            } else {
                ...
                content_element.setAttribute('required', 'required');
                document.querySelector('[data-url]').classList.remove('d-none');

                if(content_element.dataset.dynamicUrlValue) {
                    content_element.value = content_element.dataset.dynamicUrlValue;
                    delete content_element.dataset.dynamicUrlValue;
                }
            }
        } else {
            link_id_element.removeAttribute('required');
    ...
    let generate_ai_qr_code = () => {
        ...
        if(document.querySelector('#generate')) {
            pause_submit_button(document.querySelector('#generate'));
        ...
        fetch(`${url}ai-qr-code-generator`, {
            ...
            .then(data => {
                if(data.status == 'error') {
                    ...
                    /* Enable the save & generate button */
                    if(document.querySelector('#generate')) {
                        enable_submit_button(document.querySelector('#generate'))
