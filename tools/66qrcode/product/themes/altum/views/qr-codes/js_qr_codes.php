...
<script>
    ...
    let qr_code_themes = <?= json_encode(require APP_PATH . 'includes/qr_codes_themes.php') ?>;

    /* QR code type slider */
    let qr_code_types_wrapper = document.querySelector('.qr-code-types-wrapper');
    let qr_code_types_wrapper_left = document.querySelector('.qr-code-types-wrapper-left');
    let qr_code_types_wrapper_right = document.querySelector('.qr-code-types-wrapper-right');

    let qr_code_types_update_fades = () => {
        if(!qr_code_types_wrapper || !qr_code_types_wrapper_left || !qr_code_types_wrapper_right) {
            return;
        }

        qr_code_types_wrapper_left.style.opacity = qr_code_types_wrapper.scrollLeft ? 1 : 0;
        qr_code_types_wrapper_right.style.opacity = (qr_code_types_wrapper.scrollLeft + qr_code_types_wrapper.clientWidth + 1 >= qr_code_types_wrapper.scrollWidth) ? 0 : 1;
    };

    let qr_code_types_scroll_to_button = button => {
        if(!qr_code_types_wrapper) {
            return;
        }

        button = button || qr_code_types_wrapper.querySelector('.qr-code-type-button.active');
    ...
    if(qr_code_types_wrapper) {
        qr_code_types_wrapper.addEventListener('scroll', qr_code_types_update_fades);
        window.addEventListener('resize', qr_code_types_update_fades);

        qr_code_types_scroll_to_button();
        qr_code_types_update_fades();
    }
    ...
    document.querySelector('input[name="type"]') && document.querySelectorAll('input[name="type"]').forEach(element => {
        element.addEventListener('change', event => {
            ...
            type_handler('input[name="type"]', 'data-type');
            qr_code_types_scroll_to_button(event.currentTarget.closest('.qr-code-type-button'));
        });
    })
    ...
    /* QR code themes */
    let qr_code_theme_set_value = (key, value) => {
        ...
        elements.forEach(element => {
            if(element.type == 'radio') {
                ...
                if(element.closest('label')) {
                    element.checked ? element.closest('label').classList.add('active') : element.closest('label').classList.remove('active');
                }
            }
            ...
            if(element.color_picker && value) {
                element.value = value;
                element.color_picker.setColor(value, true);
            }
    ...
    document.querySelectorAll('[data-qr-code-theme]').forEach(element => element.addEventListener('click', event => {
        let theme = qr_code_themes[event.currentTarget.value].qr_code;

        Object.entries(theme).forEach(([key, value]) => {
            qr_code_theme_set_value(key, value);
        });
        ...
        let reload = document.querySelector('input[name="reload"]');

        if(reload) {
            reload.value = Date.now();
            reload.dispatchEvent(new Event('change'));
        }
    }));
    ...
    let url_dynamic_handler = () => {
        ...
        let url_dynamic_existing_link = document.querySelector('#url_dynamic_existing_link') && document.querySelector('#url_dynamic_existing_link').checked;

        if(url_dynamic) {
            document.querySelector('[data-url-dynamic-existing-link]').classList.remove('d-none');
            document.querySelector('#url_dynamic_existing_link').removeAttribute('disabled');

            if(url_dynamic_existing_link) {
                url_element.removeAttribute('required');
                document.querySelector('[data-url]').classList.add('d-none');
                link_id_element.setAttribute('required', 'required');
                link_id_element.removeAttribute('disabled');
                document.querySelector('[data-link-id]').classList.remove('d-none');

                if(link_id_element.options.length) {
                    if(!url_element.dataset.dynamicUrlValue) {
                        url_element.dataset.dynamicUrlValue = url_element.value;
                    }

                    url_element.value = link_id_element.options[link_id_element.selectedIndex].getAttribute('data-url');
                }
            } else {
                link_id_element.removeAttribute('required');
                link_id_element.setAttribute('disabled', 'disabled');
                document.querySelector('[data-link-id]').classList.add('d-none');
                url_element.setAttribute('required', 'required');
                document.querySelector('[data-url]').classList.remove('d-none');

                if(url_element.dataset.dynamicUrlValue) {
                    url_element.value = url_element.dataset.dynamicUrlValue;
                    delete url_element.dataset.dynamicUrlValue;
                }
            }
        } else {
            link_id_element.removeAttribute('required');
            link_id_element.setAttribute('disabled', 'disabled');
            document.querySelector('[data-link-id]').classList.add('d-none');
            document.querySelector('#url_dynamic_existing_link').setAttribute('disabled', 'disabled');
            document.querySelector('[data-url-dynamic-existing-link]').classList.add('d-none');
            url_element.setAttribute('required', 'required');
            document.querySelector('[data-url]').classList.remove('d-none');
    ...
    if(document.querySelector('#url_dynamic')) {
        ...
        document.querySelector('#url_dynamic_existing_link') && document.querySelector('#url_dynamic_existing_link').addEventListener('change', url_dynamic_handler);
    ...
    let link_id_handler = () => {
        ...
        if(link_id_element && document.querySelector('#url_dynamic') && document.querySelector('#url_dynamic').checked && document.querySelector('#url_dynamic_existing_link') && document.querySelector('#url_dynamic_existing_link').checked) {
    ...
    let qr_code_request_controller = null;
    let qr_code_request_id = 0;
    ...
    let reload_qr_code_event_listener = event => {
        ...
        let targeted_element_value = ['checkbox', 'radio'].includes(targeted_element.type) ? targeted_element.checked : targeted_element.value;

        if(sessionStorage.getItem(targeted_element.id) == targeted_element_value) {
            return;
        } else {
            sessionStorage.setItem(targeted_element.id, targeted_element_value);
        }

        /* Stop the previous request */
        let current_request_id = ++qr_code_request_id;
        if(qr_code_request_controller) {
            qr_code_request_controller.abort();
            qr_code_request_controller = null;
        }
        ...
        qr_code_delay_timer = setTimeout(() => {
            ...
            qr_code_request_controller = new AbortController();

            fetch(`${url}qr-code-generator`, {
                method: 'POST',
                body: form_data,
                signal: qr_code_request_controller.signal,
            })
                .then(response => response.ok ? response.json() : Promise.reject(response))
                .then(data => {
                    /* Ignore old responses */
                    if(current_request_id !== qr_code_request_id) {
                        return;
                    }
                ...
                .catch(error => {
                    ...
                    if(current_request_id !== qr_code_request_id || error.name == 'AbortError') {
                        return;
                    }
