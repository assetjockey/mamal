...
<div class="container mt-5">
    <div id="barcode_reader_notifications"></div>
...
<script>
    ...
    const barcode_reader_notifications = document.querySelector('#barcode_reader_notifications');

    const display_barcode_reader_error = message => {
        const alert = document.createElement('div');
        const icon = document.createElement('i');
        const text = document.createTextNode(message);
        const close_button = document.createElement('button');

        barcode_reader_notifications.innerHTML = '';

        alert.className = 'alert alert-danger alert-dismissible fade show';
        alert.setAttribute('role', 'alert');

        icon.className = 'fas fa-fw fa-times-circle text-danger mr-2';

        close_button.type = 'button';
        close_button.className = 'close';
        close_button.setAttribute('data-dismiss', 'alert');
        close_button.setAttribute('aria-label', <?= json_encode(l('global.close')) ?>);
        close_button.innerHTML = '<span aria-hidden="true">&times;</span>';

        alert.appendChild(icon);
        alert.appendChild(text);
        alert.appendChild(close_button);
        barcode_reader_notifications.appendChild(alert);
    };
    ...
    if(navigator.mediaDevices && typeof navigator.mediaDevices.getUserMedia === 'function') {
        ...
        document.querySelector('#start_scanner').addEventListener('click', async event => {
            ...
            try {
                ...
                display_barcode_reader_error(<?= json_encode(l('qr_reader.camera_error_message')) ?>);
            }
    ...
    <script type="application/ld+json">
        {
            ...
            "itemListElement": [
                ...
                },
                {
                    "@type": "ListItem",
                    "position": 2,
                    "name": <?= json_encode(l('barcode_reader.title')) ?>,
                    "item": <?= json_encode(url('barcode-reader')) ?>
                }
            ]
        }
    </script>
