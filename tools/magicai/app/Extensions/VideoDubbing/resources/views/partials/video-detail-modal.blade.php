<template x-teleport="body">
    <div
        x-on:dubbing-videos-updated.window="videos = $event.detail"
        x-data="{
            open: false,
            currentIndex: -1,
            videos: {{ $videosJson ?? '[]' }},
        ...
        <div class="lqd-modal-img-content-wrap relative z-10 w-[min(1230px,calc(100%-4rem))]">
            ...
            >
                ...
                <div class="w-full p-6 md:w-1/2 md:px-10 lg:w-4/12 lg:py-7">
                    <h4 class="mb-2 text-sm font-semibold text-heading-foreground">{{ __('Title') }}</h4>
                    <p class="text-2xs leading-[1.4em] text-foreground/70" x-text="currentVideo?.title || '{{ __('Untitled') }}'"></p>

                    {{-- Action buttons --}}
                    <div class="-ms-2 mt-4 flex items-center gap-2">
                        <a
                            class="inline-flex size-9 items-center justify-center rounded-full border border-foreground/10 text-foreground/60 transition-colors hover:bg-foreground/5 hover:text-foreground"
                            :href="currentVideo?.video_url"
                            :download="'dubbing-' + currentVideo?.id + '.mp4'"
                            title="{{ __('Download') }}"
                        >
                            <x-tabler-download class="size-4" />
                        </a>
                        <x-button
                            class="inline-flex size-9 items-center justify-center rounded-full border border-foreground/10 text-foreground/60 transition-colors hover:bg-red-50 hover:text-red-500"
                            variant="ghost"
                            hover-variant="none"
                            size="none"
                            type="button"
                            ...
                            @click="
                                if (confirm('{{ __('Are you sure?') }}')) {
                                    window.location.href = '{{ LaravelLocalization::localizeUrl(route('dashboard.user.video-dubbing.delete', '')) }}/' + currentVideo?.id;
                                }
                            "
                        >
                    ...
                    <div class="mt-10 space-y-3.5">
                        <div class="flex w-full items-center justify-between gap-1 py-1.5 text-2xs font-medium">
                            ...
                            <p class="mb-0 opacity-50" x-text="formatDate(currentVideo?.created_at) || '{{ __('None') }}'"></p>
                        </div>
                        <div class="flex w-full items-center justify-between gap-1 py-1.5 text-2xs font-medium">
                            <p class="mb-0">{{ __('Provider') }}</p>
                            <p class="mb-0 opacity-50" x-text="currentVideo?.provider || '{{ __('None') }}'"></p>
                        </div>
                        <div class="flex w-full items-center justify-between gap-1 py-1.5 text-2xs font-medium">
                            <p class="mb-0">{{ __('Target Language') }}</p>
                            <p class="mb-0 opacity-50" x-text="currentVideo?.target_language_name || '{{ __('None') }}'"></p>
                        </div>
                        <div
                        ...
                        >
                            ...
                            <p class="mb-0 opacity-50" x-text="currentVideo?.formatted_duration || '{{ __('None') }}'"></p>
                        </div>
                    </div>
                </div>
            </div>
