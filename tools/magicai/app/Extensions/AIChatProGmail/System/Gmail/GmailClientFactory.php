...
namespace App\Extensions\AIChatProGmail\System\Gmail;
...
class GmailClientFactory
{
    ...
    public function make(AIChatProConnector $connector): Gmail
    {
        ...
        if ($expiresAt && now()->gte($expiresAt) && $refreshToken) {
            ...
            if ($tokenResponse->successful()) {
                ...
                Log::warning('[connectors] Gmail token refresh failed; marking connector inactive', [
                ...
                throw new RuntimeException('Gmail connector authorization expired. Please reconnect.');
            }
        }

        $client = new Google_Client;
        $client->setClientId(setting('ai_chat_pro_gmail_client_id') ?: config('ai-chat-pro-gmail.client_id'));
        $client->setClientSecret(setting('ai_chat_pro_gmail_client_secret') ?: config('ai-chat-pro-gmail.client_secret'));
