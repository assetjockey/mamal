...
@push('script')
    <script>
        const APS_EDIT_ROUTES = {
            generate: '{{ route('dashboard.user.ai-photoshoot.edit_image.generate') }}',
            checkStatus: '{{ url('dashboard/user/ai-photoshoot/edit_image/status') }}',
        ...
        document.addEventListener('alpine:init', () => {
            Alpine.data('editImageApp', () => ({
                ...
                async generateEditImage() {
                    if (!this.imageFile) {
                        window.toastr?.warning("{{ __('Please upload an image') }}");
                        return;
                    }

                    if (!this.editPrompt.trim()) {
                        window.toastr?.warning("{{ __('Please describe what you would like to change') }}");
                        return;
                    }
                    ...
                    try {
                        const response = await fetch(APS_EDIT_ROUTES.generate, {
                            method: 'POST',
                            body: formData,
                            headers: {
                        ...
                        window.toastr?.error(error.message || "{{ __('Failed to edit image') }}");
                ...
                checkGenerationStatus(id) {
                    ...
                    const checkInterval = setInterval(async () => {
                        ...
                        try {
                            const response = await fetch(`${APS_EDIT_ROUTES.checkStatus}/${id}`, {
                                headers: {
                                    'X-Requested-With': 'XMLHttpRequest',
                                    'Accept': 'application/json'
                                }
                            ...
                            if (data.status?.toLowerCase() === 'completed') {
                                ...
                                window.toastr?.success("{{ __('Image edited successfully!') }}");

                                this.results = data.results || (data.edit_image ? [data.edit_image] : []);
                ...
                editResult(result) {
                    ...
                    window.location.href = APS_EDIT_ROUTES.editImage;
                },
                ...
                videoResult(result) {
                    ...
                    window.location.href = APS_EDIT_ROUTES.createVideo + '/' + result.id;
                },
            }));
        });
    </script>
