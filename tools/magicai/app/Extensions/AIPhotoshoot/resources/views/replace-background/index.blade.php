...
@section('title', __('Replace Background'))
...
@endsection
@section('titlebar_subtitle', __('Upload your product images and change the background quickly.'))
    ...
    >
        <div class="flex flex-col gap-4 lg:flex-row lg:gap-9">
                    ...
                    <div>
                        <label class="mb-4 flex items-center gap-1 text-2xs font-medium">
                            ...
                            <x-info-tooltip
                                ...
                                text="{{ __('Upload the product image whose background you want to replace.') }}"
                            />
                        </label>
                        ...
                        <div
                            class="cursor-pointer rounded-[10px] border border-dashed border-foreground/10 p-6 text-center transition hover:border-primary hover:bg-primary/5 sm:p-8"
                                ...
                                @drop.prevent="handleProductDrop($event)"
                        ...
                        <x-button
                            ...
                            type="submit"
                            variant="primary"
                            ::disabled="generating || !productImage || !hasBackgroundSelected()"
                        >
            ...
            <div class="flex w-full flex-col lg:basis-2/3">
                <div class="mb-5 flex flex-wrap items-center justify-between gap-3 border-b transition-border">
                    <div class="flex flex-wrap gap-4">
                        <button
                            class="-mb-px border-b py-2.5 text-[12px] font-semibold text-heading-foreground transition"
                            type="button"
                        >
                            {{ __('Pick a Background Preset') }}
                        </button>
                        <button
                ...
                {{-- Combined grid: user backgrounds first, then presets --}}
                <div
                    class="grid grid-cols-2 gap-2 sm:grid-cols-3"
                    x-show="!results.length && !generating"
                >
                    <template
                        x-for="bg in userBackgrounds"
                        :key="`user-${bg.id}`"
                    >
                        <div
                            class="group relative rounded border p-1.5 transition-all hover:-translate-y-1 hover:shadow-lg hover:shadow-black/5 [&.selected]:ring-4 [&.selected]:ring-primary"
                            @click="bg.status?.toLowerCase() !== 'processing' && selectUserBackground(bg)"
                            :class="{
                                'cursor-pointer': bg.status?.toLowerCase() !== 'processing',
                                'selected': selectedBackgroundType === 'uploaded' && selectedBackgroundId === bg.id,
                                'opacity-60': bg.status?.toLowerCase() === 'processing',
                            }"
                        >
                            <div
                                ...
                                x-show="selectedBackgroundType === 'uploaded' && selectedBackgroundId === bg.id"
                                x-cloak
                            >
                            ...
                            <img
                                class="mb-1.5 aspect-[1/1.2767] w-full rounded object-cover"
                                :src="bg.thumbnail || bg.image_url"
                                :alt="bg.name"
                                loading="lazy"
                            >
                            ...
                            <div class="flex items-end justify-between gap-2">
                                <div class="min-w-0 flex-1">
                                    <p
                                        ...
                                        x-text="bg.category || '{{ __('My Background') }}'"
                                    ></p>
                                    <p
                                            ...
                                            <ul class="py-1 text-xs font-medium">
                                                <li>
                                                    <a
                                                        ...
                                                        @click.prevent="deleteUserBackground(bg.id); toggle('collapse')"
                                                    >
                    ...
                    <template
                        x-for="bg in backgrounds"
                        :key="bg.slug"
                    >
                        <div
                            class="group relative cursor-pointer rounded border p-1.5 transition-all hover:-translate-y-1 hover:shadow-lg hover:shadow-black/5 [&.selected]:ring-4 [&.selected]:ring-primary"
                            :class="{ 'selected': selectedBackgroundType === 'preset' && selectedBackgroundSlug === bg.slug }"
                            @click="selectPresetBackground(bg.slug)"
                        >
                            <div
                                class="absolute end-2 top-2 z-10 inline-grid size-7 place-items-center rounded-full bg-secondary text-secondary-foreground shadow-lg shadow-black/5"
                                x-show="selectedBackgroundType === 'preset' && selectedBackgroundSlug === bg.slug"
                                x-cloak
                            >
                                <x-tabler-check class="size-4" />
                            </div>
                ...
                <div
                    ...
                    x-show="generating"
                >
                    <template x-for="n in numImages" :key="`bg-skeleton-${n}`">
                        <div
        ...
        {{-- Universal Upload/Create Background Modal --}}
        <div
                ...
                <div class="h-full max-h-[90vh] w-full overflow-y-auto rounded-xl bg-background p-5 shadow-xl shadow-black/5 md:p-8">
                    ...
                    >
                        ...
                        <div
                            ...
                            @drop.prevent="handleModalFileDrop($event)"
                            ...
                            <template x-if="!uploadPreview">
                                <div>
                                    <img
                                        ...
                                        src="{{ asset('vendor/ai-photoshoot/images/background-modal.png') }}"
                                        alt="{{ __('Background') }}"
                                    >
                                    ...
                                    <x-button
                                        ...
                                        variant="outline"
                                        size="xl"
                                        @click.stop="$refs.modalFileInput.click()"
                                    >
                                        {{ __('Browse Files') }}
                                    </x-button>

                                    <ul class="list-inside list-disc text-[12px] opacity-50">
                                        <li>{{ __('PNG or JPG (Max: :size MB)', ['size' => $apsMaxUploadMb]) }}</li>
                                    </ul>
                                </div>
                            </template>
                        ...
                        >
                            ...
                            <x-forms.input
                                ...
                                placeholder="{{ __('e.g. Soft Pink Curtain') }}"
                            />
                        </div>
                        ...
                        >
                            <span x-text="uploading ? '{{ __('Uploading...') }}' : '{{ __('Upload Background Image') }}'"></span>
                    ...
                    >
                        ...
                        <x-forms.input
                            ...
                            size="lg"
                            rows="8"
                            label="{{ __('Background Description') }}"
                            type="textarea"
                            x-model="createForm.description"
                            placeholder="{{ __('Describe your background in detail...') }}"
                        />
                        ...
                        <x-button
                            ...
                            ::disabled="creating"
                        >
                            <span x-text="creating ? '{{ __('Creating...') }}' : '{{ __('Create Background') }}'"></span>
                            <span class="inline-grid size-7 place-items-center rounded-full bg-background text-foreground">
                                <x-tabler-chevron-right class="size-4" />
                            </span>
                        </x-button>
...
@include('ai-photoshoot::replace-background.scripts')
