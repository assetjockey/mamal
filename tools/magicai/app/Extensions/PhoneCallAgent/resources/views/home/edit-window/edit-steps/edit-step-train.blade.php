...
>
    <h2 class="mb-3.5">@lang('Agent Training')</h2>
    <p class="mb-3 text-xs/5 opacity-60 lg:max-w-[360px]">
        @lang('This step is optional but highly recommended to personalize your agent\'s knowledge base.')
    </p>
    <x-button class="w-full text-2xs" variant="outline" x-show="editingStep !== 1" size="lg" @click.prevent="setEditingStep('>')" type="button">
        @lang('Skip')
        <x-tabler-chevron-right class="size-4" />
    </x-button>

    @php
        $tabs = ['url' => ['label' => __('URL')], 'pdf' => ['label' => __('PDF')], 'text' => ['label' => __('Text')]];
    @endphp
    <div class="lqd-ext-chatbot-training mt-16 flex flex-col justify-center gap-9">
        <ul class="flex w-full flex-wrap justify-between gap-3 rounded-3xl bg-foreground/5 p-1 text-xs font-medium sm:flex-nowrap sm:rounded-full">
            @foreach ($tabs as $key => $tab)
                <li class="grow">
                    <button
                        @class(['px-6 py-2.5 grow leading-tight rounded-full transition-all hover:bg-background/80 [&.lqd-is-active]:bg-background [&.lqd-is-active]:shadow-[0_2px_12px_hsl(0_0%_0%/10%)] w-full', 'lqd-is-active' => $loop->first])
        ...
        <div class="lqd-ext-chatbot-training-content grid">
            @include('phone-call-agent::home.training.training-tabs.training-tab-website')
            @include('phone-call-agent::home.training.training-tabs.training-tab-pdf')
            @include('phone-call-agent::home.training.training-tabs.training-tab-text')
        </div>
    </div>

    <x-button class="mt-4 w-full" variant="secondary" @click.prevent="setEditingStep('>')" size="lg" type="button">
        @lang('Next')
    </x-button>
</div>
    ...
    <script>
        (() => {
            document.addEventListener('alpine:init', () => {
                Alpine.data('phoneCallAgentTraining', () => ({
                    activeTab: 'url',
                    fetching: false,
                    uploading: false,
                    embeddings: [],
                    ...
                    async fetchEmbeddings() {
                        if (!this.activeAgent || this.fetching) { return; }
                        this.fetching = true;
                        const res = await fetch(`{{ route('dashboard.phone-call-agent.train.data') }}?id=${this.activeAgent.id}`);
                        if (!res.ok) {
                            this.fetching = false;
                    ...
                    async trainEmbeddings(event) {
                        ...
                        this.embeddings = this.embeddings.map(embedding => {
                            ...
                            if (newEmbedding) {
                                if (newEmbedding.status == 'Not Trained') { trainedAll = false; }
                                return newEmbedding;
                            }
                            return embedding;
                        });
                    ...
                    async deleteEmbedding(embeddingId) {
                        ...
                        const res = await fetch('{{ route('dashboard.phone-call-agent.train.delete') }}', {
                            method: 'DELETE',
                            headers: { 'Content-Type': 'application/json', 'Accept': 'application/json', 'X-CSRF-TOKEN': '{{ csrf_token() }}' },
                            body: JSON.stringify({ id: this.activeAgent.id, data: [embeddingId] }),
                        });
                        if (!res.ok) {
                            this.fetching = false;
                        ...
                        const data = await res.json();
                        if (data.status === 200 && data.message) { toastr.success(data.message); }
                    ...
                    async addUrl(event) {
                        ...
                        formData.append('id', this.activeAgent.id);
                        const res = await fetch(form.action, {
                            method: 'POST',
                        ...
                        this.embeddings = this.embeddings.filter(embedding => embedding.type !== 'url').concat(websites);
                        this.fetching = false;
                    },
                    async uploadFile(event) {
                        ...
                        this.embeddings = this.embeddings.filter(embedding => embedding.type !== 'file').concat(files);
                    ...
                    async addText(event, textOrQA = 'text') {
                        ...
                        this.embeddings = this.embeddings.filter(embedding => embedding.type !== 'text').concat(items);
                        event.target.reset();
