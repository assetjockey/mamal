...
class DeleteContactAction implements ActionInterface
{
    ...
    /**
     * Config keys:
     *   - contact_id      (string) : contact ID to delete (required)
     *   - store_output_as (string) : context key (default: deleted_contact)
     */
    public function execute(array $config, array $context, AIAgentWorkflow $workflow, AIAgentWorkflowRun $run): array
    {
        $storeOutputAs = $config['store_output_as'] ?? 'deleted_contact';
        $contactId = $this->interpolate($config['contact_id'] ?? '', $context);

        $graph = $this->factory->make($workflow->user_id);

        $graph->createRequest('DELETE', "/me/contacts/{$contactId}")
            ->execute();

        return array_merge($context, [
            $storeOutputAs => ['contact_id' => $contactId, 'deleted' => true],
        ]);
    }
