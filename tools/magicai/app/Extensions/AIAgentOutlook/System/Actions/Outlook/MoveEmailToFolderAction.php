...
class MoveEmailToFolderAction implements ActionInterface
{
    public function __construct(private readonly GraphClientFactory $factory) {}

    /**
     * Config keys:
     *   - message_id        (string) : message ID to move (required)
     *   - destination_id    (string) : destination folder ID or well-known name (required)
     *   - store_output_as   (string) : context key (default: moved_email)
     */
    public function execute(array $config, array $context, AIAgentWorkflow $workflow, AIAgentWorkflowRun $run): array
    {
        $storeOutputAs = $config['store_output_as'] ?? 'moved_email';
        ...
        $moved = $graph->createRequest('POST', "/me/messages/{$messageId}/move")
