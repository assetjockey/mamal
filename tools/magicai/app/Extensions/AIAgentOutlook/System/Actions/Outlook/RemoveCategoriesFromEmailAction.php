...
class RemoveCategoriesFromEmailAction implements ActionInterface
{
     ...
     *   - store_output_as (string) : context key (default: remove_categories_result)
     */
    public function execute(array $config, array $context, AIAgentWorkflow $workflow, AIAgentWorkflowRun $run): array
    {
        $storeOutputAs = $config['store_output_as'] ?? 'remove_categories_result';
        ...
        $graph->createRequest('PATCH', "/me/messages/{$messageId}")
            ->attachBody(['categories' => []])
            ->execute();

        return array_merge($context, [
            $storeOutputAs => ['message_id' => $messageId, 'categories_cleared' => true],
        ]);
    }
