...
class MarkEmailAsReadUnreadAction implements ActionInterface
{
     ...
     *   - is_read         (bool)   : true to mark read, false to mark unread (required)
     *   - store_output_as (string) : context key (default: mark_result)
     */
    public function execute(array $config, array $context, AIAgentWorkflow $workflow, AIAgentWorkflowRun $run): array
    {
        $storeOutputAs = $config['store_output_as'] ?? 'mark_result';
        $messageId = $this->interpolate($config['message_id'] ?? '', $context);
        $isRead = filter_var($config['is_read'] ?? true, FILTER_VALIDATE_BOOLEAN);

        $graph = $this->factory->make($workflow->user_id);

        $graph->createRequest('PATCH', "/me/messages/{$messageId}")
            ->attachBody(['isRead' => $isRead])
            ->execute();

        return array_merge($context, [
            $storeOutputAs => ['message_id' => $messageId, 'is_read' => $isRead],
        ]);
    }
