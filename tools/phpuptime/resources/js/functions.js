...
// Whitelist Tooltip tags and attributes
...
// Handle ordered select fields
document.querySelectorAll('[data-order-multi-select]').forEach(function(selectContainer) {
    let selectTargetName = selectContainer.dataset.orderMultiSelect;
    let visibleSelectTarget = document.querySelector('#i-' + selectTargetName + '-target');

    let newSelect = [];

    visibleSelectTarget.addEventListener('change', function () {
        // Store the currently selected elements from the visible select input as an array
        let valuesVisibleSelect = Array.from(visibleSelectTarget.selectedOptions).map(optionElement => optionElement.value);

        // Push the selected elements to the array
        valuesVisibleSelect.forEach(function (value) {
            if (!newSelect.includes(value)) {
                newSelect.push(value);
            }
        });

        // Filter the array against the selected items to ensure previous leftovers are removed
        newSelect = newSelect.filter(function (value) {
            return valuesVisibleSelect.includes(value);
        });

        // Remove previous inputs
        selectContainer.innerHTML = '';

        // Add the new inputs to the container
        newSelect.forEach(function(value) {
            let input = document.createElement('input');
            input.type = 'hidden';
            input.name = selectTargetName.replace('-', '_') + '[]';
            input.value = value;
...
document.querySelectorAll('[data-inputs-container]').forEach(containerElement => {
    containerElement.querySelectorAll('[data-inputs-group]:not([data-inputs-template])').forEach(function (inputsGroupElement) {
        inputsGroupElement.querySelector('select[name^="alerts"]').addEventListener('change', function () {
            updateDynamicFieldHelpText(inputsGroupElement, containerElement.dataset.inputsContainer);
        });
    });
    ...
    containerElement.addEventListener('click', function (e) {
        if (e.target.closest('[data-inputs-delete]')) {
            ...
            if (containerElement.querySelectorAll('input[data-input="value"]').length == 1) {
                if (containerElement.querySelector('input[name="' + containerElement.dataset.inputsContainer + '[empty][key]"]')) {
                    containerElement.querySelector('input[name="' + containerElement.dataset.inputsContainer + '[empty][key]"]').removeAttribute('disabled');
                }
                containerElement.querySelector('input[name="' + containerElement.dataset.inputsContainer + '[empty][value]"]').removeAttribute('disabled');
            }
        }

        if (e.target.closest('[data-inputs-add]')) {
            // Clone the input template
            let inputsGroupElement = containerElement.querySelector('[data-inputs-template]').cloneNode(true);
            ...
            inputsGroupElement.querySelectorAll('input, select').forEach(function (inputElement) {
                ...
                inputElement.setAttribute('name', containerElement.dataset.inputsContainer + '['+ inputId +'][' + inputElement.dataset.input + ']');
            });

            containerElement.querySelector('input[name="' + containerElement.dataset.inputsContainer + '[empty][key]"]') && containerElement.querySelector('input[name="' + containerElement.dataset.inputsContainer + '[empty][key]"]').setAttribute('disabled', 'disabled');
            containerElement.querySelector('input[name="' + containerElement.dataset.inputsContainer + '[empty][value]"]') && containerElement.querySelector('input[name="' + containerElement.dataset.inputsContainer + '[empty][value]"]').setAttribute('disabled', 'disabled');

            // On alert input change
            inputsGroupElement.querySelector('select[name="' + containerElement.dataset.inputsContainer + '[' + inputId + '][key]"]') && inputsGroupElement.querySelector('select[name="' + containerElement.dataset.inputsContainer + '[' + inputId + '][key]"]').addEventListener('change', function () {
                updateDynamicFieldHelpText(inputsGroupElement, containerElement.dataset.inputsContainer);
            });
...
/**
 * Handle the dynamic field help text addition.
 *
 * @param inputsGroupElement
 * @param inputsContainerName
 */
let updateDynamicFieldHelpText = (inputsGroupElement, inputsContainerName) => {
    const selectElement = inputsGroupElement.querySelector('select[name^="' + inputsContainerName + '"]');

    // Update the alert value input placeholder
    inputsGroupElement.querySelector('input[name="' + inputsContainerName + '[' + selectElement.getAttribute('name').match(/\d+/)[0] + '][value]"]').setAttribute('placeholder', document.querySelector('[data-alert="' + inputsGroupElement.querySelector('select[name="' + inputsContainerName + '[' + selectElement.getAttribute('name').match(/\d+/)[0] + '][key]"]').value + '"]').dataset.placeholder);

    // Show the input help text
    if (document.querySelector('[data-alert="' + inputsGroupElement.querySelector('select[name="' + inputsContainerName + '[' + selectElement.getAttribute('name').match(/\d+/)[0] + '][key]"]').value + '"]').innerHTML) {
        inputsGroupElement.querySelector('[data-help-text]').classList.remove('d-none');
        inputsGroupElement.querySelector('[data-help-text]').innerHTML = document.querySelector('[data-alert="' + inputsGroupElement.querySelector('select[name="' + inputsContainerName + '[' + selectElement.getAttribute('name').match(/\d+/)[0] + '][key]"]').value + '"]').innerHTML;
    }
    // Hide the input help text
    else {
        inputsGroupElement.querySelector('[data-help-text]').classList.add('d-none');
        inputsGroupElement.querySelector('[data-help-text]').innerHTML = '';
    }
}

if (document.querySelector('#form-monitor')) {
    document.querySelector('select[name="alert_condition"]').addEventListener('change', function () {
        if (this.value == 'url_unavailable') {
            document.querySelector('#alert-text-lookup').classList.add('d-none');
        } else {
            document.querySelector('#alert-text-lookup').classList.remove('d-none');
        }
    });
}
