...
<pre class="m-0">{!! str_replace(':id', '<span class="text-success">{monitor_id}</span>', route('api.stats.show', ['id' => ':id'])) !!}</pre>
</div>
...
curl --location --request GET '{!! str_replace([':id', '%3Aname', '%3Afrom', '%3Ato'], ['<span class="text-success">{monitor_id}</span>', '<span class="text-success">{name}</span>', '<span class="text-success">{from}</span>', '<span class="text-success">{to}</span>'], route('api.stats.show', ['id' => ':id', 'name' => ':name', 'from' => ':from', 'to' => ':to'])) !!}' \
