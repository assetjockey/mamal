...
class CheckoutController
{
    ...
    public function process(ProcessCheckoutRequest $request, $id): View|RedirectResponse
    {
        ...
        elseif ($coupon && $coupon->isRedeemable() == 1) {
    ...
    private function initPayPal(Request $request, Plan $plan, ?Coupon $coupon, ?Collection $taxRates, string $amount): RedirectResponse
    {
        ...
        try {
            $payPalProductRequest = $httpClient->request('GET', 'https://'.(config('settings.paypal_mode') == 'sandbox' ? 'api-m.sandbox' : 'api-m').'.paypal.com/v1/catalogs/products/' . $payPalProduct, [
                'headers' => [
    ...
    private function initStripe(Request $request, Plan $plan, ?Coupon $coupon, ?Collection $taxRates, string $amount): View|RedirectResponse
    {
        ...
        try {
            $stripeProductRequest = $httpClient->request('GET', 'https://api.stripe.com/v1/products/' . $plan->id, [
                'auth' => [config('settings.stripe_secret'), '']
            ]);
            $stripeProduct = json_decode($stripeProductRequest->getBody()->getContents());

            if ($plan->name != $stripeProduct->name) {
                try {
                    $stripeProductRequest = $httpClient->request('POST', 'https://api.stripe.com/v1/products/' . $stripeProduct->id, [
                        'auth' => [config('settings.stripe_secret'), ''],
                        'form_params' => [
                            'name' => $plan->name
                        ]
            ...
            try {
                $stripeProductRequest = $httpClient->request('POST', 'https://api.stripe.com/v1/products', [
                    'auth' => [config('settings.stripe_secret'), ''],
                    'form_params' => [
                        'id' => $plan->id,
                        'name' => $plan->name,
        ...
        try {
            $stripePlanRequest = $httpClient->request('GET', 'https://api.stripe.com/v1/plans/' . $stripePlan, [
                'auth' => [config('settings.stripe_secret'), ''],
            ]);
            $stripePlan = json_decode($stripePlanRequest->getBody()->getContents());
        } catch (Exception) {
            try {
                $stripePlanRequest = $httpClient->request('POST', 'https://api.stripe.com/v1/plans', [
                    'auth' => [config('settings.stripe_secret'), ''],
                    'form_params' => [
                        'id' => $stripePlan,
                        'amount' => $stripeAmount,
        ...
        try {
            ...
            if (config('settings.stripe_ideal')) {
                $paymentMethodTypes['payment_method_types'][] = 'ideal';
            }
            if (config('settings.stripe_klarna')) {
                $paymentMethodTypes['payment_method_types'][] = 'klarna';
            }
            if (config('settings.stripe_sepa_direct_debit')) {
                $paymentMethodTypes['payment_method_types'][] = 'sepa_debit';
            }
            ...
            $stripeSessionRequest = $httpClient->request('POST', 'https://api.stripe.com/v1/checkout/sessions', [
                ...
                'form_params' => [
                    ...
                    'line_items[0][price]' => $stripePlan->id,
                    'line_items[0][quantity]' => 1,

                    'subscription_data[metadata][user]' => $request->user()->id,
                    'subscription_data[metadata][plan]' => $plan->id,
                    'subscription_data[metadata][plan_amount]' => $request->input('interval') == 'year' ? $plan->amount_year : $plan->amount_month,
                    'subscription_data[metadata][amount]' => $amount,
                    'subscription_data[metadata][currency]' => $plan->currency,
                    'subscription_data[metadata][interval]' => $request->input('interval'),
                    'subscription_data[metadata][coupon]' => $coupon->id ?? null,
                    'subscription_data[metadata][tax_rates]' => $taxRates->pluck('id')->implode('_'),
                    ...$paymentMethodTypes,
                ]
            ]);
     ...
     */
    private function initCoinbase(Request $request, Plan $plan, ?Coupon $coupon, ?Collection $taxRates, string $amount): RedirectResponse
    {
        $httpClient = new GuzzleClient();
