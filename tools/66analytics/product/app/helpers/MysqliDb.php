...
class MysqliDb
{
    ...
    /**
     * Insert several rows using real multi-value INSERT statements, split into chunks.
     *
     * @param string $tableName  The name of the table.
     * @param array  $insertData Two-dimensional Data-array containing information for inserting into the DB.
     * @param int    $split      Number of rows to insert per query.
     * @param array  $dataKeys   Optional Table Key names, if not set in insertDataSet.
     *
     * @return bool|array Boolean indicating the insertion failed (false), else return chunk result arrays.
     * @throws Exception
     */
    public function insertBulkInChunks(string $tableName, array $insertData, int $split = 5000, ?array $dataKeys = null)
    {
        if($split < 1) {
            throw new \Exception('Split value must be greater than 0');
        }
        ...
        foreach($chunks as $chunk) {
            ...
            $result = $this->insertBulk($tableName, $chunk, $dataKeys);
            if($result === false) {
                return false;
            }
    ...
    /**
     * Insert several rows at once using a single multi-value INSERT statement.
     *
     * @param string $tableName       The name of the table.
     * @param array  $multiInsertData Two-dimensional Data-array containing information for inserting into the DB.
     * @param array  $dataKeys        Optional Table Key names, if not set in insertDataSet.
     *
     * @return bool|array Boolean indicating the insertion failed (false), else return insert metadata.
     * @throws Exception
     */
    public function insertBulk(string $tableName, array $multiInsertData, ?array $dataKeys = null)
    {
        if(empty($multiInsertData)) {
            return array();
        }

        list($dataColumns, $insertRows) = $this->_normalizeBulkInsertData($multiInsertData, $dataKeys);
        ...
        try {
            $result = $this->_buildBulkInsert($tableName, $insertRows, $dataColumns);
            if($result === false) {
                if($autoCommit) {
                    $this->rollback();
                }
    ...
    /**
     * Normalize multi-row insert data so all rows use the same columns in the same order.
     *
     * @param array $multiInsertData Two-dimensional Data-array containing information for inserting into the DB.
     * @param array $dataKeys        Optional Table Key names, if not set in insertDataSet.
     *
     ...
     * @throws Exception
     */
    private function _normalizeBulkInsertData(array $multiInsertData, ?array $dataKeys = null)
    {
        $dataColumns = null;
        $insertRows = array();
        ...
        foreach($multiInsertData as $insertData) {
            if(!is_array($insertData)) {
                throw new \Exception('Bulk insert data must be a two-dimensional array');
            }

            if($dataKeys !== null) {
                if(count($dataKeys) !== count($insertData)) {
                    throw new \Exception('Bulk insert column count does not match value count');
                }

                $insertData = array_combine($dataKeys, array_values($insertData));
            }

            if($dataColumns === null) {
                $dataColumns = array_keys($insertData);
                if(empty($dataColumns)) {
                    throw new \Exception('Bulk insert rows must contain at least one column');
                }
            }

            $insertRow = array();
            foreach($dataColumns as $column) {
                if(!array_key_exists($column, $insertData)) {
                    throw new \Exception("Bulk insert row is missing column '{$column}'");
                }
                $insertRow[$column] = $insertData[$column];
            }

            foreach(array_keys($insertData) as $column) {
                if(!in_array($column, $dataColumns, true)) {
                    throw new \Exception("Bulk insert row contains unexpected column '{$column}'");
                }
            }
        ...
        return array($dataColumns, $insertRows);
    }

    /**
     * Internal function to build and execute real multi-row INSERT calls.
     *
     * @param string $tableName  The name of the table.
     * @param array  $insertRows Two-dimensional Data-array containing information for inserting into the DB.
     * @param array  $dataColumns Table Key names.
     *
     * @return bool|array Boolean indicating the insert query failed (false), else insert metadata.
     * @throws Exception
     */
    private function _buildBulkInsert($tableName, array $insertRows, array $dataColumns)
    {
        if($this->isSubQuery) {
            return;
        }
        ...
        foreach($insertRows as $insertData) {
            $this->_query .= '(';
            $this->_buildDataPairs($insertData, $dataColumns, true);
            $this->_query .= '), ';
        }

        $this->_query = rtrim($this->_query, ', ');
        $this->_buildBulkOnDuplicate($insertRows[0]);
    ...
    /**
     * Helper function to add ON DUPLICATE KEY UPDATE to multi-row insert statements.
     *
     * @param array $tableData Variable with values
     *
     * @throws Exception
     */
    protected function _buildBulkOnDuplicate($tableData)
    {
        if(is_array($this->_updateColumns) && !empty($this->_updateColumns)) {
            $this->_query .= " ON DUPLICATE KEY UPDATE ";
            if($this->_lastInsertId) {
            ...
            if(!empty($updateColumns)) {
                $this->_buildDataPairs($tableData, $updateColumns, false);
            }
