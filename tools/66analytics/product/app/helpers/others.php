...
function get_chart_data(array $main_array) {
    ...
    /* generate labels */
    $dataset_arrays['labels']   = json_encode(array_map('strval', array_keys($main_array)));
...
function is_ip_in_cidr($ip, $cidr) {
    ...
    $subnet_binary = inet_pton($cidr_parts[0]);
    $bits = (int) $cidr_parts[1];
    ...
    if($bytes && substr($ip_binary, 0, $bytes) !== substr($subnet_binary, 0, $bytes)) {
        return false;
    }
    ...
    if($remaining_bits) {
        ...
        return (ord($ip_binary[$bytes]) & $mask) == (ord($subnet_binary[$bytes]) & $mask);
    }
...
function is_cloudflare_ip($ip) {
    ...
    foreach($cloudflare_ip_ranges as $cloudflare_ip_range) {
        if(is_ip_in_cidr($ip, $cloudflare_ip_range)) {
            return true;
        }
    }
...
function get_ip() {
    ...
    /* use Cloudflare visitor IP only from Cloudflare requests */
    if(
        ...
        $cached_ip_address = $_SERVER['HTTP_CF_CONNECTING_IP'];
        return $cached_ip_address;
    }
...
function process_dynamic_page_link($row) {
	...
	if($row->url == 'broadcast-subscribe') {
		if(!settings()->content->broadcasts_is_enabled || (!is_logged_in() && !settings()->content->broadcasts_guests_is_enabled)) return null;

		return [
			'url' => url('broadcast-subscribe'),
			'target' => $row->target,
			'title' => l('broadcast_subscribe.menu'),
			'icon' => $row->icon,
			'attributes' => [],
			'category_id' => $row->footer_category_id,
		];
