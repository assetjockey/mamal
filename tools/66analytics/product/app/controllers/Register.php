	...
	public function index() {
		...
		if(!settings()->users->register_is_enabled) {
            ...
            if(
                ...
                && db()->where('token', $unique_registration_identifier)->where('user_email', $_GET['email'])->where('is_accepted', '0')->has('teams_associations')
            ) {
		...
		if(!empty($_POST) && !settings()->users->register_only_social_logins) {
			...
			$_POST['is_broadcast_subscribed'] = settings()->content->broadcasts_is_enabled && settings()->content->broadcasts_display_register_checkbox && isset($_POST['is_broadcast_subscribed']);
			...
			if(!Alerts::has_field_errors() && !Alerts::has_errors()) {
				...
				if($active == '1') {
					...
					/* Send internal notification if needed */
					if(settings()->internal_notifications->admins_is_enabled && settings()->internal_notifications->new_newsletter_subscriber && $_POST['is_broadcast_subscribed']) {
						db()->insert('internal_notifications', [
							'for_who' => 'admin',
							'from_who' => 'system',
