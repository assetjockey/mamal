...
class AnnotationUpdate extends Controller {
    ...
    public function index() {
        ...
        if(!$annotation = db()->where('annotation_id', $annotation_id)->where('website_id', $this->website->website_id)->where('user_id', $this->user->user_id)->getOne('annotations')) {
            redirect('annotations');
        }

        /* Check for the plan limit */
        $total_rows = database()->query("SELECT COUNT(*) AS `total` FROM `annotations` WHERE `website_id` = {$annotation->website_id} AND `user_id` = {$this->user->user_id}")->fetch_object()->total ?? 0;
        ...
        if(!empty($_POST)) {
            ...
            if(!Alerts::has_field_errors() && !Alerts::has_errors()) {
                ...
                redirect('annotation-update/' . $annotation_id);
            }
        }
        ...
        $view = new \Altum\View('annotation-update/index', (array) $this);
