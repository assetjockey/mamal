...
class AdminSettings extends Controller {
	...
	public function content() {
		...
		if(!empty($_POST)) {
            ...
            if(isset($_POST['author_id']) && is_array($_POST['author_id'])) {
                foreach($_POST['author_id'] as $key => $author_id) {
                    $author_id_original = $author_id;
                    $author_id = get_slug($author_id);

                    $author_name = isset($_POST['author_name'][$key]) ? $_POST['author_name'][$key] : '';
                    $author_name = isset($_POST['author_name'][$author_id_original]) ? $_POST['author_name'][$author_id_original] : $author_name;
                    $author_name = isset($_POST['author_name'][$author_id]) ? $_POST['author_name'][$author_id] : $author_name;
                    $author_name = input_clean($author_name, 256);

                    if(!$author_id || !$author_name) {
                        continue;
                    }

                    $author_avatar = isset($_POST['author_avatar'][$key]) ? $_POST['author_avatar'][$key] : '';
                    $author_avatar = isset($_POST['author_avatar'][$author_id_original]) ? $_POST['author_avatar'][$author_id_original] : $author_avatar;
                    $author_avatar = isset($_POST['author_avatar'][$author_id]) ? $_POST['author_avatar'][$author_id] : $author_avatar;

                    $author_description = isset($_POST['author_description'][$key]) ? $_POST['author_description'][$key] : '';
                    $author_description = isset($_POST['author_description'][$author_id_original]) ? $_POST['author_description'][$author_id_original] : $author_description;
                    $author_description = isset($_POST['author_description'][$author_id]) ? $_POST['author_description'][$author_id] : $author_description;

                    $author_url = isset($_POST['author_url'][$key]) ? $_POST['author_url'][$key] : '';
                    $author_url = isset($_POST['author_url'][$author_id_original]) ? $_POST['author_url'][$author_id_original] : $author_url;
                    $author_url = isset($_POST['author_url'][$author_id]) ? $_POST['author_url'][$author_id] : $author_url;

                    $blog_authors[$author_id] = [
                        'id' => $author_id,
                        'name' => $author_name,
                        'avatar' => get_url($author_avatar, 512, false),
                        'description' => input_clean($author_description, 512),
                        'url' => get_url($author_url, 512, false),
                        'url_nofollow' => isset($_POST['author_url_nofollow'][$key]) || isset($_POST['author_url_nofollow'][$author_id_original]) || isset($_POST['author_url_nofollow'][$author_id]),
                    ];
                }
            }
            ...
            $value = json_encode([
                ...
                /* Blog featured and related posts */
                'blog_featured_post_is_enabled' => isset($_POST['blog_featured_post_is_enabled']),
                'blog_related_posts_is_enabled' => isset($_POST['blog_related_posts_is_enabled']),
                'blog_related_posts_limit' => (int) max(1, min((int) $_POST['blog_related_posts_limit'], 12)),
                'blog_table_of_contents_is_enabled' => isset($_POST['blog_table_of_contents_is_enabled']),
                'blog_authors_is_enabled' => isset($_POST['blog_authors_is_enabled']),
                ...
                'blog_popular_widget_posts_limit' => (int) max(1, (int) $_POST['blog_popular_widget_posts_limit']),
                'blog_latest_widget_is_enabled' => isset($_POST['blog_latest_widget_is_enabled']),
                'blog_latest_widget_posts_limit' => (int) max(1, (int) $_POST['blog_latest_widget_posts_limit']),
                ...
                'pages_table_of_contents_is_enabled' => isset($_POST['pages_table_of_contents_is_enabled']),
                'pages_views_is_enabled' => isset($_POST['pages_views_is_enabled']),

				'broadcasts_is_enabled' => isset($_POST['broadcasts_is_enabled']),
				'broadcasts_guests_is_enabled' => isset($_POST['broadcasts_guests_is_enabled']),
				...
				'broadcasts_display_index_box' => isset($_POST['broadcasts_display_index_box']),
				'broadcasts_display_register_checkbox' => isset($_POST['broadcasts_display_register_checkbox']),
				'broadcasts_display_account_checkbox' => isset($_POST['broadcasts_display_account_checkbox']),
            ]);
    ...
    public function payment() {
        ...
        if(!empty($_POST)) {
            ...
            $value = json_encode([
                ...
                'user_plan_renewal_reminder' => (int) $_POST['user_plan_renewal_reminder'],
    ...
    public function razorpay() {
        ...
        if(!empty($_POST)) {
            ...
            $value = json_encode([
                ...
                'webhook_secret' => $_POST['webhook_secret'],
				'webhook_identifier' => $_POST['webhook_identifier'],
                'currencies' => $_POST['currencies'] ?? [],
            ]);
    ...
    public function onepay() {
        ...
        if(!empty($_POST)) {
            ...
            /* Keep only the 1pay instance name */
            $_POST['instance_name'] = input_clean(trim($_POST['instance_name']), 128);
            $_POST['instance_name'] = str_replace(['https://', 'http://'], '', $_POST['instance_name']);
            $_POST['instance_name'] = explode('/', $_POST['instance_name'])[0];
            $_POST['instance_name'] = str_ends_with($_POST['instance_name'], '.1pay.ch') ? str_replace('.1pay.ch', '', $_POST['instance_name']) : $_POST['instance_name'];

            /* Clean optional 1pay fields */
            $_POST['look_and_feel_profile'] = input_clean(trim($_POST['look_and_feel_profile']), 64);
            $_POST['subscription_cancellation_interval'] = input_clean(trim($_POST['subscription_cancellation_interval']), 32);

            $value = json_encode([
                'is_enabled' => $_POST['is_enabled'],
                'instance_name' => $_POST['instance_name'],
                'api_key' => $_POST['api_key'],
                'webhook_signing_key' => $_POST['webhook_signing_key'],
                'look_and_feel_profile' => $_POST['look_and_feel_profile'],
                'subscription_cancellation_interval' => $_POST['subscription_cancellation_interval'],
                'currencies' => $_POST['currencies'] ?? [],
            ]);

            $this->update_settings('onepay', $value);
        }
    }
        ...
        if(!empty($_POST)) {
            ...
            if(!Alerts::has_field_errors() && !Alerts::has_errors()) {
                ...
                foreach($css_files as $key => $value) {
                    ...
                    /* Offload uploading */
                    if(\Altum\Plugin::is_active('offload') && settings()->offload->assets_url) {
                        try {
        ...
        if(!empty($_POST)) {
            ...
            if(isset($this->params[0]) && $this->params[0] == 'regenerate') {
                ...
                $clean_fields = [
                    ...
                    'broadcast_subscriber_new',
                    'broadcast_subscriber_unsubscribe',
                    'broadcast_subscriber_delete',
                    'payment_new',
                    'code_redeemed',
                    'contact',
                    'cron_start',
    ...
    public function analytics() {
        ...
        if(!empty($_POST)) {
            ...
            $_POST['annotations_is_enabled'] = (int) isset($_POST['annotations_is_enabled']);
            $_POST['dashboard_views_is_enabled'] = (int) isset($_POST['dashboard_views_is_enabled']);
            ...
            $value = json_encode([
                'annotations_is_enabled' => $_POST['annotations_is_enabled'],
                'dashboard_views_is_enabled' => $_POST['dashboard_views_is_enabled'],
