...
let type_handler = (selector, data_key, matching_selector = '=') => {
    ...
    document.querySelectorAll(`[${data_key}]:not([${data_key}${matching_selector}"${type}"])`).forEach(element => {
        ...
        /* Check for input ignoring flag */
        let ignore_input = element.getAttribute('data-type-handler-ignore-input');
...
let tooltips_initiate = () => {
    ...
    $('[data-tooltip-hide-on-click]').off('click.tooltip_hide').on('click.tooltip_hide', event => {
        $(event.currentTarget).tooltip('hide');
    });

    window.removeEventListener('beforeprint', tooltips_before_print);
    window.addEventListener('beforeprint', tooltips_before_print);
}

let tooltips_before_print = () => {
    $('[data-toggle="tooltip"],[data-tooltip]').tooltip('hide');
}
...
const nr = (number, decimals = 0, display_decimals_if_zero = false, extra = false) => {
    ...
    if(extra) {
        ...
        if(decimals > 0) {
            ...
            formatted_number = String(formatted_number).replace(dotzero, '');
        }

        return formatted_number;
    }

    if(number === 0 && !display_decimals_if_zero) {
        return 0;
    }
...
let character_counter_count = (element, target) => {
    ...
    let color_class = 'text-muted';
    if(filled_percentage > 75) color_class = 'text-info';
    if(filled_percentage > 90) color_class = 'text-warning';
    if(filled_percentage > 97) color_class = 'text-danger';

    character_counter_wrapper.innerHTML = `
        <span data-current-characters>${current_characters}</span>/<span data-allowed-characters>${allowed_characters}</span> (<span class="${color_class}">${filled_percentage}%</span>)
    `;
}
