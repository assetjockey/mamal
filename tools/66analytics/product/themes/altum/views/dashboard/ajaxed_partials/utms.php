...
<div class="altum-animate altum-animate-fill-none altum-animate-fade-in">
    ...
    <?php else: ?>
            ...
            <div class="row-fade-show-icon position-relative">
                ...
                <div class="d-flex justify-content-between mb-1 row-fade-show-icon-content">
                    <div class="d-flex align-items-center" style="min-width:0;">
                        <?php if($row->utm): ?>
                            <a href="#" data-target="#utms_medium_campaign_modal" data-toggle="modal" data-utm-source="<?= $row->utm ?>" class="text-truncate" style="min-width:0;"><?= $row->utm ?></a>
                            <a href="#" data-add-filter data-filter-by="utm_source" data-filter-rule="is" data-filter-value="<?= e($row->utm) ?>" class="text-muted ml-1 flex-shrink-0" data-toggle="tooltip" title="<?= l('analytics.filters.create') ?>" data-tooltip-hide-on-click><i class="fas fa-fw fa-xs fa-filter"></i></a>
                        <?php else: ?>
                            <?= l('global.none') ?>
                        <?php endif ?>
                    </div>
        ...
        <?php if($data->total_rows > count($data->rows)): ?>
            <a href="<?= url($this->base_url_path . 'utms') ?>" class="text-body font-size-small mt-2"><?= sprintf(l('global.view_x_more'), nr($data->total_rows - count($data->rows))) ?></a>
        <?php endif ?>
