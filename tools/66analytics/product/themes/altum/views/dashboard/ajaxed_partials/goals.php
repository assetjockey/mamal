...
<div class="altum-animate altum-animate-fill-none altum-animate-fade-in">
    ...
    <?php else: ?>
        ...
        <?php foreach($data->rows as $row): ?>
            ...
            <div class="row-fade-show-icon position-relative">
                ...
                <div class="d-flex justify-content-between mb-1 row-fade-show-icon-content">
                    <div>
                        <a href="#" class="text-truncate text-body" data-toggle="modal" data-target="#goal_update_modal" data-goal-id="<?= $row->goal_id ?>" data-key="<?= $row->key ?>" data-type="<?= $row->type ?>" data-path="<?= ltrim($row->path, '/') ?>" data-scroll-percentage="<?= $row->scroll_percentage ?>" data-name="<?= $row->name ?>"><?= $row->name ?></a>
                    </div>
        ...
        <?php if($data->total_rows > count($data->rows)): ?>
            <a href="<?= url($this->base_url_path . 'goals') ?>" class="text-body font-size-small mt-2"><?= sprintf(l('global.view_x_more'), nr($data->total_rows - count($data->rows))) ?></a>
        <?php endif ?>
