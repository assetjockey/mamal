...
<div class="altum-animate altum-animate-fill-none altum-animate-fade-in">
    ...
    <?php else: ?>
        ...
        <?php foreach($data->rows as $row): ?>
            ...
            <?php $row_filters = [] ?>
            <?php if($row->utm_medium) $row_filters[] = ['by' => 'utm_medium', 'rule' => 'is', 'value' => $row->utm_medium] ?>
            <?php if($row->utm_campaign) $row_filters[] = ['by' => 'utm_campaign', 'rule' => 'is', 'value' => $row->utm_campaign] ?>
            ...
            <div class="row-fade-show-icon position-relative">
                ...
                <div class="d-flex justify-content-between mb-1 row-fade-show-icon-content">
                    <div class="d-flex align-items-center" style="min-width:0;">
                        <span class="text-truncate" style="min-width:0;"><?= $row->utm_medium ?? l('global.none') ?> / <?= $row->utm_campaign ?? l('global.none') ?></span>

                        <?php if(count($row_filters)): ?>
                            <a href="#" data-add-filters="<?= e(json_encode($row_filters)) ?>" class="text-muted ml-1 flex-shrink-0" data-toggle="tooltip" title="<?= l('analytics.filters.create') ?>" data-tooltip-hide-on-click><i class="fas fa-fw fa-xs fa-filter"></i></a>
                        <?php endif ?>
                    </div>
