    ...
    <div class="card-body">
        ...
        <form id="broadcast_update_form" action="" method="post" role="form">
            ...
            <div class="form-group">
                ...
                <small class="form-text text-muted"><?= sprintf(l('global.variables'), '<code data-copy>' . implode('</code> , <code data-copy>', ['{{WEBSITE_TITLE}}', '{{USER:NAME}}', '{{USER:EMAIL}}']) . '</code> <a href="#" class="text-decoration-none font-weight-500 text-muted" data-show-more>' . l('global.show_more') . '</a><span hidden> , <code data-copy>' . implode('</code> , <code data-copy>', ['{{USER:CONTINENT_NAME}}', '{{USER:COUNTRY_NAME}}', '{{USER:CITY_NAME}}', '{{USER:DEVICE_TYPE}}', '{{USER:OS_NAME}}', '{{USER:BROWSER_NAME}}', '{{USER:BROWSER_LANGUAGE}}']) . '</code></span>') ?></small>
            </div>

            <div class="form-group custom-control custom-switch" data-type="external">
                <input id="is_system_email" name="is_system_email" type="checkbox" value="system" class="custom-control-input" <?= ($data->broadcast->settings->is_system_email ?? false) ? 'checked="checked"' : null ?> <?= $data->broadcast->status == 'sent' ? 'disabled="disabled"' : null ?>>
                <label class="custom-control-label" for="is_system_email"><?= l('admin_broadcasts.is_system_email') ?></label>
            ...
            <div class="form-group" data-segment="custom" data-broadcast-audience="system">
            ...
            <div class="form-group" data-segment="custom" data-broadcast-audience="">
                <label for="broadcast_subscribers_ids"><i class="fas fa-fw fa-sm fa-users text-muted mr-1"></i> <?= l('admin_broadcasts.broadcast_subscribers_ids') ?></label>
                <input type="text" id="broadcast_subscribers_ids" name="broadcast_subscribers_ids" value="<?= $data->broadcast->broadcast_subscribers_ids ?? '' ?>" class="form-control <?= \Altum\Alerts::has_field_errors('broadcast_subscribers_ids') ? 'is-invalid' : null ?>" placeholder="<?= l('admin_broadcasts.broadcast_subscribers_ids_placeholder') ?>" required="required" <?= $data->broadcast->status == 'sent' ? 'readonly="readonly"' : null ?> />
                <?= \Altum\Alerts::output_field_error('broadcast_subscribers_ids') ?>
                <small class="form-text text-muted"><?= l('admin_broadcasts.broadcast_subscribers_ids_help') ?></small>
            </div>

            <div data-segment="filter" data-broadcast-audience="">
                        ...
                        <?php foreach(['registered', 'guest'] as $subscriber_type): ?>
                            <div class="col-6 mb-3">
                                <div class="custom-control custom-checkbox">
                                    <input id="<?= 'filters_subscriber_type###' . $subscriber_type ?>" name="filters_subscriber_type[]" value="<?= $subscriber_type ?>" type="checkbox" class="custom-control-input" <?= in_array($subscriber_type, $data->broadcast->settings->filters_subscriber_type ?? []) ? 'checked="checked"' : null ?>>
                ...
                <div class="form-group">
                    <label><i class="fas fa-fw fa-sm fa-right-to-bracket text-muted mr-1"></i> <?= l('admin_broadcasts.segment.filter.subscription_source') ?></label>
                    <div class="row">
                        <?php foreach(['register', 'account', 'index'] as $source): ?>
                            <div class="col-6 mb-3">
                                <div class="custom-control custom-checkbox">
                                    <input id="<?= 'filters_broadcast_subscribers_source###' . $source ?>" name="filters_broadcast_subscribers_source[]" value="<?= $source ?>" type="checkbox" class="custom-control-input" <?= in_array($source, $data->broadcast->settings->filters_broadcast_subscribers_source ?? []) ? 'checked="checked"' : null ?>>
                                    <label class="custom-control-label" for="<?= 'filters_broadcast_subscribers_source###' . $source ?>"><?= l('admin_broadcasts.segment.filter.subscription_source.' . $source) ?></label>
                                </div>
                            </div>
                        <?php endforeach ?>
                    </div>
            ...
            <div data-segment="filter" data-broadcast-audience="system">
                <h2 class="h6 mb-3"><?= l('admin_broadcasts.segment.filter.account') ?></h2>
                ...
                <div class="form-group">
                    ...
                    <div class="row">
                        <div class="col-6 mb-3">
                            <div class="custom-control custom-checkbox">
                                <input id="<?= 'filters_plans###free' ?>" name="filters_plans[]" value="free" type="checkbox" class="custom-control-input" <?= in_array('free', $data->broadcast->settings->filters_plans ?? []) ? 'checked="checked"' : null ?>>
                        ...
                        <div class="col-6 mb-3">
                            <div class="custom-control custom-checkbox">
                                <input id="<?= 'filters_plans###custom' ?>" name="filters_plans[]" value="custom" type="checkbox" class="custom-control-input" <?= in_array('custom', $data->broadcast->settings->filters_plans ?? []) ? 'checked="checked"' : null ?>>
                        ...
                        <?php foreach($data->plans as $plan): ?>
                            <div class="col-6 mb-3">
                                <div class="custom-control custom-checkbox">
                                    <input id="<?= 'filters_plans###' . $plan->plan_id ?>" name="filters_plans[]" value="<?= $plan->plan_id ?>" type="checkbox" class="custom-control-input" <?= in_array($plan->plan_id, $data->broadcast->settings->filters_plans ?? []) ? 'checked="checked"' : null ?>>
                ...
                <div class="form-group">
                    ...
                    <div class="row">
                        <div class="col-6 mb-3">
                            <div class="custom-control custom-checkbox">
                                <input id="<?= 'filters_status###active' ?>" name="filters_status[]" value="1" type="checkbox" class="custom-control-input" <?= in_array('1', $data->broadcast->settings->filters_status ?? []) || in_array(1, $data->broadcast->settings->filters_status ?? [], true) ? 'checked="checked"' : null ?>>
                        ...
                        <div class="col-6 mb-3">
                            <div class="custom-control custom-checkbox">
                                <input id="<?= 'filters_status###unconfirmed' ?>" name="filters_status[]" value="0" type="checkbox" class="custom-control-input" <?= in_array('0', $data->broadcast->settings->filters_status ?? []) || in_array(0, $data->broadcast->settings->filters_status ?? [], true) ? 'checked="checked"' : null ?>>
                        ...
                        <div class="col-6 mb-3">
                            <div class="custom-control custom-checkbox">
                                <input id="<?= 'filters_status###disabled' ?>" name="filters_status[]" value="2" type="checkbox" class="custom-control-input" <?= in_array('2', $data->broadcast->settings->filters_status ?? []) || in_array(2, $data->broadcast->settings->filters_status ?? [], true) ? 'checked="checked"' : null ?>>
                ...
                <div class="form-group">
                    ...
                    <div class="row">
                        <?php foreach(['direct', 'admin_create', 'admin_api_create', 'facebook', 'twitter', 'discord', 'google', 'linkedin', 'microsoft'] as $source): ?>
                            <div class="col-6 mb-3">
                                <div class="custom-control custom-checkbox">
                                    <input id="<?= 'filters_source###' . $source ?>" name="filters_source[]" value="<?= $source ?>" type="checkbox" class="custom-control-input" <?= in_array($source, $data->broadcast->settings->filters_source ?? []) ? 'checked="checked"' : null ?>>
                ...
                <div class="form-group">
                    ...
                    <select id="filters_countries" name="filters_countries[]" class="custom-select" multiple="multiple">
                        <?php foreach(get_countries_array() as $key => $value): ?>
                            <option value="<?= $key ?>" <?= in_array($key, $data->broadcast->settings->filters_countries ?? []) ? 'selected="selected"' : null ?>><?= $value ?></option>
                        <?php endforeach ?>
                    </select>
            ...
            <div class="form-group" data-segment="filter">
                ...
                <div class="row">
                    <?php foreach(\Altum\Language::$active_languages as $language_name => $language_code): ?>
                        <div class="col-6 mb-3">
                            <div class="custom-control custom-checkbox">
                                <input id="<?= 'filters_languages###' . $language_code ?>" name="filters_languages[]" value="<?= $language_name ?>" type="checkbox" class="custom-control-input" <?= in_array($language_name, $data->broadcast->settings->filters_languages ?? []) ? 'checked="checked"' : null ?>>
            ...
            <button class="btn btn-block btn-gray-200 font-size-little-small font-weight-450 mb-4" type="button" data-toggle="collapse" data-target="#advanced_container" aria-expanded="false" aria-controls="advanced_container" data-segment="filter">
                ...
                <div class="form-group" data-segment="filter">
                    ...
                    <div class="row">
                        <?php foreach(['desktop', 'tablet', 'mobile'] as $device_type): ?>
                            <div class="col-6 mb-3">
                                <div class="custom-control custom-checkbox">
                                    <input id="<?= 'filters_device_type###' . $device_type ?>" name="filters_device_type[]" value="<?= $device_type ?>" type="checkbox" class="custom-control-input" <?= in_array($device_type, $data->broadcast->settings->filters_device_type ?? []) ? 'checked="checked"' : null ?>>
                    ...
                    <div class="form-group">
                        ...
                        <select id="filters_continents" name="filters_continents[]" class="custom-select" multiple="multiple">
                            <?php foreach(get_continents_array() as $continent_code => $continent_name): ?>
                                <option value="<?= $continent_code ?>" <?= in_array($continent_code, $data->broadcast->settings->filters_continents ?? []) ? 'selected="selected"' : null ?>><?= $continent_name ?></option>
                            <?php endforeach ?>
                        </select>
                        <?php if(get_this_device_type() == 'desktop'): ?><small class="form-text text-muted"><?= l('global.select_multiple_help') ?></small><?php endif ?>
                    </div>
            ...
            <div class="alert alert-info" role="alert"><?= sprintf(l('admin_broadcast_create.info'), (settings()->content->broadcasts_emails_per_cron ?? 40)) ?></div>
...
<script>
    ...
    let broadcast_type_handler = () => {
        let segment = document.querySelector('[name="segment"]').value;
        let audience = document.querySelector('#is_system_email').checked ? 'system' : '';

        document.querySelectorAll('[data-segment], [data-broadcast-audience]').forEach(element => {
            let segment_matches = !element.hasAttribute('data-segment') || element.getAttribute('data-segment') == segment;
            let audience_matches = !element.hasAttribute('data-broadcast-audience') || element.getAttribute('data-broadcast-audience') == audience;
            let is_visible = segment_matches && audience_matches;

            element.classList.toggle('d-none', !is_visible);
    ...
    document.querySelector('[name="segment"]').addEventListener('change', broadcast_type_handler);
    document.querySelector('#is_system_email').addEventListener('change', broadcast_type_handler);

    broadcast_type_handler();
    ...
    document.querySelectorAll('#is_system_email,[name^="filters_"]').forEach(element => element.addEventListener('change', async event => {
        await get_segment_count();
    }));
    ...
    let get_segment_count = async () => {
        ...
        /* Keep audience synced */
        if(document.querySelector('#is_system_email').checked) {
            query.set('is_system_email', '1');
        }

        /* Filter preparing on query string */
        if(segment == 'filter') {
            query = new URLSearchParams(new FormData(document.querySelector('#broadcast_update_form')));
            ['preview_email', 'content', 'name'].forEach(field => query.delete(field));
        }
