...
class AdminSettings extends Controller {
	...
	public function custom_images() {
		...
		if(!empty($_POST)) {
			...
			$images_keys = [
				'index/api.webp',
				'index/archived-audit.webp',
				'index/audit.webp',
				'index/audit_refresh.webp',
				'index/domains.webp',
				'index/export.webp',
				'index/generate-audit.webp',
				'index/hero.webp',
				'index/password_protection.webp',
				'index/public_audits.webp',
	...
	public function audits() {
		...
		if(!empty($_POST)) {
			...
			/* Proxy testing */
			if(isset($_POST['proxy_test'])) {
				$available_proxies = settings()->audits->proxies ?? [];
				$proxy_key = $_POST['proxy_test'];

				if(isset($available_proxies[$proxy_key])) {
					$selected_proxy = $available_proxies[$proxy_key];
					...
					try {
						...
						$curl_opts = [
							...
							CURLOPT_MAXREDIRS => 5,
							CURLOPT_PROXY => $selected_proxy->address,
							CURLOPT_PROXYTYPE => $proxy_type_map[$selected_proxy->type] ?? CURLPROXY_HTTP,
						];
						...
						/* Send test request */
						$response = \Unirest\Request::get('https://api.ipify.org?format=json', $request_headers);

						/* Evaluate response */
						if($response->code == 200) {

							/* Set a nice success message */
							Alerts::add_success(l('admin_settings.audits.proxy_test.success'));

							/* Redirect */
							redirect('admin/settings/audits');

						} else {

							Alerts::add_error(l('admin_settings.audits.proxy_test.error'));
							Alerts::add_info('Request response code: ' . $response->code);
						...
						Alerts::add_error(l('admin_settings.audits.proxy_test.error'));
						Alerts::add_info('Request error: ' . $exception->getMessage());

						/* Redirect */
						redirect('admin/settings/audits');

					} finally {
						/* Always clear curl opts */
			...
			$_POST['blacklisted_domains'] = array_filter(array_map('trim', explode(',', $_POST['blacklisted_domains'])));
			$_POST['google_safe_browsing_api_key'] = trim($_POST['google_safe_browsing_api_key']);
			$_POST['audits_per_cron'] = (int) $_POST['audits_per_cron'];
			$_POST['excluded_words'] = array_filter(array_map('trim', explode(',', $_POST['excluded_words'] ?? '')));
			$_POST['ai_is_enabled'] = (int) isset($_POST['ai_is_enabled']);
			...
			foreach($_POST['address'] as $key => $value) {
				$proxies[$key] = [
					'is_enabled' => isset($_POST['name'][$key]),
					'name' => $_POST['name'][$key],
					'type' => $_POST['type'][$key],
			...
			$value = json_encode([
				...
				'bulk_processing_before_queue' => (int) $_POST['bulk_processing_before_queue'],
				...
				'audits_per_cron' => $_POST['audits_per_cron'],
				'excluded_words' => $_POST['excluded_words'],

                /* AI */
				'ai_is_enabled' => $_POST['ai_is_enabled'],
				'openai_api_url' => $_POST['openai_api_url'],
				'openai_api_key' => $_POST['openai_api_key'],
				'openai_webhook_secret_key' => $_POST['openai_webhook_secret_key'],
				'openai_model' => $_POST['openai_model'],
				...
				/* Proxies */
				'proxies_is_enabled' => isset($_POST['proxies_is_enabled']),
				'proxies_exclusive' => isset($_POST['proxies_exclusive']),
				'proxies' => $proxies,
			]);
