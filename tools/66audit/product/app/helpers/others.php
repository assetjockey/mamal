...
function process_dynamic_page_link($row) {
	...
	if($row->url == 'top-websites') {
		if(!settings()->audits->directory_is_enabled) return null;
		if(!(settings()->audits->directory_access == 'everyone' || (settings()->audits->directory_access == 'users' && is_logged_in()))) return null;

		return [
			'url' => url('top-websites'),
			'target' => $row->target,
			'title' => l('top_websites.menu'),
			'icon' => $row->icon,
			'attributes' => [],
			'category_id' => $row->footer_category_id,
		];
