...
<div class="container">
    ...
    <?php if($total_archived_audits): ?>
        <div class="position-relative mb-3 d-print-none">
            ...
            <div class="d-flex align-items-center audit-archived-audits-wrapper">
                ...
                if($data->audit->is_external_redirected || $data->audit->refresh_error) {
                ...
                ?>
                <div id="current_audit" class="card p-3 text-center position-relative white-space-normal min-width-fit-content border-primary" data-toggle="tooltip" data-html="true" title="<?= \Altum\Date::get($data->audit->last_refresh_datetime, 2) . '<br /><small>' . \Altum\Date::get($data->audit->last_refresh_datetime, 3) . '</small>' ?>">
    ...
    <?php
    $major_issues_percentage = $data->audit->total_tests ? number_format($data->audit->major_issues * 100 / $data->audit->total_tests, '2', '.', '') : 0;
    $moderate_issues_percentage = $data->audit->total_tests ? number_format($data->audit->moderate_issues * 100 / $data->audit->total_tests, '2', '.', '') : 0;
    $minor_issues_percentage = $data->audit->total_tests ? number_format($data->audit->minor_issues * 100 / $data->audit->total_tests, '2', '.', '') : 0;
    $passed_tests_percentage = $data->audit->total_tests ? number_format($data->audit->passed_tests * 100 / $data->audit->total_tests, '2', '.', '') : 0;
    ?>
        ...
        <div class="d-flex flex-column flex-md-row">
            ...
            <div class="card-body text-truncate d-flex justify-content-between align-items-center">
                <div class="text-truncate">
                    ...
                    <?php if($data->audit->refresh_error || $data->audit->is_external_redirected || $data->audit->is_queued): ?>
                    ...
                    <?php else: ?>
                        ...
                        <p class="small text-muted white-space-normal m-0">
                            ...
                            if($data->audits_stats->average_score > 0) {
                                $percentage_difference = round((($data->audit->score - $data->audits_stats->average_score) / $data->audits_stats->average_score) * 100, 2);
                            } else {
                                $percentage_difference = 0;
                            }
                            ?>

                            <?php if($data->audit->score >= $data->audits_stats->average_score): ?>
                ...
                <div class="d-flex">
                    <?php if(!$data->audit->is_queued && !$data->audit->is_external_redirected && !$data->audit->refresh_error): ?>
                        <div class="dropdown">
                            ...
                            <div class="dropdown-menu dropdown-menu-right d-print-none">
                                <a href="<?= url('audit/' . $data->audit->audit_id . '&export=csv')  ?>" target="_blank" class="dropdown-item <?= $this->user->plan_settings->export->csv ? null : 'disabled pointer-events-all' ?>" <?= $this->user->plan_settings->export->csv ? null : get_plan_feature_disabled_info() ?>>
                                    <i class="fas fa-fw fa-sm fa-file-csv mr-2"></i> <?= sprintf(l('global.export_to'), 'CSV') ?>
                                </a>
                                <a href="<?= url('audit/' . $data->audit->audit_id . '&export=json') ?>" target="_blank" class="dropdown-item <?= $this->user->plan_settings->export->json ? null : 'disabled pointer-events-all' ?>" <?= $this->user->plan_settings->export->json ? null : get_plan_feature_disabled_info() ?>>
	...
	<?php /* Queued audits */ ?>
	<?php if($data->audit->is_queued):?>
        <div class="card mb-4">
            <div class="card-header bg-white">
                <div class="d-flex justify-content-between">
                    <span class="small font-weight-bold"><?= l('audits.queued') ?></span>

                    <span class="badge badge-light-light">
                        <i class="fas fa-fw fa-sm fa-stopwatch"></i>
                    </span>
                </div>
            </div>

            <div class="card-body">
                <span class="small font-weight-bold text-muted"><?= $data->audit->refresh_error ?></span>
                <div class="d-flex align-items-center">
					<?php for($i = 0; $i <= 9; $i++): ?>
                        <div class="audit-badge audit-badge-loading mr-1"></div>
					<?php endfor ?>
                </div>
            </div>
        </div>
    ...
    <?php /* Error notice */ ?>
    <?php if($data->audit->refresh_error):?>
        <div class="card mb-4">
            <div class="card-header bg-white">
                <div class="d-flex justify-content-between">
    ...
    <?php if($data->audit->is_external_redirected):?>
        <div class="card mb-4">
            <div class="card-header bg-white">
                <div class="d-flex justify-content-between">
            ...
            <div class="card-body">
                ...
                <div class="row mb-2">
                    ...
                    <div class="col">
                        <a href="<?= $data->audit->resolved_url ?>" target="_blank" rel="noreferrer">
                            <?= $data->audit->resolved_url ?>
                        </a>
                    </div>
                </div>
    ...
    <?php if(!$data->audit->is_queued && !$data->audit->is_external_redirected && !$data->audit->refresh_error): ?>
        ...
        <?php if(settings()->audits->ai_is_enabled && ($data->audit->ai_summary || $data->audit->ai_summary_id)): ?>
            <div class="card mb-4">
                <div class="card-header bg-white">
                    <div class="d-flex justify-content-between">
                ...
                <div class="card-body">
                    <?php if($data->audit->ai_summary): ?>
                        <?= $data->audit->ai_summary ?>
                    <?php else: ?>
                        <div id="ai_summary_processing">
                            <div class="small font-weight-bold mb-3 text-muted"><?= l('audits.ai_summary_processing') ?></div>
        ...
        <div class="card mb-4">
            <div class="card-header bg-white">
                <div class="d-flex justify-content-between">
                    <span class="small font-weight-bold"><?= sprintf(l('audits.total_issues_x'), $data->audit->total_issues) ?></span>
            ...
            <div class="card-body">
                ...
                <?php foreach(['major', 'moderate', 'minor'] as $issue_key): ?>
                    <?php foreach($data->audit->issues->{$issue_key} as $test => $issues): ?>
                        <?php foreach($issues as $issue): ?>
                            ...
                            <div class="row position-relative mb-2 py-1 audit-overview-issue rounded-2x flex-wrap ">
                                ...
                                <div class="col-auto">
                                    <a href="<?= url('audit/' . $data->audit->audit_id . '#' . $test) ?>" class="stretched-link"><i class="fas fa-fw fa-sm fa-link text-gray-900"></i></a>
                                </div>
                            </div>
                        <?php endforeach ?>
                    <?php endforeach ?>
            ...
            <div class="card-body">
                <div class="row align-items-center">
                    ...
                    <div class="col text-center text-md-left">
                        <p class="h6"><?= $data->audit->data->title ? e($data->audit->data->title) : l('audits.title_missing') ?></p>
        ...
        <div class="row mb-2">
            ...
            <div class="col-12 col-md-4 p-3 text-truncate">
                <div class="card d-flex flex-row h-100 overflow-hidden" data-toggle="tooltip" data-html="true" title="<?= sprintf(l('global.datetime_tooltip'), '<br />' . \Altum\Date::get($data->audit->datetime, 2) . '<br /><small>' . \Altum\Date::get($data->audit->datetime, 3) . '</small>' . '<br /><small>(' . \Altum\Date::get_timeago($data->audit->datetime) . ')</small>') ?>">
                    ...
                    <div class="card-body text-truncate">
                        <?= $data->audit->datetime ? \Altum\Date::get_timeago($data->audit->datetime) : l('global.na') ?>
                    </div>
                </div>
            </div>
        </div>
            ...
            <div class="card-body">
                ...
                <?php if(settings()->audits->available_tests->meta_robots): ?>
                    <div id="meta_robots" class="row">
                        ...
                        <div class="col-12 col-xl-8">
                            ...
                            <?php else: ?>
                                <div class="mb-3">
                                    <?php if (!empty($data->audit->data->meta_robots)): ?>
                            ...
                            <?php else: ?>
                                <div class="mb-3">
                                    <button class="btn btn-sm btn-block btn-gray-200 mb-2" type="button" data-toggle="collapse" data-target="#opengraph_container" aria-expanded="false" aria-controls="opengraph_container" <?= count((array) $data->audit->data->opengraph) ? null : 'disabled="disabled"' ?>>
                                        <?= sprintf(l('audits.test.opengraph_count'), '<strong>' . count((array) $data->audit->data->opengraph) . '</strong>') ?>
                                    </button>
                ...
                <?php if(settings()->audits->available_tests->schemas): ?>
                    <div id="schemas" class="row">
                        <div class="col-12 col-xl-4 mb-2 mb-xl-0">
                            <div>
                                <?= get_audit_test_icon('schemas', $data->audit->issues) ?>
                        ...
                        <div class="col-12 col-xl-8">
                            ...
                            <?php else: ?>
                                <div class="mb-3">
                                    <button class="btn btn-sm btn-block btn-gray-200 mb-2" type="button" data-toggle="collapse" data-target="#schemas_container" aria-expanded="false" aria-controls="schemas_container" <?= count((array) $data->audit->data->schemas) ? null : 'disabled="disabled"' ?>>
                                        <?= sprintf(l('audits.test.schemas_count'), '<strong>' . count((array) $data->audit->data->schemas) . '</strong>') ?>
                                    </button>

                                    <div class="collapse" id="schemas_container">
                                        <div class="card card-body">
                                            <ol class="mb-0 pl-3 audit-ol">
                                                <?php foreach ($data->audit->data->schemas as $schema): ?>
                                                    <li class="mb-3">
                                ...
                                <?php if(in_array('missing', $data->audit->issues->minor->schemas ?? [])): ?>
                                    <div class="mb-3 audit-issue-wrapper audit-issue-minor">
                                ...
                                <?php if(in_array('invalid', $data->audit->issues->minor->schemas ?? [])): ?>
                                    <div class="mb-3 audit-issue-wrapper audit-issue-minor">
                                ...
                                <div class="mb-3">
                                    <?php foreach(['h2', 'h3', 'h4', 'h5', 'h6'] as $heading_type): ?>
                                        <button class="btn btn-sm btn-block btn-gray-200 mb-2" type="button" data-toggle="collapse" data-target="#other_headings_container_<?= $heading_type ?>" aria-expanded="false" aria-controls="other_headings_container_<?= $heading_type ?>" <?= count($data->audit->data->headings->{$heading_type}) ? null : 'disabled="disabled"' ?>>
                                            <?= sprintf(l('audits.test.other_headings_count'), '<strong>' . count($data->audit->data->headings->{$heading_type}) . '</strong>', $heading_type) ?>
                                        </button>
                                ...
                                <?php if(!empty($data->audit->data->top_words)): ?>
                                    <div class="mb-3">
                                        <button class="btn btn-sm btn-block btn-gray-200 mb-2" type="button" data-toggle="collapse" data-target="#words_used_container" aria-expanded="false" aria-controls="words_used_container">
                                            <?= sprintf(l('audits.test.words_used_count'), '<strong>' . count((array) $data->audit->data->top_words) . '</strong>') ?>
                                        </button>
                                ...
                                <?php if(count((array) $data->audit->data->deprecated_html_tags)): ?>
                                    <div class="mb-3">
                                        <button class="btn btn-sm btn-block btn-gray-200 mb-2" type="button" data-toggle="collapse" data-target="#deprecated_html_tags_container" aria-expanded="false" aria-controls="deprecated_html_tags_container">
                                            <?= sprintf(l('audits.test.deprecated_html_tags_count'), '<strong>' . count((array) $data->audit->data->deprecated_html_tags) . '</strong>') ?>
                                        </button>
                                ...
                                <div class="mb-3">
                                    <?php if(count((array) $data->audit->data->inline_css)): ?>
                                        <button class="btn btn-sm btn-block btn-gray-200 mb-2" type="button" data-toggle="collapse" data-target="#inline_css_container" aria-expanded="false" aria-controls="inline_css_container">
                                            <?= sprintf(l('audits.test.inline_css_count'), '<strong>' . count((array) $data->audit->data->inline_css) . '</strong>') ?>
                                        </button>
                                ...
                                <div class="mb-3">
                                    <?php if(count((array) $data->audit->data->emails)): ?>
                                        <button class="btn btn-sm btn-block btn-gray-200 mb-2" type="button" data-toggle="collapse" data-target="#emails_container" aria-expanded="false" aria-controls="emails_container">
                                            <?= sprintf(l('audits.test.emails_count'), '<strong>' . count((array) $data->audit->data->emails) . '</strong>') ?>
                                        </button>
            ...
            <div class="card-body">
                ...
                <?php if(settings()->audits->available_tests->meta_robots_images): ?>
                    <div id="meta_robots_images" class="row">
                        <div class="col-12 col-xl-4 mb-2 mb-xl-0">
                            <div>
                                <?= get_audit_test_icon('meta_robots_images', $data->audit->issues) ?>
                        ...
                        <div class="col-12 col-xl-8">
                            ...
                            <?php else: ?>
                                <div class="mb-3">
                                    <?php if (!empty($data->audit->data->meta_robots_images)): ?>
                                        <p class="small font-weight-bold m-0"><?= e(implode(', ', $data->audit->data->meta_robots_images)) ?></p>
                                    <?php else: ?>
                                ...
                                <?php if(in_array('excluded', $data->audit->issues->moderate->meta_robots_images ?? [])): ?>
                                    <div class="mb-3 audit-issue-wrapper audit-issue-moderate">
                ...
                <?php if(settings()->audits->available_tests->header_robots_images): ?>
                    <div id="header_robots_images" class="row">
                        <div class="col-12 col-xl-4 mb-2 mb-xl-0">
                            <div>
                                <?= get_audit_test_icon('header_robots_images', $data->audit->issues) ?>
                        ...
                        <div class="col-12 col-xl-8">
                            ...
                            <?php else: ?>
                                <div class="mb-3">
                                    <p class="small font-weight-bold m-0"><?= $data->audit->data->response_headers->x_robots_tag ? e($data->audit->data->response_headers->x_robots_tag) : l('audits.test.header_robots_images_missing') ?></p>

                                    <small class="text-muted"><?= l('audits.test.header_robots_images_help') ?></small>
                                </div>

                                <?php if(in_array('excluded', $data->audit->issues->moderate->header_robots_images ?? [])): ?>
                                    <div class="mb-3 audit-issue-wrapper audit-issue-moderate">
            ...
            <div class="card-body">
                ...
                <?php if(settings()->audits->available_tests->not_found): ?>
                    <div id="not_found" class="row">
                        ...
                        <div class="col-12 col-xl-8">
                            ...
                            <?php else: ?>
                                <div class="mb-3">
                                    <p class="small m-0"><?= remove_url_protocol_from_url($data->audit->data->not_found_url) ?> - <?= $data->audit->data->not_found_status_code ?></p>
                    ...
                    <div id="page_size" class="row">
                        ...
                        <div class="col-12 col-xl-8">
                            ...
                            <?php else: ?>
                                ...
                                <?php if(in_array('too_big_major', $data->audit->issues->major->page_size ?? [])): ?>
                                    <div class="mb-3 audit-issue-wrapper audit-issue-major">
                                ...
                                <?php if($data->audit->data->non_deferred_scripts_count): ?>
                                    <div class="mb-3">
                                        <button class="btn btn-sm btn-block btn-gray-200 mb-2" type="button" data-toggle="collapse" data-target="#non_deferred_scripts_container" aria-expanded="false" aria-controls="non_deferred_scripts_container">
                                            <?= sprintf(l('audits.test.non_deferred_scripts_count'), '<strong>' . $data->audit->data->non_deferred_scripts_count . '</strong>') ?>
                                        </button>
                        ...
                        <div class="col-12 col-xl-8">
                            ...
                            <?php else: ?>
                                ...
                                <button class="btn btn-sm btn-block btn-gray-200 mb-2" type="button" data-toggle="collapse" data-target="#http_requests_container_css" aria-expanded="false" aria-controls="http_requests_container_css" <?= $data->audit->data->http_requests_data->css ? null : 'disabled="disabled"' ?>>
                                    <?= sprintf(l('audits.test.http_requests_css'), '<strong>' . $data->audit->data->http_requests_data->css . '</strong>') ?>
                                </button>
                                ...
                                <button class="btn btn-sm btn-block btn-gray-200 mb-2" type="button" data-toggle="collapse" data-target="#http_requests_container_js" aria-expanded="false" aria-controls="http_requests_container_js" <?= $data->audit->data->http_requests_data->js ? null : 'disabled="disabled"' ?>>
                                    <?= sprintf(l('audits.test.http_requests_js'), '<strong>' . $data->audit->data->http_requests_data->js . '</strong>') ?>
                                </button>
                                ...
                                <button class="btn btn-sm btn-block btn-gray-200 mb-2" type="button" data-toggle="collapse" data-target="#http_requests_container_images" aria-expanded="false" aria-controls="http_requests_container_images" <?= $data->audit->data->http_requests_data->images ? null : 'disabled="disabled"' ?>>
                                    <?= sprintf(l('audits.test.http_requests_images'), '<strong>' . $data->audit->data->http_requests_data->images . '</strong>') ?>
                                </button>
                                ...
                                <?php foreach(['audios', 'videos', 'iframes'] as $http_request_type): ?>
                                    <button class="btn btn-sm btn-block btn-gray-200 mb-2" type="button" data-toggle="collapse" data-target="#<?= 'http_requests_container_' . $http_request_type ?>" aria-expanded="false" aria-controls="<?= 'http_requests_container_' . $http_request_type ?>" <?= $data->audit->data->http_requests_data->{$http_request_type} ? null : 'disabled="disabled"' ?>>
                                        <?= sprintf(l('audits.test.http_requests_' . $http_request_type), '<strong>' . $data->audit->data->http_requests_data->{$http_request_type} . '</strong>') ?>
                                    </button>
                ...
                <?php if(settings()->audits->available_tests->safe_browsing && !empty(settings()->audits->google_safe_browsing_api_key)): ?>
                    <div id="safe_browsing" class="row">
                        <div class="col-12 col-xl-4 mb-2 mb-xl-0">
                            <div>
                                <?= get_audit_test_icon('safe_browsing', $data->audit->issues) ?>
                        ...
                        <div class="col-12 col-xl-8">
                            ...
                            <?php else: ?>
                                <div class="mb-3">
                                    <p class="small m-0"><strong><?= empty($data->audit->data->threats) ? l('audits.test.safe_browsing.true') : sprintf(l('audits.test.safe_browsing.false.threats'), implode(',', $data->audit->data->threats))  ?></strong></p>
                                    <small class="text-muted"><?= l('audits.test.safe_browsing_help') ?></small>
                                </div>

                                <?php if(in_array('false', $data->audit->issues->major->safe_browsing ?? [])): ?>
                                    <div class="mb-3 audit-issue-wrapper audit-issue-major">
                ...
                <?php if(settings()->audits->available_tests->hsts): ?>
                    <div id="hsts" class="row">
                        <div class="col-12 col-xl-4 mb-2 mb-xl-0">
                            <div>
                                <?= get_audit_test_icon('hsts', $data->audit->issues) ?>
                        ...
                        <div class="col-12 col-xl-8">
                            ...
                            <?php else: ?>
                                <div class="mb-3">
                                    <p class="small m-0"><strong><?= $data->audit->data->hsts ?: l('global.none') ?></strong></p>
                                    <small class="text-muted"><?= l('audits.test.hsts_help') ?></small>
                                </div>

                                <?php if(in_array('missing', $data->audit->issues->moderate->hsts ?? [])): ?>
                                    <div class="mb-3 audit-issue-wrapper audit-issue-moderate">
                                ...
                                <?php if(in_array('invalid', $data->audit->issues->moderate->hsts ?? [])): ?>
                                    <div class="mb-3 audit-issue-wrapper audit-issue-moderate">
                ...
                <?php if(settings()->audits->available_tests->spf): ?>
                    <div id="spf" class="row">
                        <div class="col-12 col-xl-4 mb-2 mb-xl-0">
                            <div>
                                <?= get_audit_test_icon('spf', $data->audit->issues) ?>
                        ...
                        <div class="col-12 col-xl-8">
                            ...
                            <?php else: ?>
                                <div class="mb-3">
                                    <p class="small m-0"><strong><?= !empty($data->audit->data->spf_records) ? '<code>' . implode('</code><code>', $data->audit->data->spf_records) . '</code>' : l('global.none') ?></strong></p>
                                    <small class="text-muted"><?= l('audits.test.spf_help') ?></small>
                                </div>

                                <?php if(in_array('missing', $data->audit->issues->moderate->spf ?? [])): ?>
                                    <div class="mb-3 audit-issue-wrapper audit-issue-moderate">
                                ...
                                <?php if(in_array('too_many', $data->audit->issues->moderate->spf ?? [])): ?>
                                    <div class="mb-3 audit-issue-wrapper audit-issue-moderate">
                                ...
                                <?php if(in_array('invalid', $data->audit->issues->moderate->spf ?? [])): ?>
                                    <div class="mb-3 audit-issue-wrapper audit-issue-moderate">
                ...
                <?php if(settings()->audits->available_tests->csp): ?>
                    <div id="csp" class="row">
                        <div class="col-12 col-xl-4 mb-2 mb-xl-0">
                            <div>
                                <?= get_audit_test_icon('csp', $data->audit->issues) ?>
                        ...
                        <div class="col-12 col-xl-8">
                            ...
                            <?php else: ?>
                                <div class="mb-3">
                                    <p class="small m-0"><strong><?= !empty($data->audit->data->csp_headers) ? '<code>' . implode('</code><code>', $data->audit->data->csp_headers) . '</code>' : l('global.none') ?></strong></p>
                                    <small class="text-muted"><?= l('audits.test.csp_help') ?></small>
                                </div>

                                <?php if(in_array('missing', $data->audit->issues->moderate->csp ?? [])): ?>
                                    <div class="mb-3 audit-issue-wrapper audit-issue-moderate">
                ...
                <?php if(settings()->audits->available_tests->referrer_policy): ?>
                    <div id="referrer_policy" class="row">
                        <div class="col-12 col-xl-4 mb-2 mb-xl-0">
                            <div>
                                <?= get_audit_test_icon('referrer_policy', $data->audit->issues) ?>
                        ...
                        <div class="col-12 col-xl-8">
                            ...
                            <?php else: ?>
                                <div class="mb-3">
                                    <p class="small m-0"><strong><?= $data->audit->data->response_headers->referrer_policy ? '<code>' . $data->audit->data->response_headers->referrer_policy . '</code>' : l('global.none') ?></strong></p>
                                    <small class="text-muted"><?= l('audits.test.referrer_policy_help') ?></small>
                                </div>

                                <?php if(in_array('missing', $data->audit->issues->minor->referrer_policy ?? [])): ?>
                                    <div class="mb-3 audit-issue-wrapper audit-issue-minor">
                                ...
                                <?php if(in_array('unsafe', $data->audit->issues->minor->referrer_policy ?? [])): ?>
                                    <div class="mb-3 audit-issue-wrapper audit-issue-minor">
                ...
                <?php if(settings()->audits->available_tests->meta_refresh): ?>
                    <div id="meta_refresh" class="row">
                        <div class="col-12 col-xl-4 mb-2 mb-xl-0">
                            <div>
                                <?= get_audit_test_icon('meta_refresh', $data->audit->issues) ?>
                        ...
                        <div class="col-12 col-xl-8">
                            ...
                            <?php else: ?>
                                <div class="mb-3">
                                    <p class="small m-0"><strong><?= $data->audit->data->meta_refresh ?: l('global.none') ?></strong></p>
                                    <small class="text-muted"><?= l('audits.test.meta_refresh_help') ?></small>
                                </div>

                                <?php if(in_array('existing', $data->audit->issues->minor->meta_refresh ?? [])): ?>
                                    <div class="mb-3 audit-issue-wrapper audit-issue-moderate">
            ...
            <div class="card-body">
                                ...
                                <?php if($data->audit->data->unsafe_external_links_count): ?>
                                    <div class="mb-3">
                                        <button class="btn btn-sm btn-block btn-gray-200 mb-2" type="button" data-toggle="collapse" data-target="#unsafe_external_links_container" aria-expanded="false" aria-controls="unsafe_external_links_container">
                                            <?= sprintf(l('audits.test.unsafe_external_links_count'), '<strong>' . $data->audit->data->unsafe_external_links_count . '</strong>') ?>
                                        </button>
                            ...
                            <?php else: ?>
                                <div class="mb-3">
                                    <button class="btn btn-sm btn-block btn-gray-200 mb-2" type="button" data-toggle="collapse" data-target="#external_links_container" aria-expanded="false" aria-controls="external_links_container" <?= $data->audit->data->external_links_count ? null : 'disabled="disabled"' ?>>
                                        <?= sprintf(l('audits.test.external_links_count'), '<strong>' . $data->audit->data->external_links_count . '</strong>') ?>
                                    </button>
                            ...
                            <?php else: ?>
                                <div class="mb-3">
                                    <button class="btn btn-sm btn-block btn-gray-200 mb-2" type="button" data-toggle="collapse" data-target="#internal_links_container" aria-expanded="false" aria-controls="internal_links_container" <?= $data->audit->data->internal_links_count ? null : 'disabled="disabled"' ?>>
                                        <?= sprintf(l('audits.test.internal_links_count'), '<strong>' . $data->audit->data->internal_links_count . '</strong>') ?>
                                    </button>
                                ...
                                <?php if(!empty($data->audit->data->social_links)): ?>
                                    <div class="mb-3">
                                        <button class="btn btn-sm btn-block btn-gray-200 mb-2" type="button" data-toggle="collapse" data-target="#social_links_container" aria-expanded="false" aria-controls="social_links_container">
                                            <?= sprintf(l('audits.test.social_links_count'), '<strong>' . count((array) $data->audit->data->social_links) . '</strong>') ?>
                                        </button>
                ...
                <?php if(settings()->audits->available_tests->unsafe_forms): ?>
                    <div id="unsafe_forms" class="row">
                        <div class="col-12 col-xl-4 mb-2 mb-xl-0">
                            <div>
                                <?= get_audit_test_icon('unsafe_forms', $data->audit->issues) ?>
                        ...
                        <div class="col-12 col-xl-8">
                            ...
                            <?php else: ?>
                                <?php if(count($data->audit->data->unsafe_forms ?? [])): ?>
                                    <div class="mb-3">
                                        <button class="btn btn-sm btn-block btn-gray-200 mb-2" type="button" data-toggle="collapse" data-target="#unsafe_forms_container" aria-expanded="false" aria-controls="unsafe_forms_container">
                                            <?= sprintf(l('audits.test.unsafe_forms_count'), '<strong>' . count($data->audit->data->unsafe_forms) . '</strong>') ?>
                                        </button>

                                        <div class="collapse" id="unsafe_forms_container">
                                            <div class="card card-body">
                                                <?php foreach ($data->audit->data->unsafe_forms as $form): ?>
                                                    <div class="text-truncate mb-3">
                                                        <p class="m-0 font-size-small">
                ...
                <div class="d-flex flex-column" style="max-width: 650px;">
                    <div class="d-flex align-items-center">
                        <div class="audit-search-preview-google-img-wrapper">
                            <img referrerpolicy="no-referrer" src="<?= get_favicon_url_from_domain($data->audit->host) ?>" class="audit-search-preview-google-img" loading="lazy" />
                        </div>

                        <div class="d-flex flex-column">
                            <div class="audit-search-preview-google-host"><?= $data->audit->opengraph->site_name ?? $data->audit->host ?></div>
                            <div class="audit-search-preview-google-url"><?= $data->audit->url ?></div>
                        </div>
                    </div>

                    <h3 class="audit-search-preview-google-title"><?= $data->audit->data->title ? e($data->audit->data->title) : l('audits.title_missing') ?></h3>

                    <div class="audit-search-preview-google-description">
                        <?= $data->audit->data->meta_description ? e($data->audit->data->meta_description) : l('audits.meta_description_missing') ?>
                    </div>
                </div>
            </div>
        </div>
                ...
                <div class="d-flex flex-column" style="max-width: 600px;">
                    <div class="d-flex align-items-center">
                        <div class="audit-search-preview-bing-img-wrapper">
                            <img referrerpolicy="no-referrer" src="<?= get_favicon_url_from_domain($data->audit->host) ?>" class="audit-search-preview-bing-img" loading="lazy" />
                        </div>

                        <div class="d-flex flex-column">
                            <div class="audit-search-preview-bing-host"><?= $data->audit->opengraph->site_name ?? $data->audit->host ?></div>
                            <div class="audit-search-preview-bing-url"><?= $data->audit->url ?></div>
                        </div>
                    </div>

                    <h3 class="audit-search-preview-bing-title"><?= $data->audit->data->title ? e($data->audit->data->title) : l('audits.title_missing') ?></h3>
        ...
        <div class="card">
            ...
            <div class="card-body audit-search-preview-yandex-wrapper">
                <div class="d-flex" style="max-width: 600px;">
                    <img referrerpolicy="no-referrer" src="<?= get_favicon_url_from_domain($data->audit->host) ?>" class="audit-search-preview-yandex-img" loading="lazy" />

                    <div class="d-flex flex-column audit-search-preview-yandex-result-wrapper">
                        <h3 class="audit-search-preview-yandex-title"><?= $data->audit->data->title ? e($data->audit->data->title) : l('audits.title_missing') ?></h3>
                        <div class="audit-search-preview-yandex-url"><?= remove_url_protocol_from_url($data->audit->url) ?></div>
...
<?php if(settings()->audits->ai_is_enabled && settings()->audits->openai_api_key && empty($data->audit->ai_summary)): ?>
    <?php ob_start() ?>
    <style>
        #ai_summary_content {
            opacity: 0;
    ...
    <script>
        ...
        let audit_id = <?= (int) $data->audit->audit_id ?>;

        let interval = setInterval(async () => {
            /* Send request to server */
            let response = await fetch(`${url}audit/get_ai_summary/${audit_id}`, {
                method: 'get',
            });
            ...
            if(data.status === 'success' && data.message) {
                let summary_block = document.querySelector('#ai_summary_content');

                summary_block.innerHTML = data.message;
                summary_block.classList.add('is_visible');
                document.querySelector('#ai_summary_processing').remove();
