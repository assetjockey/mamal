...
<div class="container">
    ...
    <?php if(settings()->main->breadcrumbs_is_enabled): ?>
        <nav aria-label="breadcrumb">
            <ol class="custom-breadcrumbs small">
                ...
                <li class="active" aria-current="page"><?= l('tools.image_alt_tags_checker.name') ?></li>
            </ol>
        </nav>
    <?php endif ?>
    ...
    <div class="row mb-4">
        <div class="col-12 col-lg d-flex align-items-center mb-3 mb-lg-0 text-truncate">
            <h1 class="h4 m-0 text-truncate"><?= l('tools.image_alt_tags_checker.name') ?></h1>

            <div class="ml-2">
                <span data-toggle="tooltip" title="<?= l('tools.image_alt_tags_checker.description') ?>">
            ...
            <div class="table-responsive table-custom-container">
                <table class="table table-custom">
                    ...
                    <tr>
                        ...
                        <td class="text-nowrap">
                            <?php if($data->result['images_count'] == $data->result['images_alt_valid_count']): ?>
                                <i class="fas fa-fw fa-sm fa-check-circle text-success mr-1"></i> <?= l('tools.image_alt_tags_checker.result.all') ?>
                            <?php else: ?>
                                <i class="fas fa-fw fa-sm fa-exclamation-circle text-warning mr-1"></i> <?= sprintf(l('tools.image_alt_tags_checker.result.some'), nr($data->result['images_count'] - $data->result['images_alt_valid_count']), nr($data->result['images_count'])) ?>
                            <?php endif ?>
                        </td>
                    </tr>
        ...
        <div class="mt-4">
            <div class="table-responsive table-custom-container">
                <table class="table table-custom">
                    ...
                    <tr>
                        <td class="font-weight-bold">
                            <?= l('tools.image_alt_tags_checker.result.images') ?>
                        </td>
                        <td class="text-nowrap">
                            <span class="badge badge-light"><?= nr($data->result['images_count']) ?></span>
                        </td>
                    </tr>
                    ...
                    <?php foreach($data->result['images'] as $image): ?>
                        <tr>
                            <td class="text-nowrap">
                                ...
                                <a href="<?= e($image['src']) ?>" target="_blank" rel="nofollow noreferrer" class="text-truncate">
                                    <?= string_truncate(remove_url_protocol_from_url(e($image['src'])), 64) ?>
                                </a>
                            </td>

                            <td class="text-nowrap">
                                <?php if($image['alt']): ?>
                                    <i class="fas fa-fw fa-sm fa-check-circle text-success mr-1"></i> <span title="<?= e($image['alt']) ?>"><?= string_truncate(e($image['alt']), 64) ?></span>
                                <?php else: ?>
                                    <i class="fas fa-fw fa-sm fa-times-circle text-danger mr-1"></i> <?= l('tools.image_alt_tags_checker.error_message.missing_alt_tag') ?>
                                <?php endif ?>
                            </td>
                        </tr>
                    <?php endforeach ?>
