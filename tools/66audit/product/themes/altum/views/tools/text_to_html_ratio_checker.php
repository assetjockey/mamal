...
<div class="container">
    ...
    <?php if(settings()->main->breadcrumbs_is_enabled): ?>
        <nav aria-label="breadcrumb">
            <ol class="custom-breadcrumbs small">
                ...
                <li class="active" aria-current="page"><?= l('tools.text_to_html_ratio_checker.name') ?></li>
            </ol>
        </nav>
    <?php endif ?>
    ...
    <div class="row mb-4">
        <div class="col-12 col-lg d-flex align-items-center mb-3 mb-lg-0 text-truncate">
            <h1 class="h4 m-0 text-truncate"><?= l('tools.text_to_html_ratio_checker.name') ?></h1>

            <div class="ml-2">
                <span data-toggle="tooltip" title="<?= l('tools.text_to_html_ratio_checker.description') ?>">
        ...
        <div class="mt-4">
            <div class="table-responsive table-custom-container">
                <table class="table table-custom">
                    ...
                    <tr>
                        <td class="font-weight-bold">
                            <?= l('tools.text_to_html_ratio_checker.result.text') ?>
                        </td>
                        <td class="text-nowrap">
                            <?= sprintf(l('tools.text_to_html_ratio_checker.result.characters'), nr($data->result['text'])) ?>
                        </td>
                    </tr>

                    <tr>
                        <td class="font-weight-bold">
                            <?= l('tools.text_to_html_ratio_checker.result.html') ?>
                        </td>
                        <td class="text-nowrap">
                            <?= sprintf(l('tools.text_to_html_ratio_checker.result.characters'), nr($data->result['html'])) ?>
                        </td>
                    </tr>

                    <tr>
                        <td class="font-weight-bold">
                            <?= l('tools.text_to_html_ratio_checker.result') ?>
                        </td>
                        <td class="text-nowrap">
                            <?= nr($data->result['text_to_html_ratio']) . '%' ?>
                        </td>
                    </tr>
