...
<div class="container">
    ...
    <?php if(settings()->main->breadcrumbs_is_enabled): ?>
        <nav aria-label="breadcrumb">
            <ol class="custom-breadcrumbs small">
                ...
                <li class="active" aria-current="page"><?= l('tools.hsts_checker.name') ?></li>
            </ol>
        </nav>
    <?php endif ?>
    ...
    <div class="row mb-4">
        <div class="col-12 col-lg d-flex align-items-center mb-3 mb-lg-0 text-truncate">
            <h1 class="h4 m-0 text-truncate"><?= l('tools.hsts_checker.name') ?></h1>

            <div class="ml-2">
                <span data-toggle="tooltip" title="<?= l('tools.hsts_checker.description') ?>">
        ...
        <div class="mt-4">
            <div class="table-responsive table-custom-container">
                <table class="table table-custom">
                    ...
                    <tr>
                        <td class="font-weight-bold">
                            <?= l('tools.hsts_checker.result.hsts') ?>
                        </td>
                        <td class="text-nowrap">
                            <?= $data->result['hsts'] ?: l('global.none') ?>
                        </td>
                    </tr>

                    <?php if($data->result['hsts']): ?>
                        <tr>
                            <td class="font-weight-bold">
                                <?= l('tools.hsts_checker.result.valid') ?>
                            </td>
                            <td class="text-nowrap">
                                <?php if($data->result['is_valid']): ?>
                                    <i class="fas fa-fw fa-sm fa-check-circle text-success mr-1"></i> <?= l('tools.hsts_checker.result.valid') ?>
                                <?php else: ?>
                                    <i class="fas fa-fw fa-sm fa-times-circle text-danger mr-1"></i> <?= l('tools.hsts_checker.result.invalid') ?>
                                <?php endif ?>
                            </td>
                        </tr>
                    <?php endif ?>
