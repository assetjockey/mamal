...
class Monitor extends Controller {
    ...
    public function index() {
        ...
        if($this->status_page->theme == 'new-york') {
            $monitor_logs_processed = (new \Altum\Models\MonitorsLogs())->get_monitor_logs_chart_data_by_monitor_id($this->monitor->monitor_id, $date->start_date_query, $date->end_date_query);
        } elseif($this->status_page->theme == 'phoenix') {
            $monitor_logs_processed = (new \Altum\Models\MonitorsLogs())->get_monitor_logs_data_by_monitor_id($this->monitor->monitor_id, $date->start_date_query, $date->end_date_query);
        }
        ...
        $monitor_logs_chart = $monitor_logs_processed['chart'] ?? null;
        $monitor_logs = $monitor_logs_processed['logs'] ?? null;
