...
class GameServerUpdate extends Controller {
    ...
    public function index() {
        ...
        /* Team checks */
        if(\Altum\Teams::is_delegated() && !\Altum\Teams::has_access('update.game_servers')) {
        ...
        if(!empty($_POST)) {
            ...
            if(!Alerts::has_field_errors() && !Alerts::has_errors()) {
                ...
                if((new \DateTime($game_server->next_check_datetime)) > (new \DateTime())) {
                    $next_check_datetime = (new \DateTime())->modify('+' . $_POST['check_interval_seconds'] . ' seconds')->format('Y-m-d H:i:s');
                }

                /* Database query */
                $game_server_id = db()->where('game_server_id', $game_server->game_server_id)->update('game_servers', [
                ...
                redirect('game-server-update/' . $game_server->game_server_id);
            }

        }

        /* Set a custom title */
        Title::set(sprintf(l('game_server_update.title'), $game_server->name));
        ...
        $view = new \Altum\View('game-server-update/index', (array) $this);
