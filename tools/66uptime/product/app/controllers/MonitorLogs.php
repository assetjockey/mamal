...
class MonitorLogs extends Controller {
    ...
    public function index() {
        ...
        $filters = (new \Altum\Filters(['is_ok', 'ping_server_id'], ['response_status_code'], ['monitor_log_id', 'response_time', 'datetime'], allowed_datetime_fields: ['datetime']));
        ...
        $paginator = (new \Altum\Paginator($total_rows, $filters->get_results_per_page(), $_GET['page'] ?? 1, url('monitor-logs/' . $monitor->monitor_id . '?' . $filters->get_get() . '&start_date=' . $start_date . '&end_date=' . $end_date . '&page={{PAGE}}')));
