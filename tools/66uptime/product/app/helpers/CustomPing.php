...
class CustomPing {
    ...
    private function pingExec() {
        ...
        /* handle failed ping command */
        if($return !== 0) {
            $this->error_type = 'ping_failed';
            return false;
        }
        ...
        if(!empty($output[1])) {
            ...
            if($response > 0 && isset($matches['time'])) {
                ...
                /* could not parse the ping response */
                $this->error_type = 'parse_error';
            }
        }
    ...
    private function pingFsockopen() {
        ...
        $fp = @fsockopen($this->host, $this->port, $error_number, $error_string, $this->timeout);

        if(!$fp) {
            /* fsockopen failed, set error details */
            $this->error_type = 'connection_failed';
            $this->error_message = "{$error_string} ({$error_number})";
            return false;
        }
    ...
    private function pingSocket() {
        ...
        if($socket = socket_create(AF_INET, SOCK_RAW, 1)) {
            ...
            if(@socket_connect($socket, $this->host, null) === false) {
                /* socket connection failed */
                $this->error_type = 'socket_connect_failed';
                $this->error_message = 'Socket connect failed.';
                return false;
            }
            ...
            if(@socket_read($socket, 255) !== false) {
                ...
                /* socket received no reply */
                $this->error_type = 'socket_no_response';
                $this->error_message = 'No reply received via socket.';
                socket_close($socket);
                return false;
            }
            ...
            $this->error_type = 'socket_create_failed';
            $this->error_message = 'Unable to create socket.';
            return false;
        }
    }
