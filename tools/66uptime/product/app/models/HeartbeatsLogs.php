...
class HeartbeatsLogs extends Model {
    ...
    public function get_heartbeat_logs_chart_data_by_heartbeat_id($heartbeat_id, $start_datetime, $end_datetime) {
        ...
        $cache_instance = cache()->getItem('s_heartbeat_logs_data?heartbeat_id=' . $heartbeat_id . '&start_datetime=' . md5($start_datetime) . '&end_datetime=' . md5($end_datetime));
        ...
        if(is_null($cache_instance->get())) {
            ...
            $data['chart'] = get_chart_data($heartbeat_logs_chart);

            cache()->save(
                $cache_instance->set($data)->expiresAfter(CACHE_DEFAULT_SECONDS)->addTag('heartbeat_id=' . $heartbeat_id)
            );
    ...
    public function get_heartbeat_logs_data_by_heartbeat_id($heartbeat_id, $start_datetime, $end_datetime) {
