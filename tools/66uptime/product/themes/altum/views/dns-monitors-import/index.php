    ...
    <?php if(settings()->main->breadcrumbs_is_enabled): ?>
        <nav aria-label="breadcrumb">
            <ol class="custom-breadcrumbs small">
                ...
                <li class="active" aria-current="page"><?= l('dns_monitors_import.breadcrumb') ?></li>
            </ol>
        </nav>
    <?php endif ?>

    <h1 class="h4 text-truncate"><i class="fas fa-fw fa-xs fa-plug mr-1"></i> <?= l('dns_monitors_import.header') ?></h1>
    <p></p>
        ...
        <div class="card-body">
            <form id="form" action="" method="post" role="form" enctype="multipart/form-data">
                ...
                <div class="form-group" data-file-input-wrapper-size-limit="<?= get_max_upload() ?>" data-file-input-wrapper-size-limit-error="<?= sprintf(l('global.error_message.file_size_limit'), get_max_upload()) ?>">
                    ...
                    <small class="form-text text-muted"><?= sprintf(l('global.csv_file_help'), '<code>name</code>, <code>target</code>', '<code>project_id</code>, <code>dns_check_interval_seconds</code>, <code>dns_types</code>, <code>notifications</code>, <code>is_enabled</code>'); ?></small>
                    <small class="form-text text-muted"><a href="<?= ASSETS_FULL_URL . 'csv/dns_monitors_example.csv' ?>" download="dns_monitors_example.csv" target="_blank"><?= l('global.csv_file_help2') ?></a></small>
