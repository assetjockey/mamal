    ...
    <?php if(settings()->main->breadcrumbs_is_enabled): ?>
        <nav aria-label="breadcrumb">
            <ol class="custom-breadcrumbs small">
                <li>
                    <a href="<?= url('game-servers') ?>"><?= l('game_servers.breadcrumb') ?></a><i class="fas fa-fw fa-angle-right"></i>
                </li>
                <li class="active" aria-current="page"><?= l('game_server_update.breadcrumb') ?></li>
            </ol>
        </nav>
    <?php endif ?>

    <div class="d-flex justify-content-between align-items-center mb-2">
        <h1 class="h4 text-truncate mb-0"><i class="fas fa-fw fa-xs fa-gamepad mr-1"></i> <?= sprintf(l('global.update_x'), $data->game_server->name) ?></h1>

        <?= include_view(THEME_PATH . 'views/game-server/game_server_dropdown_button.php', ['id' => $data->game_server->game_server_id, 'resource_name' => $data->game_server->name]) ?>
    </div>
    <p></p>
        ...
        <div class="card-body">
            ...
            <form action="" method="post" role="form">
                ...
                <div class="form-group">
                    ...
                    <input type="text" id="name" name="name" class="form-control <?= \Altum\Alerts::has_field_errors('name') ? 'is-invalid' : null ?>" value="<?= $data->game_server->name ?>" required="required" />
                ...
                <div class="row">
                    <div class="col-lg-6">
                        <div class="form-group" data-type="port">
                            ...
                            <input type="text" id="target" name="target" class="form-control <?= \Altum\Alerts::has_field_errors('target') ? 'is-invalid' : null ?>" value="<?= $data->game_server->target ?>" placeholder="<?= l('game_server.input.target_placeholder') ?>" required="required" />
                    ...
                    <div class="col-lg-3">
                        <div class="form-group" data-type="port">
                            ...
                            <input type="number" min="0" max="100000" id="port" name="port" class="form-control <?= \Altum\Alerts::has_field_errors('port') ? 'is-invalid' : null ?>" value="<?= $data->game_server->port ?>" placeholder="<?= l('game_server.input.port_placeholder') ?>" required="required" />
                    ...
                    <div class="col-lg-3">
                        <div class="form-group" data-type="port">
                            ...
                            <input type="number" min="0" max="100000" id="query_port" name="query_port" class="form-control <?= \Altum\Alerts::has_field_errors('query_port') ? 'is-invalid' : null ?>" value="<?= $data->game_server->query_port ?>" placeholder="<?= l('game_server.input.query_port_placeholder') ?>" required="required" />
                ...
                <div class="form-group">
                    ...
                    <select id="type" name="type" class="custom-select" required="required">
                        <?php foreach($data->game_server_types as $key => $value): ?>
                            <option value="<?= $key ?>" <?= $data->game_server->type == $key ? 'selected="selected"' : null ?> data-port="<?= $value['port'] ?>" data-query-port="<?= $value['query_port'] ?>"><?= $value['name'] ?></option>
                        <?php endforeach ?>
                    </select>
                </div>
                ...
                <div class="form-group">
                    ...
                    <select id="check_interval_seconds" name="check_interval_seconds" class="custom-select" required="required">
                        <?php foreach($data->game_server_check_intervals as $key => $value): ?>
                            <option value="<?= $key ?>" <?= $data->game_server->settings->check_interval_seconds == $key ? 'selected="selected"' : null ?> <?= !in_array($key, $this->user->plan_settings->game_servers_check_intervals ?? []) ? 'disabled="disabled"' : null ?>><?= $value ?></option>
                        <?php endforeach ?>
                    </select>
                ...
                <div class="form-group custom-control custom-switch">
                    <input id="is_enabled" name="is_enabled" type="checkbox" class="custom-control-input" <?= $data->game_server->is_enabled ? 'checked="checked"' : null?>>
                    <label class="custom-control-label" for="is_enabled"><?= l('game_server.input.is_enabled') ?></label>
                </div>
                    ...
                    <div class="form-group">
                        ...
                        <select id="timeout_seconds" name="timeout_seconds" class="custom-select" required="required">
                            <?php foreach($data->monitor_timeouts as $key => $value): ?>
                                <option value="<?= $key ?>" <?= $data->game_server->settings->timeout_seconds == $key ? 'selected="selected"' : null ?>><?= $value ?></option>
                            <?php endforeach ?>
                        </select>
                    </div>
                        ...
                        <div class="form-group">
                            ...
                            <select id="project_id" name="project_id" class="custom-select">
                                ...
                                <?php foreach($data->projects as $project_id => $project): ?>
                                    <option value="<?= $project_id ?>" <?= $data->game_server->project_id == $project_id ? 'selected="selected"' : null ?>><?= $project->name ?></option>
                                <?php endforeach ?>
                            </select>
