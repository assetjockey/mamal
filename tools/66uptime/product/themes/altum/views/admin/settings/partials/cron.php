...
<?php foreach(['reset', 'monitors', 'heartbeats', 'domain_names', 'dns_monitors', 'game_servers', 'monitors_email_reports', 'heartbeats_email_reports', 'monitors_logs_cleanup', 'broadcasts'] as $cron): ?>
    <div class="form-group">
        ...
        <div class="input-group">
            ...
            <?php if(in_array($cron, ['monitors', 'heartbeats', 'domain_names', 'dns_monitors', 'monitors_email_reports', 'heartbeats_email_reports'])): ?>
                <div class="input-group-append">
                    <a
                            href="<?= url('admin/settings/monitors_heartbeats') ?>"
                            class="btn btn-light"
                            data-toggle="tooltip"
...
<p class="text-muted small">
    This functionality runs exclusively via the <strong>command line</strong> and <strong>requires the pcntl extension</strong> to be enabled on your host.<br /><br />
    It executes monitor checks in parallel, allowing Cron jobs to finish faster and process more monitors in less time.
</p>
...
<div class="form-group">
    ...
    <small class="form-text text-muted"><?= sprintf(l('admin_settings.cron.last_execution'), isset(settings()->cron->monitors_datetime) ? \Altum\Date::get_timeago(settings()->cron->monitors_datetime) : l('global.na')) ?></small>
    <?php if(isset(settings()->cron->monitors_datetime)): ?>
        <small class="form-text text-muted"><?= sprintf(l('admin_settings.cron.last_processing_time'), isset(settings()->cron->monitors_datetime_processing) ? nr(settings()->cron->monitors_datetime_processing, 8) . ' ' . l('global.date.seconds') : l('global.na')) ?></small>
    <?php endif ?>
</div>
