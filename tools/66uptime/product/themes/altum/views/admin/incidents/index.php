...
<div class="d-flex flex-column flex-md-row justify-content-between mb-4">
    <h1 class="h3 mb-3 mb-md-0 text-truncate"><i class="fas fa-fw fa-xs fa-exclamation-circle text-primary-900 mr-2"></i> <?= l('admin_incidents.header') ?></h1>

    <div class="d-flex position-relative d-print-none">
        <div class="ml-3">
            <a href="<?= url('admin/statistics/incidents') ?>" class="btn btn-gray-300" data-tooltip title="<?= l('global.statistics') ?>">
        ...
        <div class="ml-3">
            <div class="dropdown">
                ...
                <div class="dropdown-menu dropdown-menu-right d-print-none">
                    <a href="<?= url('admin/incidents?' . $data->filters->get_get() . '&export=csv') ?>" target="_blank" class="dropdown-item <?= $this->user->plan_settings->export->csv ? null : 'disabled pointer-events-all' ?>" <?= $this->user->plan_settings->export->csv ? null : get_plan_feature_disabled_info() ?>>
                        <i class="fas fa-fw fa-sm fa-file-csv mr-2"></i> <?= sprintf(l('global.export_to'), 'CSV') ?>
                    </a>
                    <a href="<?= url('admin/incidents?' . $data->filters->get_get() . '&export=json') ?>" target="_blank" class="dropdown-item <?= $this->user->plan_settings->export->json ? null : 'disabled pointer-events-all' ?>" <?= $this->user->plan_settings->export->json ? null : get_plan_feature_disabled_info() ?>>
...
<form id="table" action="<?= SITE_URL . 'admin/incidents/bulk' ?>" method="post" role="form">
        ...
        <table class="table table-custom">
            ...
            <?php foreach($data->incidents as $row): ?>
                ...
                <tr>
                    ...
                    <td class="text-truncate text-muted">
                        <div>
                            <span><?= $row->monitor_name ?? $row->heartbeat_name ?></span>
                        </div>
                    ...
                    <td class="text-truncate">
                        <span id="incident_id_<?= $row->incident_id ?>" title="<?= $row->comment ?: l('global.none') ?>" data-toggle="tooltip">
                    ...
                    <td class="text-truncate">
                        <div class="d-flex justify-content-end">
                            <?= include_view(THEME_PATH . 'views/admin/incidents/admin_incident_dropdown_button.php', ['id' => $row->incident_id]) ?>
                        </div>
                    </td>
                </tr>
            <?php endforeach ?>
