                                ...
                                <div class="form-row">
                                    ...
                                    <div class="col-12 col-sm-auto mt-3 mt-sm-0">
                                        @if(config('settings.captcha_driver'))
                                            ...
                                            @include('shared.captcha', ['id' => 'report-form'])

                                            <x-captcha-button data-callback="{{ (config('settings.captcha_driver') == 'turnstile' ? '' : 'captchaFormSubmit') }}" form-id="report-form" class="btn btn-primary btn-lg btn-block font-size-lg" data-sitekey="{{ config('settings.captcha_site_key') }}" data-theme="{{ (config('settings.dark_mode') == 1 ? 'dark' : 'light') }}">{{ __('Analyze') }}</x-captcha-button>
                                        @else
