                                ...
                                @foreach($projects as $project)
                                    <div class="list-group-item px-0">
                                        <div class="row">
                                            <div class="d-none col-auto align-items-center" data-bulk-checkbox-column>
                                                <div class="custom-control custom-checkbox" data-bulk-check>
                                                    <input type="checkbox" class="custom-control-input" id="bulk-check-{{ $project->name }}" name="bulk[]" value="{{ $project->name }}" data-bulk-checkbox>
                                                    <label class="custom-control-label user-select-none" for="bulk-check-{{ $project->name }}"></label>
                                                </div>
                                            </div>
                                            <div class="col">
                                                <div class="row align-items-center">
                                                        ...
                                                        <div class="row">
                                                            <div class="col-12 col-md-8 col-lg-6 d-flex">
                                                                <div class="d-flex align-items-center text-truncate">
                                                                    <img src="{{ favicon($project->name) }}" rel="noreferrer" class="width-4 height-4 {{ (__('lang_dir') == 'rtl' ? 'ml-3' : 'mr-3') }}">

                                                                    <a href="{{ route('reports', ['project' => $project->name]) }}" dir="ltr" class="text-truncate">{{ $project->name }}</a>
                                                                </div>
                                                            </div>

                                                            <div class="d-none d-md-flex align-items-center col-md-4 col-lg-2">
                                                                <a href="{{ route('reports', ['project' => $project->name]) }}" class="badge {{ (($project->score / $project->reports) > 79 ? 'badge-success' : (($project->score / $project->reports) > 49 ? 'badge-warning' : 'badge-danger')) }} text-truncate">{{ (($project->score / $project->reports) > 79 ? __('Good') : (($project->score / $project->reports) > 49 ? __('Decent') : __('Bad'))) }}</a>
                                                            </div>

                                                            <div class="col-lg-2 d-none d-lg-block text-truncate">
                                                                <a href="{{ route('reports', ['project' => $project->name]) }}" class="text-dark text-truncate">{{ number_format($project->reports, 0, __('.'), __(',')) }}</a>
                                                            </div>

                                                            <div class="d-none d-lg-flex col-lg-2 text-truncate">
                                                                <span class="text-truncate" data-tooltip="true" title="{{ $project->created_at->tz(Auth::user()->timezone ?? config('settings.timezone'))->format(__('Y-m-d') . ' H:i:s') }}">{{ $project->created_at->diffForHumans() }}</span>
                                                            </div>
                                                        </div>
                                                    </div>
