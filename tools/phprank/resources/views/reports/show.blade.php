    ...
    <div class="d-none d-print-block border-bottom pb-3 mb-3">
        <div class="row">
            <div class="col">
                <div class="row no-gutters">
                    <div class="col-auto d-flex align-items-center">
                        <img src="{{ $report->user->brandLogoUrl }}" class="height-10 {{ (__('lang_dir') == 'rtl' ? 'ml-3' : 'mr-3') }}">
                    </div>
                    <div class="col d-flex align-items-center">
                        <div class="">
                            @if(isset($report->user->brand_information->name) && $report->user->brand_information->name)
                                <div class="font-weight-bold">{{ $report->user->brand_information->name }}</div>
                            @endif

                            @if(isset($report->user->brand_information->url) && $report->user->brand_information->url)
                                <div><a href="{{ $report->user->brand_information->url }}" rel="nofollow noreferrer noopener" target="_blank" class="text-decoration-none">{{ $report->user->brand_information->url }}</a></div>
                            @endif
                        </div>
                    </div>
                </div>
            ...
            <div class="col-auto d-flex align-items-center {{ (__('lang_dir') == 'rtl' ? 'text-left' : 'text-right') }}">
                <div>
                    @if(isset($report->user->brand_information->email) && $report->user->brand_information->email)
                        <div><a href="mailto:{{ $report->user->brand_information->email }}" class="text-secondary text-decoration-none">{{ $report->user->brand_information->email }}</a></div>
                    @endif

                    @if(isset($report->user->brand_information->phone) && $report->user->brand_information->phone)
                        <div><a href="tel:{{ $report->user->brand_information->phone }}" class="text-secondary text-decoration-none">{{ $report->user->brand_information->phone }}</a></div>
                    @endif
                </div>
            </div>
        </div>
        ...
        <div class="card-header align-items-center">
            <div class="row">
                ...
                <div class="col-auto d-flex align-items-center">
                    ...
                    <div class="d-print-none text-muted" data-tooltip="true" title="{{ $report->generated_at->tz(Auth::user()->timezone ?? config('settings.timezone'))->format(__('Y-m-d') . ' H:i:s') }}">{{ $report->generated_at->tz(Auth::user()->timezone ?? config('settings.timezone'))->diffForHumans() }}</div>
                </div>
            </div>
        </div>
                    ...
                    <div class="row">
                        <div class="col-12 col-lg-auto">
                            <div class="mx-auto mx-lg-0 position-relative d-flex align-items-center justify-content-center width-40 height-40 p-2">
                                <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 160 160" class="{{ ($report->score > 79 ? 'stroke-success' : ($report->score > 49 ? 'stroke-warning' : 'stroke-danger')) }}" width="144" height="144">
                                    <circle cx="80" cy="80" r="75" stroke="#ddd" stroke-width="5" fill="transparent"/>

                                    <path d="M80,5A75,75,0,1,1,5,80,75,75,0,0,1,80,5" stroke-linecap="round" stroke-width="10" fill="transparent" stroke-dasharray="{{ number_format(($report->score / 100) * (2*3.14*75)) }},{{ number_format((2*3.14*75)) }}"/>
                                </svg>
                                <div class="position-absolute">
                                    <div class="d-flex flex-column align-items-center">
                                        <div class="font-weight-bold h1 mb-0">
                                            {{ number_format($report->score) }}
                                        </div>
