                                                        ...
                                                        <div class="max-height-96 overflow-auto pt-3">
                                                            ...
                                                            <div class="form-group px-4">
                                                                ...
                                                                <select name="project" id="i-project" class="custom-select custom-select-sm">
                                                                    ...
                                                                    @foreach($projects as $project)
                                                                        <option value="{{ $project->name }}" @if(request()->input('project') == $project->name && request()->input('project') !== null) selected @endif>{{ $project->name }}</option>
                                                                    @endforeach
                                                                </select>
                                                            </div>

                                                            <div class="form-group px-4">
                                                                <label for="i-score" class="small">{{ __('Score') }}</label>
                                                                <select name="score" id="i-score" class="custom-select custom-select-sm">
                                                                    <option value="">{{ __('All') }}</option>
                                                                    @foreach(['good' => __('Good'), 'decent' => __('Decent'), 'bad' => __('Bad')] as $key => $value)
                                                                        <option value="{{ $key }}" @if(request()->input('score') == $key && request()->input('score') !== null) selected @endif>{{ $value }}</option>
                                                                    @endforeach
                                                                </select>
                                                            </div>
                                                            ...
                                                            <div class="form-group px-4">
                                                                ...
                                                                <select name="sort_by" id="i-sort-by" class="custom-select custom-select-sm">
                                                                    @foreach(['id' => __('Date created'), 'generated_at' => __('Date generated'), 'url' => __('URL'), 'score' => __('Score')] as $key => $value)
                                ...
                                @foreach($reports as $report)
                                    <div class="list-group-item px-0">
                                        <div class="row">
                                            <div class="d-none col-auto align-items-center" data-bulk-checkbox-column>
                                                <div class="custom-control custom-checkbox" data-bulk-check>
                                                    <input type="checkbox" class="custom-control-input" id="bulk-check-{{ $report->id }}" name="bulk[]" value="{{ $report->id }}" data-bulk-checkbox>
                                                    <label class="custom-control-label user-select-none" for="bulk-check-{{ $report->id }}"></label>
                                                </div>
                                            </div>
                                            <div class="col">
                                                <div class="row align-items-center">
                                                    <div class="col text-truncate">
                                                        <div class="row">
                                                                ...
                                                                <div class="row no-gutters w-100 align-items-center">
                                                                    <div class="col-6 d-none d-lg-flex">
                                                                        <div class="progress height-1.25 w-100">
                                                                            <div class="progress-bar rounded {{ ($report->score > 79 ? 'bg-success' : ($report->score > 49 ? 'bg-warning' : 'bg-danger')) }}" role="progressbar" style="width: {{ number_format($report->score) }}%" aria-valuenow="{{ number_format($report->score) }}" aria-valuemin="0" aria-valuemax="100"></div>
                                                                        </div>
                                                                    </div>
                                                                    <div class="col-3 d-none d-lg-flex small text-center px-2">
                                                                        <span class="font-weight-bold">{{ number_format($report->score) }}</span><span class="text-muted">/100</span>
                                                                    </div>
                                                            ...
                                                            <div class="d-none d-lg-flex col-lg-2 text-truncate">
                                                                <span class="text-truncate" data-tooltip="true" title="{{ $report->generated_at->tz(Auth::user()->timezone ?? config('settings.timezone'))->format(__('Y-m-d') . ' H:i:s') }}">{{ $report->generated_at->diffForHumans() }}</span>
                                                            </div>
                                                        </div>
                                                    </div>
