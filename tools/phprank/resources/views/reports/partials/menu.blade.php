...
<div class="dropdown-menu {{ (__('lang_dir') == 'rtl' ? 'dropdown-menu' : 'dropdown-menu-right') }} border-0 shadow z-1001">
    ...
    <a class="dropdown-item d-flex align-items-center" href="{{ $report->fullUrl }}" target="_blank" rel="nofollow noreferrer noopener">@include('icons.open-in-new', ['class' => 'text-muted fill-current width-4 height-4 '.(__('lang_dir') == 'rtl' ? 'ml-3' : 'mr-3')]) {{ __('Open') }}</a>

        <a class="dropdown-item d-flex align-items-center" href="{{ route('reports', ['project' => $report->project]) }}">@include('icons.account-tree', ['class' => 'text-muted fill-current width-4 height-4 '.(__('lang_dir') == 'rtl' ? 'ml-3' : 'mr-3')]) {{ __('Project') }}</a>

    @if(request()->is('admin/*') || Auth::check() && Auth::user()->role == 1 || Auth::check() && $report->user_id == Auth::user()->id)
        <div class="dropdown-divider"></div>

        <a class="dropdown-item text-danger d-flex align-items-center" href="#" data-toggle="modal" data-target="#modal" data-action="{{ request()->is('admin/*') || (Auth::user()->role == 1 && $report->user_id != Auth::user()->id) ? route('admin.reports.destroy', $report->id) : route('reports.destroy', ['id' => $report->id, 'project' => request()->input('project')]) }}" data-button-class="btn btn-danger position-relative" data-title="{{ __('Delete') }}" data-text="{{ __('Are you sure you want to delete :name?', ['name' => $report->url]) }}">@include('icons.delete', ['class' => 'fill-current width-4 height-4 '.(__('lang_dir') == 'rtl' ? 'ml-3' : 'mr-3')]) {{ __('Delete') }}</a>
    @endif
</div>
