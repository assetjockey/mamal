...
trait ReportTrait
{
    ...
    {
        // If the request is to generate or update a report
        if ($request->input('url') || $request->input('update')) {
        ...
        if (isset($urlRequest) && $urlRequest['response']) {
            ...
            if (config('settings.request_http_version') == '2') {
                // HTTP2
                $data['results']['http2'] = [
                    'passed' => true,
                    'importance' => 'medium',
            ...
            $report->score = (($report->reportScore / $report->totalScore) * 100);
