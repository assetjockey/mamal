...
class Report extends Model
{
     ...
     * @return Builder
     */
    public function scopeOfScore(Builder $query, $value)
    {
        if ($value == 'good') {
            return $query->where('score', '>', 79);
        } elseif ($value == 'decent') {
            return $query->where([['score', '>=', 49], ['score', '<=', 79]]);
        }

        return $query->where('score', '<', 49);
    }
     ...
     */
    public function getReportScoreAttribute()
    {
        $points = 0;
        foreach ($this->results as $key => $value) {
            if ($value['passed']) {
