...
class AccountController extends Controller
{
    ...
    {
        if ($request->has('remove_brand_logo')) {
            if ($request->user()->brand_logo) {
                Storage::disk(config('settings.storage_driver'))->delete('users/' . $request->user()->id . '/brand/' . $request->user()->brand_logo);
            }

            $request->user()->brand_logo = null;
            ...
            try {
                Image::useImageDriver(config('settings.image_driver'))
                    ->loadFile($request->file('brand_logo')->getRealPath())
                    ->fit(Fit::Contain, config('settings.user_brand_logo_size'), config('settings.user_brand_logo_size'))
                    ->format('jpg')
                    ->quality(80)
                    ->save();

                Storage::disk(config('settings.storage_driver'))->putFile('users/' . $request->user()->id . '/brand/', $request->file('brand_logo'), 'public');

                $request->user()->brand_logo = $request->file('brand_logo')->hashName();
            } catch (\Exception $e) {}
        }
        ...
        $request->user()->brand_information = [
