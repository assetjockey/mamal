...
class ProjectController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        return ProjectResource::collection(Report::select([DB::raw("`project` as `name`, SUBSTRING_INDEX(GROUP_CONCAT(`created_at` ORDER BY `created_at` ASC), ',', 1) AS `created_at`, COUNT(*) as `reports`, SUM(`score`) as `score`")])->where('user_id', $request->user()->id)
