...
class ProjectController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $projects = Report::select([DB::raw("`project` as `name`, SUBSTRING_INDEX(GROUP_CONCAT(`created_at` ORDER BY `created_at` ASC), ',', 1) AS `created_at`, COUNT(*) as `reports`, SUM(`score`) as `score`")])->where('user_id', $request->user()->id)
    ...
    {
        if ($request->has('bulk')) {
            $reports = Report::where('user_id', '=', $request->user()->id)->whereIn('project', array_slice(json_decode($id, true), 0, 100))->get(['id'])->toArray();

            Report::destroy($reports);

            return redirect()->route('projects')->with('success', __(':count records have been deleted.', ['count' => $request->input('bulk')]));
        }
    ...
    public function export(Request $request)
    {
        ...
        $projects = Report::select([DB::raw("`project` as `name`, COUNT(*) as `reports`, SUM(`score`) as `score`")])->where('user_id', $request->user()->id)
            ->when($search, function ($query) use ($search, $searchBy) {
                return $query->searchProject($search);
            })
            ->groupBy('project')
        ...
        $content->insertOne([__('Name'), __('Score'), __('Reports')]);
        foreach ($projects as $project) {
            $content->insertOne([$project->name, number_format($project->score / $project->reports), $project->reports]);
        }
