@component(theme_view('layouts.app', 'app'), [
    ...
    <div
        ...
        class="min-h-full px-5 py-5 sm:px-6 xl:px-8"
    >
        <div class="mx-auto max-w-[1540px] space-y-6">
            ...
            <x-ui.card class="overflow-hidden border-none shadow-[0_30px_90px_-56px_rgba(15,23,42,0.42)]" style="background: linear-gradient(135deg, rgba(var(--theme-accent-rgb), 0.10), rgba(var(--theme-surface-overlay-rgb), 0.98) 42%, rgba(var(--theme-surface-overlay-rgb), 0.98));">
                <div class="grid gap-8 xl:grid-cols-[minmax(0,1fr)_520px] xl:items-center">
                    <div class="space-y-4">
                        <span class="inline-flex items-center gap-2 rounded-lg border px-3 py-1.5 text-[11px] font-semibold uppercase tracking-[0.22em]" style="border-color: rgba(var(--theme-border-color-rgb), 0.58); background: rgba(var(--theme-surface-overlay-rgb), 0.82); color: var(--theme-accent);">
                            ...
                            {{ __('Creative asset search') }}
                        </span>
                        <div class="space-y-3">
                            <h1 class="text-3xl font-semibold tracking-[-0.05em] sm:text-4xl" style="color: var(--theme-header-text-color);">{{ __('Search Media Online') }}</h1>
                            <p class="max-w-3xl text-sm leading-7 sm:text-[15px]" style="color: var(--theme-muted-text-color);">{{ __('Find stock images and videos from your connected providers, then send the best picks straight into your Files library without leaving the workspace.') }}</p>
                        </div>
                    </div>

                    <div class="rounded-[1.35rem] border p-4 sm:p-5" style="border-color: rgba(var(--theme-border-color-rgb), 0.72); background:
                        radial-gradient(circle at 100% 0%, rgba(var(--theme-accent-rgb), 0.10), transparent 32%),
                        rgba(var(--theme-surface-overlay-rgb), 0.94);
                    ">
                        <div class="flex items-start justify-between gap-4">
                            <div>
                                <p class="text-[11px] font-semibold uppercase tracking-[0.2em]" style="color: var(--theme-muted-text-color);">{{ __('Asset finder') }}</p>
                                <h2 class="mt-3 text-2xl font-semibold tracking-[-0.04em]" style="color: var(--theme-header-text-color);">{{ __('Search and import') }}</h2>
                                <p class="mt-1 text-sm" style="color: var(--theme-muted-text-color);">{{ __('Pull online media directly into Files.') }}</p>
                            </div>
                            <span class="inline-flex h-12 w-12 shrink-0 items-center justify-center rounded-[1rem] shadow-[0_14px_30px_-22px_rgba(var(--theme-accent-rgb),0.65)]" style="background: linear-gradient(135deg, rgba(var(--theme-accent-rgb), 0.18), rgba(var(--theme-accent-rgb), 0.08)); color: var(--theme-accent);">
                                <i class="fa-light fa-images text-lg"></i>
                            </span>
                        </div>
                        ...
                        <div class="mt-5 grid gap-3">
                            ...
                            <div class="grid gap-3 lg:grid-cols-[minmax(0,0.9fr)_minmax(0,1.1fr)]">
                                ...
                                <div class="grid content-end gap-3 sm:grid-cols-[minmax(0,1fr)_minmax(0,1fr)]">
                                    <x-ui.button type="button" class="justify-center gap-2" x-on:click="searchMedia()" x-bind:disabled="searching || !keyword.trim()">
                                    ...
                                    <x-ui.button type="button" variant="outline" class="justify-center gap-2" x-on:click="saveSelected()" x-bind:disabled="saving || !selected.length">
                                        <i class="fa-light fa-folder-arrow-down text-[13px]"></i>
            ...
            <div class="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
                <x-ui.card class="overflow-hidden" style="background:
                    radial-gradient(circle at 100% 0%, rgba(59, 130, 246, 0.16), transparent 36%),
                    linear-gradient(135deg, rgba(59, 130, 246, 0.07), rgba(var(--theme-surface-base-rgb,255,255,255),0.96) 58%);
                    border-color: rgba(59, 130, 246, 0.18);
                ">
                    <div class="flex h-full items-start justify-between gap-4">
                        <div class="min-w-0 space-y-2">
                            <p class="text-[11px] font-semibold uppercase tracking-[0.16em]" style="color: rgb(37 99 235);">{{ __('Providers') }}</p>
                            <p class="truncate text-2xl font-semibold tracking-[-0.04em]" style="color: var(--theme-header-text-color);" x-text="providerCount()"></p>
                            <p class="line-clamp-2 text-sm leading-6 text-slate-500 dark:text-slate-400">{{ __('Connected search sources') }}</p>
                        </div>
                        <span class="inline-flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl" style="background: rgba(59, 130, 246, 0.10); color: rgb(37, 99, 235);">
                ...
                <x-ui.card class="overflow-hidden" style="background:
                    ...
                    linear-gradient(135deg, rgba(16, 185, 129, 0.07), rgba(var(--theme-surface-base-rgb,255,255,255),0.96) 58%);
                    border-color: rgba(16, 185, 129, 0.18);
                ">
                    <div class="flex h-full items-start justify-between gap-4">
                        <div class="min-w-0 space-y-2">
                            <p class="text-[11px] font-semibold uppercase tracking-[0.16em]" style="color: rgb(5 150 105);">{{ __('Results') }}</p>
                            <p class="truncate text-2xl font-semibold tracking-[-0.04em]" style="color: var(--theme-header-text-color);" x-text="results.length"></p>
                            <p class="line-clamp-2 text-sm leading-6 text-slate-500 dark:text-slate-400">{{ __('Assets returned by search') }}</p>
                        </div>
                        <span class="inline-flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl" style="background: rgba(16, 185, 129, 0.10); color: rgb(5, 150, 105);">
                ...
                <x-ui.card class="overflow-hidden" style="background:
                    ...
                    linear-gradient(135deg, rgba(245, 158, 11, 0.08), rgba(var(--theme-surface-base-rgb,255,255,255),0.96) 58%);
                    border-color: rgba(245, 158, 11, 0.20);
                ">
                    <div class="flex h-full items-start justify-between gap-4">
                        <div class="min-w-0 space-y-2">
                            <p class="text-[11px] font-semibold uppercase tracking-[0.16em]" style="color: rgb(217 119 6);">{{ __('Selected') }}</p>
                            <p class="truncate text-2xl font-semibold tracking-[-0.04em]" style="color: var(--theme-header-text-color);" x-text="selectedCount()"></p>
                            <p class="line-clamp-2 text-sm leading-6 text-slate-500 dark:text-slate-400">{{ __('Queued for Files import') }}</p>
                        </div>
                        <span class="inline-flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl" style="background: rgba(245, 158, 11, 0.12); color: rgb(217, 119, 6);">
            ...
            <section class="sticky top-[calc(var(--app-shell-header-height,79px)+1rem)] z-10">
                <x-ui.card class="px-4 py-3 shadow-[0_20px_60px_-44px_rgba(15,23,42,0.25)] backdrop-blur-xl" style="border-color: rgba(var(--theme-accent-rgb), 0.18); background:
                    radial-gradient(circle at 0% 50%, rgba(var(--theme-accent-rgb), 0.14), transparent 28%),
                    linear-gradient(90deg, rgba(var(--theme-accent-rgb),0.08), rgba(var(--theme-surface-overlay-rgb),0.95) 42%, rgba(245,158,11,0.07));
                ">
                    <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                        <div class="flex items-center gap-3">
                            <span class="inline-flex h-10 w-10 items-center justify-center rounded-2xl" style="background-color: rgba(var(--theme-accent-rgb), 0.12); color: var(--theme-accent-color);">
