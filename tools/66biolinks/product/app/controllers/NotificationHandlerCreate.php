...
class NotificationHandlerCreate extends Controller {
    ...
    public function index() {
                        ...
                        case 'ntfy':
                            ...
                            try {
                                ...
                                $response = \Unirest\Request::post(
                                    ...
                                    l('notification_handlers.test_description')
                                );
                            } catch (\Exception $exception) {
                                Alerts::add_error(l('notification_handlers.error_message_test') . '<br />' . e($exception->getMessage()));
                            }

                            if(isset($response) && ($response->code < 200 || $response->code > 299)) {
                                Alerts::add_error(l('notification_handlers.error_message_test') . '<br />' . '<strong>' . $response->code . ':</strong> ' .  e($response->raw_body));
                            }
						...
						case 'sixsixtext_save_contact':
							...
							try {
								$response = \Unirest\Request::post(
									...
									[
										'Authorization' => 'Bearer ' . $settings['sixsixtext_save_contact_api_key'],
									],
									[
										'phone_number' => $settings['sixsixtext_save_contact_phone_number'],
										'name' => 'John Doe',
						...
						case 'sixsixtext_send_sms':
							...
							try {
								$response = \Unirest\Request::post(
									...
									[
										'Authorization' => 'Bearer ' . $settings['sixsixtext_send_sms_api_key'],
									],
									[
										'device_id' => $settings['sixsixtext_send_sms_device_id'],
										'sim_subscription_id' => $settings['sixsixtext_send_sms_device_sim_subscription_id'],
										'phone_numbers' => $settings['sixsixtext_send_sms_phone_number'],
										'content' => l('notification_handlers.test_title'),
									]
								);
							} catch (\Exception $exception) {
								Alerts::add_error(l('notification_handlers.error_message_test') . '<br />' . e($exception->getMessage()));
        ...
        $values = [
            ...
            'ntfy' => isset($_POST['ntfy']) ? $_POST['ntfy'] : '',
            'twilio' => $_POST['twilio'] ?? '',
            ...
            'sixsixtext_send_sms_api_key' => $_POST['sixsixtext_send_sms_api_key'] ?? '',
			'sixsixtext_send_sms_device_id' => $_POST['sixsixtext_send_sms_device_id'] ?? '',
			'sixsixtext_send_sms_device_sim_subscription_id' => $_POST['sixsixtext_send_sms_device_sim_subscription_id'] ?? '',
			'sixsixtext_send_sms_phone_number' => $_POST['sixsixtext_send_sms_phone_number'] ?? '',
			'sixsixtext_save_contact_api_key' => $_POST['sixsixtext_save_contact_api_key'] ?? '',
			'sixsixtext_save_contact_phone_number' => $_POST['sixsixtext_save_contact_phone_number'] ?? '',
        ];
