...
class Link extends Controller {
	...
	private function process_static() {
		...
		if(isset($this->link->settings->static_folder) && $this->link->settings->static_folder) {
			$static_folder_name = $this->link->settings->static_folder;
		} elseif(isset($this->link->additional->static_folder) && $this->link->additional->static_folder) {
			$static_folder_name = $this->link->additional->static_folder;
		}
		...
		$full_requested_file = Uploads::get_full_path('static') . $static_folder_name . '/' . $requested_file;
		...
		$static_ai_cache_buster = isset($this->link->additional->static_ai_cache_buster) && $this->link->additional->static_ai_cache_buster ? $this->link->additional->static_ai_cache_buster : null;

		if(!$static_ai_cache_buster && isset($this->link->additional->static_ai_backup_folder) && preg_match('/^[a-f0-9]{32}$/', $this->link->additional->static_ai_backup_folder)) {
			/* Existing AI backup */
			$static_ai_cache_buster = $this->link->additional->static_ai_backup_folder;
		}
		...
		/* Cache headers */
		if($static_ai_cache_buster || in_array($requested_file_extension, ['html', 'htm'], true)) {
		...
		/* Output the file source */
		if($static_ai_cache_buster && in_array($requested_file_extension, ['html', 'htm', 'css'], true)) {
			/* Add cache buster */
			$file_content = file_get_contents($full_requested_file);

			if($file_content === false) {
				throw_404();
			}

			echo $this->add_static_ai_cache_buster_to_content($file_content, $static_ai_cache_buster, $requested_file_extension);
		} else {
	...
	private function add_static_ai_cache_buster_to_content($content, $static_ai_cache_buster, $requested_file_extension) {
		/* HTML assets */
		if(in_array($requested_file_extension, ['html', 'htm'], true)) {
			$content = preg_replace_callback('/\b(href|src)=([\'"])([^\'"]+)\2/i', function($matches) use ($static_ai_cache_buster) {
				$asset_url = $this->add_static_ai_cache_buster_to_url($matches[3], $static_ai_cache_buster);

				if($asset_url == $matches[3]) {
					return $matches[0];
				}

				return $matches[1] . '=' . $matches[2] . $asset_url . $matches[2];
			}, $content);
		}

		/* CSS assets */
		if($requested_file_extension == 'css') {
			$content = preg_replace_callback('/url\((["\']?)([^"\')]+)\1\)/i', function($matches) use ($static_ai_cache_buster) {
				$original_asset_url = trim($matches[2]);
				$asset_url = $this->add_static_ai_cache_buster_to_url($original_asset_url, $static_ai_cache_buster);

				if($asset_url == $original_asset_url) {
					return $matches[0];
				}

				return 'url(' . $matches[1] . $asset_url . $matches[1] . ')';
			}, $content);
		}
	...
	private function add_static_ai_cache_buster_to_url($asset_url, $static_ai_cache_buster) {
		/* Skip external urls */
		if($asset_url == '' || $asset_url[0] == '#' || $asset_url[0] == '/' || preg_match('~^(?:[a-z][a-z0-9+\-.]*:|//)~i', $asset_url)) {
			return $asset_url;
		}

		$asset_url_parts = explode('#', $asset_url, 2);
		$asset_url_without_anchor = $asset_url_parts[0];
		$asset_url_anchor = isset($asset_url_parts[1]) ? '#' . $asset_url_parts[1] : '';
		$asset_url_path_parts = explode('?', $asset_url_without_anchor, 2);
		$asset_url_path = $asset_url_path_parts[0];
		$asset_url_extension = mb_strtolower(pathinfo($asset_url_path, PATHINFO_EXTENSION));

		if(!$asset_url_extension || !in_array($asset_url_extension, \Altum\Uploads::$uploads['static']['inside_zip_whitelisted_file_extensions'], true)) {
			return $asset_url;
		}

		$asset_url_separator = strpos($asset_url_without_anchor, '?') === false ? '?' : '&';

		return $asset_url_without_anchor . $asset_url_separator . 'v=' . urlencode($static_ai_cache_buster) . $asset_url_anchor;
	}
	...
	public function email_collector() {
		...
		if(\Altum\Plugin::is_active('newsletters')) {
			newsletters_subscribe_from_data($link->user_id, $_POST['email'], $_POST['name'], $biolink_block->type, $datum_id, $link->link_id, $link->project_id, $biolink_block->biolink_block_id);
		}
