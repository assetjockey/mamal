...
class Cron extends Controller {
	...
	public function projects_email_reports() {
		...
		$result = database()->query("
            ...
            WHERE
                ...
                AND `projects`.`email_reports_last_datetime` <= '{$minimum_datetime}'
            LIMIT 25
        ");
		...
		while($row = $result->fetch_object()) {
			...
			$links_result = database()->query("
                SELECT
                    ...
                    `links`.`url`,
                    `links`.`type`,
                    COUNT(`track_links`.`id`) AS `pageviews`,
					COALESCE(SUM(`track_links`.`is_unique`), 0) AS `visitors`
                FROM
                    `links`
                LEFT JOIN
	...
	private function users_reset() {
		$values = [
			'ai_static_prompts_current_month' => 0,
		];

		/* Reset newsletter usage only when the plugin is active */
		if(\Altum\Plugin::is_active('newsletters')) {
			$values['newsletter_emails_current_month'] = 0;
		}
	...
	public function newsletters() {
		if(\Altum\Plugin::is_active('newsletters')) {
			...
			require_once \Altum\Plugin::get('newsletters')->path . 'controllers/Cron.php';

			$this->close();

			/* Mark cron execution */
			$this->update_cron_execution_datetimes('newsletters_datetime');
		}
	}
