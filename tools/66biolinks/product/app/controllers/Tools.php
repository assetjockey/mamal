...
//            }
	...
	public function case_converter() {
		...
		if(!empty($_POST)) {
			$_POST['text'] = isset($_POST['text']) ? input_clean($_POST['text']) : '';
			$_POST['type'] = isset($_POST['type']) && in_array($_POST['type'], ['lowercase', 'uppercase', 'sentencecase', 'camelcase', 'pascalcase', 'capitalcase', 'constantcase', 'dotcase', 'snakecase', 'paramcase'], true) ? $_POST['type'] : 'lowercase';
			...
			if(!Alerts::has_field_errors() && !Alerts::has_errors()) {
				...
				$filtered_text = trim(string_filter_alphanumeric($_POST['text']));
				$words = preg_split('/\s+/u', $filtered_text, -1, PREG_SPLIT_NO_EMPTY);
				...
				switch($_POST['type']) {
					...
					case 'sentencecase':
						...
						$data['result'] = mb_strtoupper(mb_substr($text, 0, 1)) . mb_substr($text, 1);
						break;

					case 'camelcase':
						$data['result'] = lcfirst(implode('', array_map('ucfirst', array_map('mb_strtolower', $words))));
						break;

					case 'pascalcase':
						$data['result'] = implode('', array_map('ucfirst', array_map('mb_strtolower', $words)));
						break;

					case 'capitalcase':
						$data['result'] = mb_convert_case($_POST['text'], MB_CASE_TITLE);
						break;

					case 'constantcase':
						$data['result'] = mb_strtoupper(implode('_', $words));
						break;

					case 'dotcase':
						$data['result'] = mb_strtolower(implode('.', $words));
						break;

					case 'snakecase':
						$data['result'] = mb_strtolower(implode('_', $words));
						break;
    ...
    public function password_generator() {
        ...
        if(!empty($_POST)) {
            $_POST['characters'] = max(1, min(2048, (int) ($_POST['characters'] ?? 8)));
            $pools = [];

            if(isset($_POST['numbers'])) $pools[] = '0123456789';
            if(isset($_POST['symbols'])) $pools[] = '!@#$%^&*()_+=-[],./\\\'<>?:"|{}';
            if(isset($_POST['lowercase'])) $pools[] = 'abcdefghijklmnopqrstuvwxyz';
            if(isset($_POST['uppercase'])) $pools[] = 'ABCDEFGHIJKLMNOPQRSTUVWXYZ';

            if(empty($pools)) $pools[] = implode('', $pools);
            ...
            if(!Alerts::has_field_errors() && !Alerts::has_errors()) {
				...
				$available_characters = implode('', $pools);
                $password = '';

                for($i = 0; $i < $_POST['characters']; $i++) {
                    $password .= $available_characters[random_int(0, strlen($available_characters) - 1)];
                }
