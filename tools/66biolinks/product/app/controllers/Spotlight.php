        ...
        if(is_logged_in()) {
            ...
            if(settings()->links->biolinks_is_enabled) {
                ...
                if(\Altum\Plugin::is_active('newsletters') && settings()->links->biolinks_is_enabled && settings()->newsletters->is_enabled) {
                    $available_pages[] = [
                        'name' => l('newsletters.title'),
                        'url' => 'newsletters'
                    ];

                    $available_pages[] = [
                        'name' => l('newsletter_create.title'),
                        'url' => 'newsletter-create'
                    ];

                    $available_pages[] = [
                        'name' => l('newsletter_subscribers.title'),
                        'url' => 'newsletter-subscribers'
                    ];

                    $available_pages[] = [
                        'name' => l('newsletter_subscriber_create.title'),
                        'url' => 'newsletter-subscriber-create'
                    ];

                    $available_pages[] = [
                        'name' => l('newsletter_subscribers_import.title'),
                        'url' => 'newsletter-subscribers-import'
                    ];
                }
            }
            ...
            if(user()->type == 1 && file_exists(APP_PATH . 'languages/admin/' . \Altum\Language::$name . '#' . \Altum\Language::$code . '.php')) {
                ...
                if(\Altum\Plugin::is_active('newsletters')) {
                    $available_pages[] = [
                        'name' => l('global.menu.admin') . ' - ' . sprintf(l('admin_settings.title'), l('admin_settings.newsletters.tab')),
                        'url' => 'admin/settings/newsletters'
                    ];
                }
                ...
                /* Newsletters plugin pages */
                if(\Altum\Plugin::is_active('newsletters') && settings()->links->biolinks_is_enabled && (settings()->newsletters->is_enabled ?? false)) {
                    $available_pages[] = [
                        'name' => l('global.menu.admin') . ' - ' . l('admin_newsletters.title'),
                        'url' => 'admin/newsletters'
                    ];

                    $available_pages[] = [
                        'name' => l('global.menu.admin') . ' - ' . l('admin_newsletter_subscribers.title'),
                        'url' => 'admin/newsletter-subscribers'
                    ];

                    /* Newsletters statistics */
                    $available_pages[] = [
                        'name' => l('global.menu.admin') . ' - ' . l('admin_statistics.newsletters.menu'),
                        'url' => 'admin/statistics/newsletters'
                    ];

                    $available_pages[] = [
                        'name' => l('global.menu.admin') . ' - ' . l('admin_statistics.newsletter_subscribers.menu'),
                        'url' => 'admin/statistics/newsletter_subscribers'
                    ];
                }
