...
class ApiQrCodes extends Controller {
    ...
    private function get() {
        ...
        $data = [
            ...
            'link_id' => $qr_code->link_id ? (int) $qr_code->link_id : null,
            'name' => $qr_code->name,
            'type' => $qr_code->type,
    ...
    private function post() {
        ...
        switch ($_POST['type']) {
            ...
            case 'url':
                $_POST['url_dynamic'] = (int) ($_POST['url_dynamic'] ?? 0);
                $_POST['url_dynamic_existing_link'] = (int) ($_POST['url_dynamic_existing_link'] ?? 0);
    ...
    private function patch() {
        ...
        switch ($_POST['type']) {
            ...
            case 'url':
                $_POST['url_dynamic'] = isset($_POST['url_dynamic']) ? (int) $_POST['url_dynamic'] : (int) (bool) $qr_code->link_id;
                $_POST['url_dynamic_existing_link'] = isset($_POST['url_dynamic_existing_link']) ? (int) $_POST['url_dynamic_existing_link'] : (int) (bool) ($qr_code->settings->url_dynamic_existing_link ?? $qr_code->link_id);

                if($_POST['url_dynamic'] && $_POST['url_dynamic_existing_link']) {
                    $_POST['link_id'] = $_POST['link_id'] ?? $qr_code->link_id;
                    $settings['url_dynamic_existing_link'] = $_POST['url_dynamic_existing_link'];
                ...
                elseif($_POST['url_dynamic']) {
                    if(isset($_POST['url'])) {
                        ...
                        $link = $qr_code->link_id ? db()->where('link_id', $qr_code->link_id)->where('user_id', $this->user->user_id)->getOne('links', ['link_id', 'location_url']) : null;
                        $_POST['url'] = $link ? $link->location_url : null;
                    }

                    $settings['url'] = $_POST['url'] = input_clean($_POST['url'] ?? null, $available_qr_codes['url']['max_length']);
                    $settings['url_dynamic_existing_link'] = 0;
                    $_POST['link_id'] = $qr_code->link_id;
                    ...
                    if(!$_POST['link_id'] && $this->user->plan_settings->links_limit != -1 && $total_links >= $this->user->plan_settings->links_limit) {
                        $this->response_error(l('global.info_message.plan_feature_limit'), 401);
                    }
                }
        ...
        if($_POST['type'] == 'url' && isset($_POST['url_dynamic']) && !$_POST['url_dynamic_existing_link']) {
            ...
            if($_POST['link_id']) {
                ...
                if($link) {
                    ...
                    if($link->domain_id) {
                        $domain = (new \Altum\Models\Domain())->get_domain_by_domain_id($link->domain_id);
                        $short_url = $domain->scheme . $domain->host . '/' . ($domain->link_id == $link->link_id ? null : $link->url);
                    } else {
                        $short_url = SITE_URL . $link->url;
                    }
                } else {
                    $_POST['link_id'] = null;
                }
            ...
            if(!$_POST['link_id']) {
                ...
                $short_url = SITE_URL . $url;
            }

            $_POST['url_dynamic_existing_link'] = 1;
            $settings['url_dynamic_existing_link'] = 1;
            $settings['url'] = $_POST['url'] = $short_url;
        }
