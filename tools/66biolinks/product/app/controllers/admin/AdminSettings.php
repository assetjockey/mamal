...
class AdminSettings extends Controller {
    ...
    public function digital_wallets() {
        ...
        if(!empty($_POST)) {
            ...
            /* Upload the Google Wallet service account */
            settings()->digital_wallets->google_wallet_service_account = \Altum\Uploads::process_upload(settings()->digital_wallets->google_wallet_service_account ?? null, 'google_wallet_service_account', 'google_wallet_service_account', 'google_wallet_service_account_remove', null);
	...
	public function newsletters() {
		...
		if(!empty($_POST)) {
			...
			/* :) */
			$_POST['is_enabled'] = (int) isset($_POST['is_enabled']);
			$_POST['custom_smtp_is_enabled'] = (int) isset($_POST['custom_smtp_is_enabled']);
			$_POST['statistics_is_enabled'] = (int) isset($_POST['statistics_is_enabled']);
			$_POST['emails_per_cron'] = isset($_POST['emails_per_cron']) && $_POST['newsletters_emails_per_cron'] > 0 ? (int) $_POST['newsletters_emails_per_cron'] : 40;

			$value = json_encode([
				'is_enabled' => $_POST['is_enabled'],
				'custom_smtp_is_enabled' => $_POST['custom_smtp_is_enabled'],
				'statistics_is_enabled' => $_POST['statistics_is_enabled'],
				'emails_per_cron' => $_POST['emails_per_cron'],
			]);

			$this->update_settings('newsletters', $value);
		}
	}
    ...
    public function notification_handlers() {
        ...
        if(!empty($_POST)) {
            ...
            /* :) */
            $_POST['is_enabled'] = (int) isset($_POST['is_enabled']);
            $_POST['sixsixtext_name'] = trim($_POST['sixsixtext_name'] ?? '');
            $_POST['sixsixtext_url'] = trim($_POST['sixsixtext_url'] ?? '');
            ...
            $value = [
                'is_enabled' => $_POST['is_enabled'],
                'sixsixtext_name' => $_POST['sixsixtext_name'],
                'sixsixtext_url' => $_POST['sixsixtext_url'],
    ...
    public function links() {
        ...
        if(!empty($_POST)) {
            ...
            $_POST['projects_email_reports_is_enabled'] = in_array($_POST['projects_email_reports_is_enabled'] ?? 0, [0, 'weekly', 'monthly']) ? $_POST['projects_email_reports_is_enabled'] : 0;
            ...
            $value = [
                ...
                'projects_email_reports_is_enabled' => $_POST['projects_email_reports_is_enabled'],
    ...
    public function tools() {
        ...
        if(!empty($_POST)) {
			...
			$tool_categories = require APP_PATH . 'includes/tools/categories.php';
			...
			/* Preserve the categories order */
			$categories_order = array_values(array_filter($_POST['categories_order'] ?? [], fn($item) => isset($tool_categories[$item])));
			$categories_order = array_keys(array_flip($categories_order) + $tool_categories);
    ...
    public function aix() {
        ...
        if(!empty($_POST)) {
            ...
            $_POST['input_moderation_is_enabled'] = (int) isset($_POST['input_moderation_is_enabled']);
            $_POST['chats_is_enabled'] = (int) isset($_POST['chats_is_enabled']);
