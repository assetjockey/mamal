...
class AdminStatistics extends Controller {
    ...
    protected function newsletters() {
        ...
        $total = ['newsletters' => 0, 'sent_emails' => 0];

        $convert_tz_sql = get_convert_tz_sql('`datetime`', $this->user->timezone);

        /* Newsletters */
        $newsletters_chart = [];
        $result = database()->query("SELECT COUNT(*) AS `total`, DATE_FORMAT({$convert_tz_sql}, '{$this->datetime['query_date_format']}') AS `formatted_date`, SUM(`sent_emails`) AS `sent_emails` FROM `newsletters` WHERE {$convert_tz_sql} BETWEEN '{$this->datetime['query_start_date']}' AND '{$this->datetime['query_end_date']}' GROUP BY `formatted_date`");
        while($row = $result->fetch_object()) {
            $row->formatted_date = $this->datetime['process']($row->formatted_date, true);

            $newsletters_chart[$row->formatted_date] = [
                'newsletters' => $row->total,
                'sent_emails' => $row->sent_emails,
            ];

            $total['newsletters'] += $row->total;
            $total['sent_emails'] += $row->sent_emails;
        }

        $newsletters_chart = get_chart_data($newsletters_chart);

        return [
            'total' => $total,
            'newsletters_chart' => $newsletters_chart,
        ];

    }

    protected function newsletter_subscribers() {

        if(!\Altum\Plugin::is_active('newsletters') || !settings()->links->biolinks_is_enabled || !(settings()->newsletters->is_enabled ?? false)) {
            redirect('admin/statistics');
        }

        $total = ['newsletter_subscribers' => 0];

        $convert_tz_sql = get_convert_tz_sql('`datetime`', $this->user->timezone);

        /* Newsletter subscribers */
        $newsletter_subscribers_chart = [];
        $result = database()->query("SELECT COUNT(*) AS `total`, DATE_FORMAT({$convert_tz_sql}, '{$this->datetime['query_date_format']}') AS `formatted_date` FROM `newsletter_subscribers` WHERE {$convert_tz_sql} BETWEEN '{$this->datetime['query_start_date']}' AND '{$this->datetime['query_end_date']}' GROUP BY `formatted_date`");
        while($row = $result->fetch_object()) {
            $row->formatted_date = $this->datetime['process']($row->formatted_date, true);

            $newsletter_subscribers_chart[$row->formatted_date] = [
                'newsletter_subscribers' => $row->total,
            ];

            $total['newsletter_subscribers'] += $row->total;
        }

        $newsletter_subscribers_chart = get_chart_data($newsletter_subscribers_chart);

        return [
            'total' => $total,
            'newsletter_subscribers_chart' => $newsletter_subscribers_chart,
        ];
    ...
    protected function tools_ratings() {
        ...
        while($row = $result->fetch_object()) {
            ...
            $total['average_rating'] += $row->average_rating * $row->total_ratings;
        }
