...
class AdminSyntheses extends Controller {
    ...
    public function index() {
        ...
        $filters = (new \Altum\Filters(['synthesis_id', 'user_id', 'project_id', 'format', 'voice_id', 'model'], ['name'], ['synthesis_id', 'last_datetime', 'datetime', 'name', 'characters'], allowed_datetime_fields: ['last_datetime', 'datetime']));
        ...
        while($row = $syntheses_result->fetch_object()) {
            ...
            if(!isset($row->model) && isset($row->voice_engine)) {
                $row->model = $row->voice_engine;
            }
            $syntheses[] = $row;
        }

        /* Export handler */
        process_export_csv($syntheses, ['synthesis_id', 'project_id', 'user_id', 'name', 'input', 'file', 'model', 'format', 'voice_id', 'characters', 'api_response_time', 'datetime', 'last_datetime'], sprintf(l('syntheses.title')));
        process_export_json($syntheses, ['synthesis_id', 'project_id', 'user_id', 'name', 'input', 'file', 'model', 'format', 'voice_id', 'settings', 'characters', 'api_response_time', 'datetime', 'last_datetime'], sprintf(l('syntheses.title')));
