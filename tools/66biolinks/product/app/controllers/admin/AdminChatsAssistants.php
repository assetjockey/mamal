...
class AdminChatsAssistants extends Controller {
    ...
    public function index() {
        ...
        $filters = (new \Altum\Filters(['chat_assistant_id'], ['name'], ['chat_assistant_id', 'datetime', 'name', 'total_usage'], allowed_datetime_fields: ['datetime']));
    ...
    public function delete() {
        ...
        /* Check for linked chats */
        $linked_chats = db()->where('chat_assistant_id', $chat_assistant_id)->getValue('chats', 'count(*)');
        if($linked_chats) {
            Alerts::add_error(sprintf(l('admin_chats_assistants.error_message.delete_used'), '<strong>' . $template->name . '</strong>'));
        }
