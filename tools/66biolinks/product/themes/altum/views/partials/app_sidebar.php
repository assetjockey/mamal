            ...
            <?php if(is_logged_in()): ?>
                ...
                <?php if(settings()->links->biolinks_is_enabled): ?>
                    ...
                    <?php if(\Altum\Plugin::is_active('newsletters') && settings()->links->biolinks_is_enabled && settings()->newsletters->is_enabled): ?>
                        <li class="<?= in_array(\Altum\Router::$controller, ['Newsletters', 'NewsletterCreate', 'NewsletterUpdate', 'NewsletterView']) ? 'active' : null ?>">
                            <a href="<?= url('newsletters') ?>"><i class="fas fa-fw fa-sm fa-envelope-open-text mr-2"></i> <?= l('newsletters.menu') ?></a>
                        </li>

                        <li class="<?= in_array(\Altum\Router::$controller, ['NewsletterSubscribers', 'NewsletterSubscriberCreate', 'NewsletterSubscribersImport']) ? 'active' : null ?>">
                            <a href="<?= url('newsletter-subscribers') ?>"><i class="fas fa-fw fa-sm fa-users mr-2"></i> <?= l('newsletter_subscribers.menu') ?></a>
                        </li>
                    <?php endif ?>
