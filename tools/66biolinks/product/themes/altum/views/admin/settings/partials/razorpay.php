...
<div>
    ...
    <div class="<?= !in_array(settings()->license->type, ['Extended License', 'extended']) ? 'container-disabled' : null ?>">
        ...
        <div class="form-group">
            <label for="key_id"><i class="fas fa-fw fa-sm fa-key text-muted mr-1"></i> <?= l('admin_settings.razorpay.key_id') ?></label>
            <input id="key_id" type="text" name="key_id" class="form-control" value="<?= settings()->razorpay->key_id ?>" />
        </div>

        <div class="form-group">
            <label for="key_secret"><i class="fas fa-fw fa-sm fa-lock text-muted mr-1"></i> <?= l('admin_settings.razorpay.key_secret') ?></label>
            <input id="key_secret" type="text" name="key_secret" class="form-control" value="<?= settings()->razorpay->key_secret ?>" />
        </div>

        <div class="form-group">
            <label for="webhook_secret"><i class="fas fa-fw fa-sm fa-lock text-muted mr-1"></i> <?= l('admin_settings.razorpay.webhook_secret') ?></label>
            <input id="webhook_secret" type="text" name="webhook_secret" class="form-control" value="<?= settings()->razorpay->webhook_secret ?>" />
        </div>

        <div class="form-group">
            <label for="webhook_identifier"><i class="fas fa-fw fa-sm fa-fingerprint text-muted mr-1"></i> <?= l('admin_settings.razorpay.webhook_identifier') ?></label>
            <input id="webhook_identifier" type="text" name="webhook_identifier" class="form-control" value="<?= settings()->razorpay->webhook_identifier ?? '' ?>" />
            <small class="form-text text-muted"><?= l('admin_settings.razorpay.webhook_identifier_help') ?></small>
        </div>
