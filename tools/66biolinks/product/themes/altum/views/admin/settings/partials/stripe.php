...
<div>
    ...
    <div class="<?= !in_array(settings()->license->type, ['Extended License', 'extended']) ? 'container-disabled' : null ?>">
        ...
        <div class="form-group">
            <label for="publishable_key"><i class="fas fa-fw fa-sm fa-key text-muted mr-1"></i> <?= l('admin_settings.stripe.publishable_key') ?></label>
            <input id="publishable_key" type="text" name="publishable_key" class="form-control" value="<?= settings()->stripe->publishable_key ?>" />
        </div>

        <div class="form-group">
            <label for="secret_key"><i class="fas fa-fw fa-sm fa-lock text-muted mr-1"></i> <?= l('admin_settings.stripe.secret_key') ?></label>
            <input id="secret_key" type="text" name="secret_key" class="form-control" value="<?= settings()->stripe->secret_key ?>" />
        </div>

        <div class="form-group">
            <label for="webhook_secret"><i class="fas fa-fw fa-sm fa-lock text-muted mr-1"></i> <?= l('admin_settings.stripe.webhook_secret') ?></label>
