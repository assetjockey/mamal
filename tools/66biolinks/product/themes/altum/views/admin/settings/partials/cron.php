...
<div class="form-group">
    <label for="cron"><i class="fas fa-fw fa-sm fa-clock text-muted mr-1"></i> <?= l('admin_settings.cron.cron') ?></label>
...
<div class="form-group">
    <label for="cron_email_reports"><i class="fas fa-fw fa-sm fa-envelope text-muted mr-1"></i> <?= l('admin_settings.cron.email_reports') ?></label>
    <div class="input-group">
...
<div class="form-group">
    <label for="cron_projects_email_reports"><i class="fas fa-fw fa-sm fa-envelope text-muted mr-1"></i> <?= l('admin_settings.cron.projects_email_reports') ?></label>
    <div class="input-group">
        <input id="cron_projects_email_reports" name="cron_projects_email_reports" type="text" class="form-control" value="<?= '* * * * * wget --quiet -O /dev/null ' . SITE_URL . 'cron/projects_email_reports?key=' . settings()->cron->key ?>" readonly="readonly" />
        <div class="input-group-append">
            <button
                    type="button"
                    class="btn btn-light"
                    ...
                    data-copy="<?= l('global.clipboard_copy') ?>"
                    data-copied="<?= l('global.clipboard_copied') ?>"
                    data-clipboard-text="<?= '* * * * * wget --quiet -O /dev/null ' . SITE_URL . 'cron/projects_email_reports?key=' . settings()->cron->key ?>"
            >
                <i class="fas fa-fw fa-sm fa-copy"></i>
            </button>
        </div>
        <div class="input-group-append">
            <a
                    href="<?= SITE_URL . 'cron/projects_email_reports?key=' . settings()->cron->key ?>"
                    target="_blank"
                    class="btn btn-light"
                    data-toggle="tooltip"
                    title="<?= l('admin_settings.cron.run_manually') ?>"
    ...
    if(!isset(settings()->cron->projects_email_reports_datetime)) {
        $text_class = 'text-danger';
    } else {

        if((new DateTime(settings()->cron->projects_email_reports_datetime)) < (new \DateTime())->modify('-1 hour')) {
            $text_class = 'text-warning';
        }

        if((new DateTime(settings()->cron->projects_email_reports_datetime)) < (new \DateTime())->modify('-1 day')) {
            $text_class = 'text-danger';
        }
    }
    ?>

    <small class="form-text <?= $text_class ?>"><?= sprintf(l('admin_settings.cron.last_execution'), isset(settings()->cron->projects_email_reports_datetime) ? \Altum\Date::get_timeago(settings()->cron->projects_email_reports_datetime) : l('global.na')) ?></small>
    <?php if(isset(settings()->cron->projects_email_reports_datetime)): ?>
        <small class="form-text text-muted"><?= sprintf(l('admin_settings.cron.last_processing_time'), isset(settings()->cron->projects_email_reports_datetime_processing) ? nr(settings()->cron->projects_email_reports_datetime_processing, 8) . ' ' . l('global.date.seconds') : l('global.na')) ?></small>
    <?php endif ?>
</div>

<div class="form-group">
    <label for="cron_broadcasts"><i class="fas fa-fw fa-sm fa-envelopes-bulk text-muted mr-1"></i> <?= l('admin_settings.cron.broadcasts') ?></label>
    <div class="input-group">
...
<?php if(\Altum\Plugin::is_active('newsletters')): ?>
    <div class="form-group">
        <label for="cron_newsletters"><i class="fas fa-fw fa-sm fa-envelope-open-text text-muted mr-1"></i> <?= l('admin_settings.cron.newsletters') ?></label>
        <div class="input-group">
            <input id="cron_newsletters" name="cron_newsletters" type="text" class="form-control" value="<?= '* * * * * wget --quiet -O /dev/null ' . SITE_URL . 'cron/newsletters?key=' . settings()->cron->key ?>" readonly="readonly" />
            <div class="input-group-append">
                <button
                        type="button"
                        class="btn btn-light"
                        ...
                        data-copy="<?= l('global.clipboard_copy') ?>"
                        data-copied="<?= l('global.clipboard_copied') ?>"
                        data-clipboard-text="<?= '* * * * * wget --quiet -O /dev/null ' . SITE_URL . 'cron/newsletters?key=' . settings()->cron->key ?>"
                >
                    <i class="fas fa-fw fa-sm fa-copy"></i>
                </button>
            </div>
            ...
            <div class="input-group-append">
                <a
                        href="<?= SITE_URL . 'cron/newsletters?key=' . settings()->cron->key ?>"
                        target="_blank"
                        class="btn btn-light"
                        data-toggle="tooltip"
                        title="<?= l('admin_settings.cron.run_manually') ?>"
        ...
        if(!isset(settings()->cron->newsletters_datetime)) {
            $text_class = 'text-danger';
        } else {

            if((new DateTime(settings()->cron->newsletters_datetime)) < (new \DateTime())->modify('-1 hour')) {
                $text_class = 'text-warning';
            }

            if((new DateTime(settings()->cron->newsletters_datetime)) < (new \DateTime())->modify('-1 day')) {
                $text_class = 'text-danger';
            }
        }
        ?>

        <small class="form-text <?= $text_class ?>"><?= sprintf(l('admin_settings.cron.last_execution'), isset(settings()->cron->newsletters_datetime) ? \Altum\Date::get_timeago(settings()->cron->newsletters_datetime) : l('global.na')) ?></small>
        <?php if(isset(settings()->cron->newsletters_datetime)): ?>
            <small class="form-text text-muted"><?= sprintf(l('admin_settings.cron.last_processing_time'), isset(settings()->cron->newsletters_datetime_processing) ? nr(settings()->cron->newsletters_datetime_processing, 8) . ' ' . l('global.date.seconds') : l('global.na')) ?></small>
        <?php endif ?>
    </div>
<?php endif ?>
...
<div <?= !\Altum\Plugin::is_active('push-notifications') ? 'data-toggle="tooltip" title="' . sprintf(l('admin_plugins.no_access'), \Altum\Plugin::get('push-notifications')->name ?? 'push-notifications') . '"' : null ?>>
    <div class="<?= !\Altum\Plugin::is_active('push-notifications') ? 'container-disabled' : null ?>">
        <div class="form-group">
            <label for="cron_push_notifications"><i class="fas fa-fw fa-sm fa-bell text-muted mr-1"></i> <?= l('admin_settings.cron.push_notifications') ?></label>
            <div class="input-group">
