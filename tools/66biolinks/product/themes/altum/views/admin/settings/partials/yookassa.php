...
<div>
    ...
    <div class="<?= !in_array(settings()->license->type, ['Extended License', 'extended']) ? 'container-disabled' : null ?>">
        ...
        <div class="form-group">
            <label for="shop_id"><i class="fas fa-fw fa-sm fa-fingerprint text-muted mr-1"></i> <?= l('admin_settings.yookassa.shop_id') ?></label>
            <input id="shop_id" type="text" name="shop_id" class="form-control" value="<?= settings()->yookassa->shop_id ?>" />
        </div>

        <div class="form-group">
            <label for="secret_key"><i class="fas fa-fw fa-sm fa-lock text-muted mr-1"></i> <?= l('admin_settings.yookassa.secret_key') ?></label>
