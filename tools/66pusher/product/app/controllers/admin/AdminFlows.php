...
class AdminFlows extends Controller {
    ...
    public function index() {
        ...
        $filters = (new \Altum\Filters(['website_id', 'segment', 'user_id'], ['name', 'title', 'description', 'url',], ['flow_id', 'name', 'title', 'last_sent_datetime', 'datetime', 'last_datetime'], allowed_datetime_fields: ['last_sent_datetime', 'datetime', 'last_datetime']));
        ...
        $paginator = (new \Altum\Paginator($total_rows, $filters->get_results_per_page(), $_GET['page'] ?? 1, url('admin/flows?' . $filters->get_get() . '&page={{PAGE}}')));
