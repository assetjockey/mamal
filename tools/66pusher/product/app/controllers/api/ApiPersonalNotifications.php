...
class ApiPersonalNotifications extends Controller {
    ...
    private function get_all() {
        ...
        $filters = (new \Altum\Filters(['website_id', 'status', 'type', 'user_id'], ['name', 'title', 'description', 'url',], ['personal_notification_id', 'name', 'title', 'scheduled_datetime', 'sent_datetime', 'datetime', 'last_datetime',], allowed_datetime_fields: ['datetime', 'last_datetime', 'sent_datetime', 'scheduled_datetime']));
        ...
        $paginator = (new \Altum\Paginator($total_rows, $filters->get_results_per_page(), $_GET['page'] ?? 1, url('api/personal-notifications?' . $filters->get_get() . '&page={{PAGE}}')));
    ...
    private function get() {
        ...
        $personal_notification = db()->where('personal_notification_id', $personal_notification_id)->where('user_id', $this->user->user_id)->getOne('personal_notifications');
    ...
    private function post() {
        ...
        $total_rows = db()->where('user_id', $this->user->user_id)->getValue('personal_notifications', 'count(*)');
    ...
    private function patch() {
        ...
        /* Check for the plan limit */
        $total_rows = db()->where('user_id', $this->user->user_id)->getValue('personal_notifications', 'count(`personal_notification_id`)');
