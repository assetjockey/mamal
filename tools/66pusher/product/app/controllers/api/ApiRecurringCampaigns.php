...
class ApiRecurringCampaigns extends Controller {
    ...
    private function get_all() {
        ...
        $filters = (new \Altum\Filters(['website_id', 'segment', 'user_id'], ['name', 'title', 'description', 'url'], ['recurring_campaign_id', 'name', 'title', 'last_run_datetime', 'next_run_datetime', 'datetime', 'last_datetime', 'total_push_notifications', 'total_sent_push_notifications', 'total_displayed_push_notifications', 'total_clicked_push_notifications', 'total_closed_push_notifications', 'total_campaigns'], allowed_datetime_fields: ['last_run_datetime', 'next_run_datetime', 'datetime', 'last_datetime']));
        ...
        $paginator = (new \Altum\Paginator($total_rows, $filters->get_results_per_page(), $_GET['page'] ?? 1, url('api/recurring-campaigns?' . $filters->get_get() . '&page={{PAGE}}')));
    ...
    private function get() {
        ...
        $recurring_campaign = db()->where('recurring_campaign_id', $recurring_campaign_id)->where('user_id', $this->user->user_id)->getOne('recurring_campaigns');
    ...
    private function post() {
        ...
        $total_rows = db()->where('user_id', $this->user->user_id)->getValue('recurring_campaigns', 'count(*)');
    ...
    private function patch() {
        ...
        /* Check for the plan limit */
        $total_rows = db()->where('user_id', $this->user->user_id)->getValue('recurring_campaigns', 'count(`recurring_campaign_id`)');
