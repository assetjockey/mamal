...
class ApiRssAutomations extends Controller {
    ...
    private function get_all() {
        ...
        $filters = (new \Altum\Filters(['website_id', 'segment', 'user_id'], ['name', 'title', 'description', 'url', 'rss_url'], ['rss_automation_id', 'name', 'title', 'last_check_datetime', 'next_check_datetime', 'datetime', 'last_datetime', 'total_push_notifications', 'total_sent_push_notifications', 'total_displayed_push_notifications', 'total_clicked_push_notifications', 'total_closed_push_notifications', 'total_campaigns'], allowed_datetime_fields: ['last_check_datetime', 'next_check_datetime', 'datetime', 'last_datetime']));
        ...
        $paginator = (new \Altum\Paginator($total_rows, $filters->get_results_per_page(), $_GET['page'] ?? 1, url('api/rss-automations?' . $filters->get_get() . '&page={{PAGE}}')));
    ...
    private function get() {
        ...
        $rss_automation = db()->where('rss_automation_id', $rss_automation_id)->where('user_id', $this->user->user_id)->getOne('rss_automations');
    ...
    private function post() {
        ...
        $total_rows = db()->where('user_id', $this->user->user_id)->getValue('rss_automations', 'count(*)');
    ...
    private function patch() {
        ...
        /* Check for the plan limit */
        $total_rows = db()->where('user_id', $this->user->user_id)->getValue('rss_automations', 'count(`rss_automation_id`)');
