...
class ApiFlows extends Controller {
    ...
    private function get_all() {
        ...
        $filters = (new \Altum\Filters(['website_id', 'segment', 'user_id'], ['name', 'title', 'description', 'url',], ['flow_id', 'name', 'title', 'last_sent_datetime', 'datetime', 'last_datetime', 'total_push_notifications', 'total_sent_push_notifications', 'total_displayed_push_notifications', 'total_clicked_push_notifications', 'total_closed_push_notifications'], allowed_datetime_fields: ['last_sent_datetime', 'datetime', 'last_datetime']));
        ...
        $paginator = (new \Altum\Paginator($total_rows, $filters->get_results_per_page(), $_GET['page'] ?? 1, url('api/flows?' . $filters->get_get() . '&page={{PAGE}}')));
    ...
    private function get() {
        ...
        $flow = db()->where('flow_id', $flow_id)->where('user_id', $this->user->user_id)->getOne('flows');
    ...
    private function post() {
        ...
        $total_rows = db()->where('user_id', $this->user->user_id)->getValue('flows', 'count(*)');
    ...
    private function patch() {
        ...
        /* Check for the plan limit */
        $total_rows = db()->where('user_id', $this->user->user_id)->getValue('flows', 'count(`flow_id`)');
        ...
        if($_POST['auto_apply_to_matching']) {
            db()->where('webiste_id', $_POST['website_id'])->update('subscribers', [
