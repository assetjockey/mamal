...
@else
    ...
    <nav class="slide-menu slide-menu position-fixed top-0 bottom-0 {{ (__('lang_dir') == 'rtl' ? 'left-auto right-0' : 'left-0') }} shadow bg-base-0 navbar navbar-light p-0 d-flex flex-column z-1030" id="slide-menu">
        <div class="min-height-0 flex-grow-1 d-flex flex-column w-100">
            ...
            @if(Auth::user()->active_plan->features->pageviews >= 0)
                @if(!Auth::user()->can_track_websites)
                    <div class="pt-3 px-4">
                ...
                <div class="row no-gutters">
                    <div class="col">
                        <div class="small text-muted">
                            {{ __(':number of :total pageviews used.', ['number' => shortenNumber(Auth::user()->website_pageviews_count), 'total' => (Auth::user()->active_plan->features->pageviews < 0 ? '∞' : shortenNumber(Auth::user()->active_plan->features->pageviews))]) }}
                        </div>
                    </div>
                </div>

                <div class="progress w-100 my-2 height-1.25">
                    <div class="progress-bar bg-danger rounded" role="progressbar" style="width: {{ (Auth::user()->active_plan->features->pageviews == 0 ? 100 : ((Auth::user()->website_pageviews_count / Auth::user()->active_plan->features->pageviews) * 100)) }}%"></div>
                </div>
            </a>
