        ...
        if ($request->is('admin/*')) {
            ...
            if ($request->input('plan_id')) {
                ...
                if ($request->input('plan_id') && ($request->input('plan_ends_at') && $request->input('plan_id') != 1)) {
                    $planEndsAt = Carbon::createFromFormat('Y-m-d', $request->input('plan_ends_at'), $user->timezone ?? config('settings.timezone'))->tz(config('app.timezone'));
                }
                ...
                if ($request->input('plan_id') && ($user->active_plan->id != $request->input('plan_id') || ($user->plan_ends_at == null && $user->plan_ends_at != $planEndsAt) || ($user->plan_ends_at && $planEndsAt && !$user->plan_ends_at->isSameDay($planEndsAt)))) {
                    $now = Carbon::now();
                    ...
                    if (!$user->can_track_websites) {
                        $user->can_track_websites = true;
                    }
                }
            }
        }
