...
class TransferRequestStatistics extends Controller {
    ...
    public function index() {
        ...
        if(!$this->user->plan_settings->analytics_is_enabled) {
            ...
            redirect('transfer-request/' . $transfer_request->transfer_request_id);
        }
        ...
        switch($type) {
            ...
            case 'entries':
                ...
                $total_rows = database()->query("SELECT COUNT(*) AS `total` FROM `statistics` WHERE `transfer_request_id` = {$transfer_request->transfer_request_id} AND (`datetime` BETWEEN '{$datetime['query_start_date']}' AND '{$datetime['query_end_date']}') {$filters->get_sql_where()}")->fetch_object()->total ?? 0;
                $paginator = (new \Altum\Paginator($total_rows, $filters->get_results_per_page(), $_GET['page'] ?? 1, url('transfer-request-statistics/' . $transfer_request->transfer_request_id . '?type=' . $type . '&start_date=' . $datetime['start_date'] . '&end_date=' . $datetime['end_date'] . $filters->get_get() . '&page={{PAGE}}')));
        ...
        /* Set a custom title */
        Title::set(sprintf(l('transfer_request_statistics.title'), $transfer_request->url));
        ...
        $data['type'] = $type;
        $view = new \Altum\View('transfer-request-statistics/statistics_' . $type, (array) $this);
        ...
        $view = new \Altum\View('transfer-request-statistics/index', (array) $this);
    ...
    public function reset() {
        ...
        if(!$transfer_request = db()->where('transfer_request_id', $transfer_request_id)->where('user_id', $this->user->user_id)->getOne('transfer_requests', ['transfer_request_id'])) {
            throw_404();
        }
        ...
        if(\Altum\Teams::is_delegated() && !\Altum\Teams::has_access('delete.transfer_requests')) {
            ...
            redirect('transfer-request-statistics/' . $transfer_request->transfer_request_id);
        }
        ...
        if(!Alerts::has_field_errors() && !Alerts::has_errors()) {
            ...
            database()->query("DELETE FROM `statistics` WHERE `transfer_request_id` = {$transfer_request->transfer_request_id} AND (`datetime` BETWEEN '{$datetime['query_start_date']}' AND '{$datetime['query_end_date']}')");
