...
class AdminTransfers extends Controller {
    ...
    public function index() {
        ...
        $filters = (new \Altum\Filters(['user_id', 'domain_id', 'project_id'], ['name', 'url', 'description'], ['transfer_id', 'expiration_datetime', 'last_datetime', 'datetime', 'name', 'url', 'pageviews', 'downloads', 'downloads_limit', 'total_files', 'total_size'], allowed_datetime_fields: ['datetime', 'last_datetime', 'expiration_datetime']));
        ...
        $paginator = (new \Altum\Paginator($total_rows, $filters->get_results_per_page(), $_GET['page'] ?? 1, url('admin/transfers?' . $filters->get_get() . '&page={{PAGE}}')));
        ...
        if(!Alerts::has_field_errors() && !Alerts::has_errors()) {
            ...
            switch($_POST['type']) {
                case 'delete':
                    ...
                    (new \Altum\Models\Transfers())->bulk_delete($_POST['selected']);
