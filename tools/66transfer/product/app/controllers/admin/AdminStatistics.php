...
class AdminStatistics extends Controller {
    ...
    protected function transfer_requests() {

        $total = ['transfer_requests' => 0];
        ...
        while($row = $result->fetch_object()) {
            ...
            $transfer_requests_chart[$row->formatted_date] = [
                'transfer_requests' => $row->total
            ];

            $total['transfer_requests'] += $row->total;
        }
        ...
        return [
            'total' => $total,
            'transfer_requests_chart' => $transfer_requests_chart,
        ];
    ...
    protected function requests_submissions() {

        $total = ['requests_submissions' => 0];

        /* statistics */
        $convert_tz_sql = get_convert_tz_sql('`datetime`', $this->user->timezone);

        $requests_submissions_chart = [];
        $result = database()->query("
            SELECT
                COUNT(*) AS `total`,
                DATE_FORMAT({$convert_tz_sql}, '{$this->datetime['query_date_format']}') AS `formatted_date`
        ...
        while($row = $result->fetch_object()) {
            ...
            $requests_submissions_chart[$row->formatted_date] = [
                'requests_submissions' => $row->total
            ];

            $total['requests_submissions'] += $row->total;
        }

        $requests_submissions_chart = get_chart_data($requests_submissions_chart);

        return [
            'total' => $total,
            'requests_submissions_chart' => $requests_submissions_chart,
        ];
