...
class ApiDomains extends Controller {
    ...
    private function get_available() {
        ...
        $filters = (new \Altum\Filters([], ['host'], ['domain_id', 'last_datetime', 'host', 'datetime'], allowed_datetime_fields: ['last_datetime', 'datetime']));
        ...
        /* Prepare the base where */
        if(settings()->transfers->additional_domains_is_enabled) {
            $base_where = "(user_id = {$this->user->user_id} OR `type` = 1)";
        } else {
            $base_where = "user_id = {$this->user->user_id}";
        }

        $base_where .= " AND `is_enabled` = 1";

        /* Exclude additional domains not allowed by plan */
        $additional_domains = $this->user->plan_settings->additional_domains ?? [];
        $additional_domains = array_map('intval', $additional_domains);

        if(settings()->transfers->additional_domains_is_enabled) {
            if(count($additional_domains)) {
                $additional_domains = implode(',', $additional_domains);
                $base_where .= " AND (`type` != 1 OR (`type` = 1 AND domain_id IN ({$additional_domains})))";
            } else {
                $base_where .= " AND `type` != 1";
            }
        }

        /* Prepare the paginator */
        $total_rows = database()->query("SELECT COUNT(*) AS `total` FROM `domains` WHERE {$base_where} {$filters->get_sql_where()}")->fetch_object()->total ?? 0;
        $paginator = (new \Altum\Paginator($total_rows, $filters->get_results_per_page(), $_GET['page'] ?? 1, url('api/domains/available?' . $filters->get_get() . '&page={{PAGE}}')));
    ...
    private function get_all() {
        ...
        /* Prepare the filtering system */
        $filters = (new \Altum\Filters(['user_id', 'is_enabled'], ['host'], ['domain_id', 'last_datetime', 'host', 'datetime'], allowed_datetime_fields: ['datetime', 'last_datetime']));
        $filters->set_default_order_by('domain_id', $this->user->preferences->default_order_type ?? settings()->main->default_order_type);
        $filters->set_default_results_per_page($this->user->preferences->default_results_per_page ?? settings()->main->default_results_per_page);
        $filters->process();
