    ...
    <div class="d-flex align-items-center col-auto p-0">
        <div class="dropdown">
            ...
            <div class="dropdown-menu dropdown-menu-right d-print-none">
                <a href="<?= url('transfer-request-submissions-statistics/' . $data->transfer_request->transfer_request_id . '?' . \Altum\Router::$original_request_query . $data->filters->get_get() . '&export=csv') ?>" target="_blank" class="dropdown-item <?= $this->user->plan_settings->export->csv ? null : 'disabled pointer-events-all' ?>" <?= $this->user->plan_settings->export->csv ? null : get_plan_feature_disabled_info() ?>>
                    <i class="fas fa-fw fa-sm fa-file-csv mr-2"></i> <?= sprintf(l('global.export_to'), 'CSV') ?>
                </a>
                <a href="<?= url('transfer-request-submissions-statistics/' . $data->transfer_request->transfer_request_id . '?' . \Altum\Router::$original_request_query . $data->filters->get_get() . '&export=json') ?>" target="_blank" class="dropdown-item <?= $this->user->plan_settings->export->json ? null : 'disabled pointer-events-all' ?>" <?= $this->user->plan_settings->export->json ? null : get_plan_feature_disabled_info() ?>>
