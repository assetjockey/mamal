            ...
            <div class="d-flex align-items-center col-auto p-0">
                <div class="dropdown">
                    ...
                    <div class="dropdown-menu dropdown-menu-right d-print-none">
                        <a href="<?= url('transfer-request-statistics/' . $data->transfer_request->transfer_request_id . '?type=' . $data->type . '&referrer_host=' . $data->referrer_host . '&start_date=' . $data->datetime['start_date'] . '&end_date=' . $data->datetime['end_date'] . '&export=csv') ?>" target="_blank" class="dropdown-item">
                            <i class="fas fa-fw fa-sm fa-file-csv mr-2"></i> <?= sprintf(l('global.export_to'), 'CSV') ?>
                        </a>
                        <a href="<?= url('transfer-request-statistics/' . $data->transfer_request->transfer_request_id . '?type=' . $data->type . '&referrer_host=' . $data->referrer_host . '&start_date=' . $data->datetime['start_date'] . '&end_date=' . $data->datetime['end_date'] . '&export=json') ?>" target="_blank" class="dropdown-item <?= $this->user->plan_settings->export->json ? null : 'disabled pointer-events-all' ?>" <?= $this->user->plan_settings->export->json ? null : get_plan_feature_disabled_info() ?>>
