...
<div class="container">
    ...
    <?php if($this->user->plan_settings->transfer_requests_limit != -1 && $data->total_transfer_requests > $this->user->plan_settings->transfer_requests_limit): ?>
        <div class="alert alert-danger">
            <i class="fas fa-fw fa-times-circle text-danger mr-2"></i> <?= sprintf(settings()->payment->is_enabled ? l('global.info_message.plan_feature_limit_removal_with_upgrade') : l('global.info_message.plan_feature_limit_removal'), '<strong>' . $data->total_transfer_requests - $this->user->plan_settings->transfer_requests_limit, mb_strtolower(l('transfer_requests.title')) . '</strong>', '<a href="' . url('plan') . '" class="font-weight-bold text-reset">' . l('global.info_message.plan_upgrade') . '</a>') ?>
        </div>
    <?php endif ?>

    <div class="row mb-4">
        <div class="col-12 col-lg d-flex align-items-center mb-3 mb-lg-0 text-truncate">
            <h1 class="h4 m-0 text-truncate"><i class="fas fa-fw fa-xs fa-inbox mr-1"></i> <?= l('transfer_requests.header') ?></h1>

            <div class="ml-2">
                <span data-toggle="tooltip" title="<?= l('transfer_requests.subheader') ?>">
        ...
        <div class="col-12 col-lg-auto d-flex flex-wrap gap-3 d-print-none">
            <div>
                <?php if($this->user->plan_settings->transfer_requests_limit != -1 && $data->total_transfer_requests >= $this->user->plan_settings->transfer_requests_limit): ?>
                    <button type="button" class="btn btn-primary disabled" <?= get_plan_feature_limit_reached_info() ?>>
                        <i class="fas fa-fw fa-plus-circle fa-sm mr-1"></i> <?= l('transfer_requests.create') ?>
                    </button>
                <?php else: ?>
                    <a href="<?= url('transfer-request-create') ?>" class="btn btn-primary" data-toggle="tooltip" data-html="true" title="<?= get_plan_feature_limit_info($data->total_transfer_requests, $this->user->plan_settings->transfer_requests_limit, isset($data->filters) ? !$data->filters->has_applied_filters : true) ?>">
                        <i class="fas fa-fw fa-plus-circle fa-sm mr-1"></i> <?= l('transfer_requests.create') ?>
                    </a>
                <?php endif ?>
            </div>
            ...
            <div>
                <div class="dropdown">
                    <button type="button" class="btn btn-light dropdown-toggle-simple <?= !empty($data->transfer_requests) ? null : 'disabled' ?>" data-toggle="dropdown" data-boundary="viewport" data-tooltip title="<?= l('global.export') ?>" data-tooltip-hide-on-click>
                        <i class="fas fa-fw fa-sm fa-download"></i>
                    </button>

                    <div class="dropdown-menu dropdown-menu-right d-print-none">
                        <a href="<?= url('transfer-requests?' . $data->filters->get_get() . '&export=csv')  ?>" target="_blank" class="dropdown-item <?= $this->user->plan_settings->export->csv ? null : 'disabled pointer-events-all' ?>" <?= $this->user->plan_settings->export->csv ? null : get_plan_feature_disabled_info() ?>>
                            <i class="fas fa-fw fa-sm fa-file-csv mr-2"></i> <?= sprintf(l('global.export_to'), 'CSV') ?>
                        </a>
                        <a href="<?= url('transfer-requests?' . $data->filters->get_get() . '&export=json') ?>" target="_blank" class="dropdown-item <?= $this->user->plan_settings->export->json ? null : 'disabled pointer-events-all' ?>" <?= $this->user->plan_settings->export->json ? null : get_plan_feature_disabled_info() ?>>
            ...
            <div>
                <div class="dropdown">
                    <button type="button" class="btn <?= $data->filters->has_applied_filters ? 'btn-dark' : 'btn-light' ?> filters-button dropdown-toggle-simple <?= !empty($data->transfer_requests) || $data->filters->has_applied_filters ? null : 'disabled' ?>" data-toggle="dropdown" data-boundary="viewport" data-tooltip data-html="true" title="<?= l('global.filters.tooltip') ?>" data-tooltip-hide-on-click>
    ...
    <?php if (!empty($data->transfer_requests)): ?>
        <?php if($data->transfer_requests_chart): ?>
        <div class="card mb-4">
            <div class="card-body">
                <div class="chart-container <?= !$data->transfer_requests_chart['is_empty'] ? null : 'd-none' ?>">
                    <canvas id="pageviews_chart"></canvas>
                </div>
                <?= !$data->transfer_requests_chart['is_empty'] ? null : include_view(THEME_PATH . 'views/partials/no_chart_data.php', ['has_wrapper' => false]); ?>

                <?php if(!$data->transfer_requests_chart['is_empty'] && settings()->main->chart_cache ?? 12): ?>
        ...
        <script>
            ...
            if(document.getElementById('pageviews_chart')) {
                ...
                new Chart(pageviews_chart, {
                    type: 'line',
                    data: {
                        labels: <?= $data->transfer_requests_chart['labels'] ?? '[]' ?>,
                        datasets: [
                            {
                                label: <?= json_encode(l('transfer_statistics.pageviews')) ?>,
                                data: <?= $data->transfer_requests_chart['pageviews'] ?? '[]' ?>,
                            ...
                            },
                            {
                                ...
                                data: <?= $data->transfer_requests_chart['visitors'] ?? '[]' ?>,
    ...
    <?php endif ?>
        ...
        <form id="table" action="<?= SITE_URL . 'transfer-requests/bulk' ?>" method="post" role="form">
