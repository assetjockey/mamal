    ...
    <?php if(settings()->main->breadcrumbs_is_enabled): ?>
        <nav aria-label="breadcrumb">
            <ol class="custom-breadcrumbs small">
                ...
                <li class="active" aria-current="page"><?= l('transfer_request_update.breadcrumb') ?></li>
            </ol>
        </nav>
    <?php endif ?>

    <div class="d-flex justify-content-between mb-4">
        <h1 class="h4 text-truncate mb-0 mr-2"><i class="fas fa-fw fa-xs fa-inbox mr-1"></i> <?= sprintf(l('global.update_x'), $data->transfer_request->name) ?></h1>

        <?= include_view(THEME_PATH . 'views/transfer-requests/transfer_request_dropdown_button.php', ['id' => $data->transfer_request->transfer_request_id, 'resource_name' => $data->transfer_request->name]) ?>
    </div>
        ...
        <div class="card-body">
            ...
            <form action="" method="post" role="form">
                ...
                <div class="form-group">
                    ...
                    <input type="text" id="name" name="name" class="form-control <?= \Altum\Alerts::has_field_errors('name') ? 'is-invalid' : null ?>" value="<?= $data->transfer_request->name ?>" required="required" />
                    <?= \Altum\Alerts::output_field_error('name') ?>
                </div>

                <div class="form-group">
                    <label for="description"><i class="fas fa-fw fa-pen fa-sm text-muted mr-1"></i> <?= l('transfer.description') ?></label>
                    <input type="text" id="description" name="description" class="form-control <?= \Altum\Alerts::has_field_errors('description') ? 'is-invalid' : null ?>" value="<?= $data->transfer_request->description ?>" maxlength="256" />
                    ...
                    <div class="form-group">
                        ...
                        <select id="domain_id" name="domain_id" class="custom-select">
                            <?php if(settings()->transfers->main_domain_is_enabled || \Altum\Authentication::is_admin()): ?>
                                <option value=" " <?= $data->transfer_request->domain_id ? null : 'selected="selected"' ?>><?= remove_url_protocol_from_url(SITE_URL) ?></option>
                            <?php endif ?>

                            <?php foreach($data->domains as $row): ?>
                                <option value="<?= $row->domain_id ?>" <?= $data->transfer_request->domain_id && $data->transfer_request->domain_id == $row->domain_id ? 'selected="selected"' : null ?>><?= remove_url_protocol_from_url($row->url) ?></option>
                            <?php endforeach ?>
                        </select>
                    </div>
                        ...
                        <div class="<?= $this->user->plan_settings->custom_url_is_enabled ? null : 'container-disabled' ?>">
                            <div class="form-group">
                                ...
                                <div class="input-group">
                                    ...
                                    <input type="text" id="url" name="url" class="form-control <?= \Altum\Alerts::has_field_errors('url') ? 'is-invalid' : null ?>" value="<?= $data->transfer_request->url ?>" maxlength="<?= ($this->user->plan_settings->url_maximum_characters ?? 64) ?>" placeholder="<?= l('global.url_slug_placeholder') ?>" onchange="update_this_value(this, get_slug)" onkeyup="update_this_value(this, get_slug)" />
                                </div>
                ...
                <div class="tab-content">
                    <div class="tab-pane fade" id="pills-expiration" role="tabpanel" aria-labelledby="expiration-tab">
                        ...
                        if($data->transfer_request->expiration_datetime) {
                            $current_expiration_datetime_object = (new \DateTime($data->transfer_request->expiration_datetime))->setTimezone(new \DateTimeZone($this->user->timezone));
                    ...
                    <div class="tab-pane fade" id="pills-protection" role="tabpanel" aria-labelledby="protection-tab">
                        <div <?= $this->user->plan_settings->password_protection_is_enabled ? null : get_plan_feature_disabled_info() ?>>
                            <div class="form-group">
                                ...
                                <input type="password" id="password" name="password" class="form-control" value="<?= $data->transfer_request->settings->password ?>" autocomplete="new-password" maxlength="64" />
                                        ...
                                        <?php foreach($data->pixels as $pixel): ?>
                                            <div class="col-12 col-lg-6">
                                                <div class="custom-control custom-checkbox my-2">
                                                    <input id="pixel_id_<?= $pixel->pixel_id ?>" name="pixels_ids[]" value="<?= $pixel->pixel_id ?>" type="checkbox" class="custom-control-input" <?= in_array($pixel->pixel_id, $data->transfer_request->pixels_ids ?? []) ? 'checked="checked"' : null ?>>
                    ...
                    <?php if(settings()->notification_handlers->is_enabled): ?>
                        <div class="tab-pane fade" id="pills-notification-handlers" role="tabpanel" aria-labelledby="notification-handlers-tab">
                            <div <?= $this->user->plan_id != 'guest' ? null : get_plan_feature_disabled_info() ?>>
                                ...
                                <div class="form-group <?= $this->user->plan_id != 'guest' ? null : 'container-disabled' ?>">
                                    <div class="d-flex flex-wrap flex-row justify-content-between">
                                    ...
                                    <div class="row">
                                        <?php foreach($data->notification_handlers as $notification_handler): ?>
                                            <div class="col-12 col-lg-6">
                                                <div class="custom-control custom-checkbox my-2">
                                                    <input id="submission_notification_handlers_ids_<?= $notification_handler->notification_handler_id ?>" name="submission_notification_handlers_ids[]" value="<?= $notification_handler->notification_handler_id ?>" type="checkbox" class="custom-control-input" <?= in_array($notification_handler->notification_handler_id, $data->transfer_request->notifications->submission ?? []) ? 'checked="checked"' : null ?>>
                                ...
                                <div class="form-group <?= $this->user->plan_id != 'guest' ? null : 'container-disabled' ?>">
                                    ...
                                    <div class="row">
                                        <?php foreach($data->notification_handlers as $notification_handler): ?>
                                            <div class="col-12 col-lg-6">
                                                <div class="custom-control custom-checkbox my-2">
                                                    <input id="pageview_notification_handlers_ids_<?= $notification_handler->notification_handler_id ?>" name="pageview_notification_handlers_ids[]" value="<?= $notification_handler->notification_handler_id ?>" type="checkbox" class="custom-control-input" <?= in_array($notification_handler->notification_handler_id, $data->transfer_request->notifications->pageview ?? []) ? 'checked="checked"' : null ?>>
                    ...
                    <div class="tab-pane fade" id="pills-advanced" role="tabpanel" aria-labelledby="advanced-tab">
                                ...
                                <div class="form-group <?= $this->user->plan_settings->projects_limit != 0 ? null : 'container-disabled' ?>">
                                    ...
                                    <select id="project_id" name="project_id" class="custom-select">
                                        ...
                                        <?php foreach($data->projects as $project_id => $project): ?>
                                            <option value="<?= $project_id ?>" <?= $data->transfer_request->project_id == $project_id ? 'selected="selected"' : null ?>><?= $project->name ?></option>
                                        <?php endforeach ?>
                                    </select>
                        ...
                        <div <?= $this->user->plan_settings->removable_branding_is_enabled ? null : get_plan_feature_disabled_info() ?>>
                            <div class="form-group custom-control custom-switch <?= $this->user->plan_settings->removable_branding_is_enabled ? null : 'container-disabled' ?>">
                                <input id="is_removed_branding" name="is_removed_branding" type="checkbox" class="custom-control-input" <?= $data->transfer_request->settings->is_removed_branding ? 'checked="checked"' : null?> <?= $this->user->plan_settings->removable_branding_is_enabled ? null : 'disabled="disabled"' ?>>
                        ...
                        <div <?= $this->user->plan_settings->custom_css_is_enabled ? null : get_plan_feature_disabled_info() ?>>
                            <div class="form-group <?= $this->user->plan_settings->custom_css_is_enabled ? null : 'container-disabled' ?>" data-character-counter="textarea">
                                ...
                                <textarea id="custom_css" class="form-control" name="custom_css" maxlength="10000" placeholder="<?= l('global.custom_css_placeholder') ?>"><?= $data->transfer_request->settings->custom_css ?></textarea>
                        ...
                        <div <?= $this->user->plan_settings->custom_js_is_enabled ? null : get_plan_feature_disabled_info() ?>>
                            <div class="form-group <?= $this->user->plan_settings->custom_js_is_enabled ? null : 'container-disabled' ?>" data-character-counter="textarea">
                                ...
                                <textarea id="custom_js" class="form-control" name="custom_js" maxlength="10000" placeholder="<?= l('global.custom_js_placeholder') ?>"><?= $data->transfer_request->settings->custom_js ?></textarea>
