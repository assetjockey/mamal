<section class="relative w-screen min-h-screen flex items-stretch overflow-hidden bg-white overflow-x-hidden">
    ...
    <div class="z-10 flex flex-1 flex-col justify-center px-6 py-16 md:px-8" style="background:#fbfaf6;background-image:linear-gradient(#edf3fb 1px,transparent 1px),linear-gradient(90deg,#edf3fb 1px,transparent 1px);background-size:34px 34px;">
        ...
        <form class="actionForm relative w-full space-y-5 rounded-[1.75rem] border border-[#ded7ca] bg-[#fffdf8] p-8 shadow-[0_28px_85px_-64px_rgba(24,23,20,.55)] md:p-12" action="{{ module_url('do_resend_activation') }}" method="POST">
            <div class="mb-10 text-center">
                ...
                <h1 class="mb-4 font-serif text-4xl leading-tight tracking-[-0.03em] text-[#181714]">{{ __("Resend activation email") }}</h1>
                <p class="text-base leading-8 text-[#6d685f]">{{ __("Need a fresh activation link? Enter your email and we will send it again.") }}</p>
            </div>
