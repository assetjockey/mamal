...
class UpdateLinkRequest extends FormRequest
{
    ...
    {
        return [
            'url' => ['bail', 'sometimes', 'required', new ValidateUrlRule(), 'max:2048', new ValidateBannedWordsRule(), new ValidateDeepLinkRule($this->user()), new ValidateGoogleSafeBrowsingRule()],
            'alias' => ['sometimes', 'alpha_dash', 'max:255', new ValidateAliasRule($this), new ValidateBannedWordsRule(), new LinkAliasGateRule($this->user())],
            ...
            'pixel_ids' => ['nullable', new ValidatePixelsOwnershipRule($this->input('user_id') ?? $this->user()->id), new LinkPixelGateRule($this->user())],
            ...
            'active_period_start_at' => ['sometimes', 'nullable', 'required_with:active_period_end_at', 'date_format:Y-m-d\TH:i:s', 'before:active_period_end_at', new LinkExpirationGateRule($this->user())],
            'active_period_end_at' => ['sometimes', 'nullable', 'required_with:active_period_start_at', 'date_format:Y-m-d\TH:i:s', 'after:active_period_start_at', new LinkExpirationGateRule($this->user())],
            'clicks_limit' => ['nullable', 'integer', 'min:0', 'digits_between:0,9', new LinkExpirationGateRule($this->user())],
            'expiration_url' => ['bail', 'nullable', new ValidateUrlRule(), 'max:2048', new ValidateBannedWordsRule(), new LinkExpirationGateRule($this->user()), new ValidateDeepLinkRule($this->user()), new ValidateGoogleSafeBrowsingRule()],
    ...
    public function withValidator(Validator $validator): void
    {
        ...
        $validator->sometimes('targets.*.value', ['bail', 'nullable', 'required_with:targets.*.key', 'max:2048', new ValidateUrlRule(), new ValidateBannedWordsRule(), new ValidateDeepLinkRule($this->user())], function ($input, $item) {
            return request()->input('targets_type') && request()->input('targets_type') != 'rotations';
        });

        $validator->sometimes('targets.*.value', ['bail', 'nullable', 'max:2048', new ValidateUrlRule(), new ValidateBannedWordsRule(), new ValidateDeepLinkRule($this->user())], function ($input, $item) {
            return request()->input('targets_type') && request()->input('targets_type') == 'rotations';
        });
    }
}
