...
class AdminController extends Controller
{
     ...
     */
    public function __construct(UserService $userService, PaymentService $paymentService, LinkService $linkService, SpaceService $spaceService, DomainService $domainService, PixelService $pixelService)
    {
    ...
    public function dashboard(): View
    {
        ...
        if (enabledPaymentProcessors()) {
            ...
            $latestLinks = Link::orderBy('id', 'desc')->limit(5)->get();
        }

        return view('admin.dashboard.index', ['stats' => $stats, 'latestUsers' => $latestUsers, 'latestPayments' => $latestPayments, 'latestLinks' => $latestLinks]);
    }
    ...
    public function settings(string $id): View
    {
        ...
        return view('admin.settings.' . $id, ['domains' => $domains]);
    }
    ...
    public function updateSetting(UpdateSettingRequest $request, string $id): RedirectResponse
    {
        ...
        if ($id == 'cronjob' || $id == 'webhook_secret_key') {
            ...
            Setting::where('name', '=', 'license_type')->update(['value' => 1]);
    ...
    public function storeTaxRate(StoreTaxRateRequest $request): RedirectResponse
    {
        ...
        return redirect()->route('admin.tax-rates')->with('success', __(':name has been created.', ['name' => $request->input('name')]));
    }
    ...
    public function updateTaxRate(UpdateTaxRateRequest $request, string $id): RedirectResponse
    {
        ...
        return redirect()->route('admin.tax-rates.edit', $id)->with('success', __('Settings saved.'));
    }
     ...
     * List the links.
     */
    public function indexLinks(Request $request): View
    {
        $search = $request->input('search');
        ...
        return view('admin.links.index', ['links' => $links, 'user' => $user, 'space' => $space, 'domain' => $domain, 'pixel' => $pixel]);
    }

    /**
     * Show the edit link form.
     */
    function editLink(string $id): View
    {
        $link = Link::withCount([
                'pixels'
            ])
        ...
        return view('links.edit', ['link' => $link, 'domains' => $domains, 'spaces' => $spaces, 'pixels' => $pixels]);
    }

    /**
     * Update the link.
     */
    public function updateLink(UpdateLinkRequest $request, string $id): RedirectResponse
    {
     ...
     * Delete the link.
     */
    public function destroyLink(Request $request, string $id): RedirectResponse
    {
        if ($request->has('bulk')) {
     ...
     * List the spaces.
     */
    public function indexSpaces(Request $request): View
    {
        $search = $request->input('search');
        $searchBy = in_array($request->input('search_by'), ['name']) ? $request->input('search_by') : 'name';
        $userId = $request->input('user_id');
        ...
        return view('admin.spaces.index', ['spaces' => $spaces, 'user' => $user]);
    }

    /**
     * Show the edit space form.
     */
    public function editSpace(string $id): View
    {
        $space = Space::withCount([
                'links',
            ])
            ->where('id', '=', $id)
            ->firstOrFail();
     ...
     * Update the space.
     */
    public function updateSpace(UpdateSpaceRequest $request, string $id): RedirectResponse
    {
     ...
     * Delete the space.
     */
    public function destroySpace(Request $request, string $id): RedirectResponse
    {
        if ($request->has('bulk')) {
     ...
     * List the domains.
     */
    public function indexDomains(Request $request): View
    {
        $search = $request->input('search');
        $searchBy = in_array($request->input('search_by'), ['name']) ? $request->input('search_by') : 'name';
        $userId = $request->input('user_id');
        ...
        return view('admin.domains.index', ['domains' => $domains, 'user' => $user]);
    }
     ...
     * Update the domain.
     */
    public function updateDomain(UpdateDomainRequest $request, string $id): RedirectResponse
    {
     ...
     * Delete the domain.
     */
    public function destroyDomain(Request $request, string $id): RedirectResponse
    {
        if ($request->has('bulk')) {
            Domain::whereIn('id', array_slice(json_decode($id, true), 0, 100))->each(function ($domain) use ($request) {
                if ($domain->id == config('settings.short_domain_id')) {
     ...
     * List the pixels.
     */
    public function indexPixels(Request $request): View
    {
        $search = $request->input('search');
        $searchBy = in_array($request->input('search_by'), ['name']) ? $request->input('search_by') : 'name';
        $userId = $request->input('user_id');
        ...
        return view('admin.pixels.index', ['pixels' => $pixels, 'user' => $user, 'link' => $link]);
    }

    /**
     * Show the edit pixel form.
     */
    public function editPixel(string $id): View
    {
        $pixel = Pixel::withCount([
                'links',
            ])
            ->where('id', '=', $id)
            ->firstOrFail();
     ...
     * Update the pixel.
     */
    public function updatePixel(UpdatePixelRequest $request, string $id): RedirectResponse
    {
     ...
     * Delete the pixel.
     */
    public function destroyPixel(Request $request, string $id): RedirectResponse
    {
        if ($request->has('bulk')) {
