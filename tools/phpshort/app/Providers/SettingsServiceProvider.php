...
class SettingsServiceProvider extends ServiceProvider
{
    ...
    public function boot(Request $request): void
    {
        try {
            ...
            config(['services.apple.key_id' => config('settings.auth_apple_key_id')]);
            config(['services.apple.team_id' => config('settings.auth_apple_team_id')]);
            config(['services.apple.private_key' => config('settings.auth_apple_private_key')]);
            ...
            if (config('settings.storage_driver') !== 'public') {
                ...
                config(['filesystems.disks.' . config('settings.storage_driver') . '.url' => (empty(config('settings.storage_url')) ? null : (str_starts_with(config('settings.storage_url'), 'https://') ? config('settings.storage_url') : 'https://' . config('settings.storage_url')))]);
                config(['filesystems.disks.' . config('settings.storage_driver') . '.endpoint' => (empty(config('settings.storage_endpoint')) ? null : (str_starts_with(config('settings.storage_endpoint'), 'https://') ? config('settings.storage_endpoint') : 'https://' . config('settings.storage_endpoint')))]);
                config(['filesystems.disks.' . config('settings.storage_driver') . '.use_path_style_endpoint' => config('settings.storage_use_path_style_endpoint') == '1']);
                config(['filesystems.disks.' . config('settings.storage_driver') . '.throw' => config('settings.storage_throw') == '1']);
            }
