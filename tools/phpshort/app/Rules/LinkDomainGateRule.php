...
class LinkDomainGateRule implements ValidationRule
{
    ...
    public function validate(string $attribute, mixed $value, Closure $fail): void
    {
        ...
        if (
            ($this->domain->user_id && $this->user->cannot('domains', [User::class])) ||
            (!$this->domain->user_id && config('settings.short_domain_id') && config('settings.short_domain_id') != $value && $this->user->cannot('additionalDomains', [User::class]))
        ) {
