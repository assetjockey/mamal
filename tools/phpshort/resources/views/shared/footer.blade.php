<footer id="footer" class="{{ $footer['class'] ?? 'footer bg-base-0 py-12' }} {{ request()->is('admin/invoices/*') || request()->is('account/invoices/*') ? 'd-print-none' : '' }}">
    <div class="container">
        @if(!isset($footer['menu']['removed']))
            <div class="row">
                <div class="col-12 col-lg">
                    <ul class="nav p-0 mx-n4 mb-4 mb-lg-0 d-flex flex-column flex-lg-row">
                        @foreach ($footer['menu']['links'] ?? [__('Contact') => route('contact'), __('Terms') => config('settings.legal_terms_url'), __('Privacy') => config('settings.legal_privacy_url'), __('Developers') => route('developers')] as $title => $url)
                            <li class="nav-item">
                                <a href="{{ $url }}" class="nav-link py-1 {{ $footer['menu']['class'] ?? null}}" @if(isset($footer['menu']['links']))  rel="nofollow noreferrer noopener" @endif>{{ $title }}</a>
                            </li>
                        @endforeach
                        @if (!isset($footer['menu']['links']))
                            @foreach ($footerPages as $footerPage)
                                <li class="nav-item d-flex">
                                    <a href="{{ route('pages.show', $footerPage['slug']) }}" class="nav-link py-1">{{ __($footerPage['name']) }}</a>
                                </li>
                            @endforeach
                        @endif
                    </ul>
                ...
                <div class="col-12 col-lg-auto">
                    <ul class="nav p-0 mx-n2 mb-4 mb-lg-0 d-flex flex-row">
                            ...
                            @if($url)
                                <li class="nav-item d-flex">
                                    <a href="{{ $url }}" class="nav-link px-2 py-1 text-secondary text-decoration-none d-flex align-items-center" data-tooltip="true" title="{{ $title }}" rel="nofollow noreferrer noopener">
                                        @include('icons.' . strtolower($title), ['class' => 'fill-current w-5 h-5'])
        ...
        <div class="row">
            <div class="col-12 col-lg order-2 order-lg-1">
                @if(!isset($footer['copyright']['text']))
            ...
            <div class="col-12 col-lg-auto order-1 order-lg-2 d-flex flex-column flex-lg-row">
                <div class="nav p-0 mx-n4 mb-4 mb-lg-0 d-flex flex-column flex-lg-row">
                    <div class="nav-item d-flex">
                        <a href="#" class="nav-link py-1 d-flex align-items-center text-secondary" id="dark-mode" data-tooltip="true" title="{{ __('Change theme') }}">
                            @include('icons.contrast', ['class' => 'w-4 h-4 fill-current me-2'])
                    ...
                    @if(count(config('app.locales')) > 1)
                        <div class="nav-item d-flex">
                            <a href="#" class="nav-link py-1 d-flex align-items-center text-secondary" data-toggle="modal" data-target="#change-language-modal" data-tooltip="true" title="{{ __('Change language') }}">
                                @include('icons.language', ['class' => 'w-4 h-4 fill-current me-2'])
