    ...
    <div class="bg-base-1 flex-fill">
        <div class="container pt-4 mt-4 pb-16">
            ...
            <div class="row mx-n2 mb-4">
                <div class="col px-2">
                    <h1 class="fs-3xl fw-medium tracking-tight m-0">{{ __('Email') }}</h1>
                </div>
            </div>

            <div class="card border-0 shadow-sm">
                <div class="card-header align-items-center">
                    <div class="fw-medium py-1">{{ __('Email') }}</div>
                </div>
                <div class="card-body">
                    ...
                    <div class="alert alert-info">
                        <div class="row">
                            <div class="col">
                                {{ __('Email service used to send outgoing messages from the website.') }}
                            </div>
                            <div class="col-auto">
                                <a href="{{ config('info.software.url') }}/{{ mb_strtolower(config('info.software.name')) }}/documentation#email" class="alert-link fw-medium" target="_blank">{{ __('Learn more') }}</a>
                            </div>
                        </div>
                    </div>
                    ...
                    <form action="{{ route('admin.settings', 'email') }}" method="post" enctype="multipart/form-data">
                        ...
                        <div class="form-group">
                            ...
                            <small class="form-text text-muted">
                                {!! __('The email address that appears in the :field field of outgoing emails.', ['field' => '<span class="badge badge-secondary">' . __('From') . '</span>']) !!} {{ __('Some SMTP providers require this email address to be the same with the one they are providing.') }}
                            </small>
                        </div>
