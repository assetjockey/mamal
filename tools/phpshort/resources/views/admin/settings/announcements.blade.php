    ...
    <div class="bg-base-1 flex-fill">
        <div class="container pt-4 mt-4 pb-16">
            ...
            <div class="row mx-n2 mb-4">
                <div class="col px-2">
                    <h1 class="fs-3xl fw-medium tracking-tight m-0">{{ __('Announcements') }}</h1>
                </div>
            </div>

            <div class="card border-0 shadow-sm">
                <div class="card-header align-items-center">
                    <div class="fw-medium py-1">{{ __('Announcements') }}</div>
                </div>
                <div class="card-body">
                    ...
                    <form action="{{ route('admin.settings', 'announcements') }}" method="post" enctype="multipart/form-data">
                        ...
                        <div class="pb-4">
                            <button class="btn btn-soft-inverse d-block w-full d-flex align-items-center justify-content-center" type="button" data-toggle="collapse" data-target="#collapseGuest" aria-expanded="{{ ($errors->has('announcement_guest') || $errors->has('announcement_guest_content') || $errors->has('announcement_guest_type') || $errors->has('announcement_guest_id') ? 'true' : 'false') }}" aria-controls="collapseGuest">
                                @include('icons.domino-mask', ['class' => 'fill-current w-4 h-4 me-2']) {{ __('Guest') }}
                            </button>
                            ...
                            <div class="collapse {{ ($errors->has('announcement_guest') || $errors->has('announcement_guest_content') || $errors->has('announcement_guest_type') || $errors->has('announcement_guest_id') ? 'show' : '') }}" id="collapseGuest">
                                ...
                                <div class="form-group mb-0">
                                    ...
                                    <select name="announcement_guest_type" id="i-announcement-guest-type" class="custom-select{{ $errors->has('announcement_guest_type') ? ' is-invalid' : '' }}">
                                        @foreach(['primary' => __('Primary'), 'secondary' => __('Secondary'), 'success' => __('Success'), 'danger' => __('Danger'), 'warning' => __('Warning'), 'info' => __('Info'), 'inverse' => __('Inverse')] as $key => $value)
                        ...
                        <div class="pb-4">
                            <button class="btn btn-soft-inverse d-block w-full d-flex align-items-center justify-content-center" type="button" data-toggle="collapse" data-target="#collapseUser" aria-expanded="{{ ($errors->has('announcement_user') || $errors->has('announcement_user_content') || $errors->has('announcement_user_type') || $errors->has('announcement_user_id') ? 'true' : 'false') }}" aria-controls="collapseUser">
                                @include('icons.people-alt', ['class' => 'fill-current w-4 h-4 me-2']) {{ __('User') }}
                            </button>
