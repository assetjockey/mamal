    ...
    <div class="bg-base-1 flex-fill">
        <div class="container pt-4 mt-4 pb-16">
            ...
            <div class="row mx-n2 mb-4">
                <div class="col px-2">
                    <h1 class="fs-3xl fw-medium tracking-tight m-0">{{ __('Checkout') }}</h1>
                </div>
            </div>
            ...
            <form method="post" enctype="multipart/form-data" id="form-payment">
                ...
                <div class="row m-n2">
                    <div class="col-12 col-lg-8 p-2">
                        <div class="card border-0 shadow-sm mb-4 overflow-hidden @if(($plan->trial_days && !$user->plan_trial_ends_at) || ($coupon && $coupon->type)) d-none @endif">
                            <div class="card-header align-items-center">
                                <div class="fw-medium py-1">{{ __('Payment method') }}</div>
                            </div>
                            <div class="card-body p-0">
                                <ul class="list-group list-group-flush">
                                    @foreach(enabledPaymentProcessors() as $key => $value)
                                        <li class="list-group-item">
                                            <div class="custom-control custom-radio">
                                                ...
                                                <label class="custom-control-label d-block" for="payment-method-{{ $key }}">
                                                    <span class="row">
                                                        <span class="col-12 col-lg d-flex align-items-center">
                                                            <img src="{{ asset('img/icons/payments/' . $key . '.svg') }}" class="w-6 rounded-sm" alt=""><span class="ms-2">{{ __($value['type']) }}</span>
                                                        </span>
                                                        <span class="col-12 col-lg-auto mt-1 mt-lg-0">
                                                            <span class="text-muted">{{ $value['name'] == 'Bank' ? __('Bank') : $value['name'] }}</span>
                                                        </span>
                                                    </span>
                                                </label>
                                                ...
                                                @if($key == 'bank')
                                                    <div class="d-none" id="bank-instructions">
                                                        ...
                                                        <div class="form-group mb-2">
                                                            ...
                                                            <div class="input-group">
                                                                <input type="text" name="payment_id" id="i-payment-id" class="form-control form-control-sm{{ $errors->has('payment_id') ? ' is-invalid' : '' }}" value="{{ old('payment_id') && !$errors->has('payment_id') ? old('payment_id') : $user->id . 'X' . $user->payments()->count() }}" readonly>
                                                                <div class="input-group-append">
                        ...
                        <div class="card border-0 shadow-sm">
                            <div class="card-header align-items-center">
                                <div class="fw-medium py-1">{{ __('Billing information') }}</div>
                            </div>
                            <div class="card-body">
                    ...
                    <div class="col-12 col-lg-4 p-2 d-flex flex-column justify-content-start">
                        @if(($plan->trial_days && !$user->plan_trial_ends_at) || ($coupon && $coupon->type))
                            <div class="card border-0 shadow-sm">
                                <div class="card-header align-items-center">
                                    <div class="fw-medium py-1">{{ __('Order summary') }}</div>
                                </div>
                                <div class="card-body p-0">
                                    <ul class="list-group list-group-flush">
                                        <li class="list-group-item">
                            ...
                            @if($plan->trial_days && !$user->plan_trial_ends_at)
                                <button type="submit" name="skip_trial" class="btn btn-secondary btn-block my-4" value="1">
                                    {{ __('Skip trial') }}
                                </button>
                            @endif
                        @else
                            <div class="card border-0 shadow-sm">
                                <div class="card-header"><div class="fw-medium py-1">{{ __('Order summary') }}</div></div>
                                <div class="card-body">
                                    <div class="btn-group btn-group-toggle d-flex" data-toggle="buttons">
                                        <label class="btn {{ $errors->has('interval') ? 'btn-outline-danger' : 'btn-outline-inverse' }} w-full{{ request()->input('interval') == 'month' ? ' active' : ''}}" id="plan-month">
                                            <input type="radio" name="interval" value="month" @if(request()->input('interval') == 'month') checked="checked" @endif>{{ __('Monthly') }}
                                        </label>
                                        <label class="btn {{ $errors->has('interval') ? 'btn-outline-danger' : 'btn-outline-inverse' }} w-full{{ request()->input('interval') == 'year' ? ' active' : ''}}" id="plan-year">
                                    ...
                                    <ul class="list-group list-group-flush">
                                        <li class="list-group-item pt-0">
                                            <div class="row">
                                                <div class="col">
                                                    ...
                                                    <div class="d-none" data-checkout="month">
                                                        <div class="small text-muted">
                                                            <span class="d-none" data-billing-type="subscription">{!! ($plan->trial_days && !$user->plan_trial_ends_at) ? __('Billed :interval, after trial ends.', ['interval' => mb_strtolower(__('Monthly'))]) :__('Billed :interval.', ['interval' => mb_strtolower(__('Monthly'))]) !!}</span>
                                                            <span class="d-none" data-billing-type="one-time">{!! __('Billed once.') !!}</span>
                                                        </div>
                                                    </div>
                                                    <div class="d-none" data-checkout="year">
                                                        <div class="small text-muted">
                                                            <span class="d-none" data-billing-type="subscription">{!! ($plan->trial_days && !$user->plan_trial_ends_at) ? __('Billed :interval, after trial ends.', ['interval' => mb_strtolower(__('Yearly'))]) :__('Billed :interval.', ['interval' => mb_strtolower(__('Yearly'))]) !!}</span>
                                                            <span class="d-none" data-billing-type="one-time">{!! __('Billed once.') !!}</span>
                                                        </div>
                                                    </div>
                                                </div>
                                            ...
                                            @foreach($taxRates as $taxRate)
                                                <li class="list-group-item">
                                                    <div class="row">
                                                        ...
                                                        <div class="col-auto">
                                                            @if($taxRate->type)
                                                                <span class="d-none" data-checkout="month">
                                                                    {{ formatMoney(checkoutExclusiveTax($plan->amount_month, $coupon->percentage ?? 0, $taxRate->percentage, $inclTaxRatesPercentage), $plan->currency) }}
                                                                </span>
                                                                <span class="d-none" data-checkout="year">
                                                                    {{ formatMoney(checkoutExclusiveTax($plan->amount_year, $coupon->percentage ?? 0, $taxRate->percentage, $inclTaxRatesPercentage), $plan->currency) }}
                                                                </span>
                                                            @else
                                                                <span class="d-none" data-checkout="month">
                                                                    {{ formatMoney(calculateInclusiveTax($plan->amount_month, $coupon->percentage ?? 0, $taxRate->percentage, $inclTaxRatesPercentage), $plan->currency) }}
                                                                </span>
                                                                <span class="d-none" data-checkout="year">
                                                                    {{ formatMoney(calculateInclusiveTax($plan->amount_year, $coupon->percentage ?? 0, $taxRate->percentage, $inclTaxRatesPercentage), $plan->currency) }}
                                                                </span>
                                                            @endif
                                        ...
                                        @if($plan->coupons && !$coupon)
                                            <li class="list-group-item">
                                                ...
                                                <div class="row mx-n1 {{ $errors->has('coupon') || old('coupon') ? '' : 'd-none' }}" data-show-container="coupon">
                                                    <div class="col px-1">
                                                        <div class="form-group mb-0">
                                ...
                                <div class="card-footer fw-bold">
                                    <div class="row">
                                        ...
                                        <div class="col-auto">
                                            <span class="d-none" data-checkout="month">
                                                {{ formatMoney(checkoutTotal($plan->amount_month, $coupon->percentage ?? 0, $exclTaxRatesPercentage, $inclTaxRatesPercentage), $plan->currency) }}
                                            </span>
                                            <span class="d-none" data-checkout="year">
                                                {{ formatMoney(checkoutTotal($plan->amount_year, $coupon->percentage ?? 0, $exclTaxRatesPercentage, $inclTaxRatesPercentage), $plan->currency) }}
                                            </span>
                            ...
                            <div class="mt-4">
                                <span class="small text-muted">
                                    <span data-billing-type="subscription">
                                    ...
                                    <span data-billing-type="one-time">
                            ...
                            <button type="submit" name="submit" class="btn btn-success btn-block my-4">
                                <span class="d-none" data-checkout="month">
                                    {{ __('Pay :amount :currency', ['amount' => formatMoney(checkoutTotal($plan->amount_month, $coupon->percentage ?? 0, $exclTaxRatesPercentage, $inclTaxRatesPercentage), $plan->currency), 'currency' => e($plan->currency)]) }}
                                </span>
                                <span class="d-none" data-checkout="year">
                                    {{ __('Pay :amount :currency', ['amount' => formatMoney(checkoutTotal($plan->amount_year, $coupon->percentage ?? 0, $exclTaxRatesPercentage, $inclTaxRatesPercentage), $plan->currency), 'currency' => e($plan->currency)]) }}
                                </span>
                            </button>
                        @endif
                            ...
                            <div class="card border-0 h-full shadow-sm">
                                <div class="card-body d-flex">
                                    <div class="d-flex position-relative text-primary w-12 h-12 align-items-center justify-content-center flex-shrink-0 me-4">
                                        ...
                                        @include('icons.email', ['class' => 'fill-current w-6 h-6'])
                                    </div>
                                    <div class="d-flex flex-column justify-content-center">
                                        <a href="{{ route('contact') }}" class="text-inverse fw-medium text-decoration-none stretched-link">{{ __('Need help?') }}</a>
