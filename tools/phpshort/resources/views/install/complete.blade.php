                    ...
                    <div class="card-body p-12">
                        <div class="h-full d-flex flex-column justify-content-center align-items-center my-16">
                            <div class="position-relative w-32 h-32 d-flex align-items-center justify-content-center">
                                ...
                                @include('icons.system-update-alt', ['class' => 'text-primary fill-current w-16 h-16'])

                                @include('icons.check-circle-filled', ['class' => 'position-absolute end-0 bottom-0 text-success fill-current w-8 h-8'])
                            </div>

                            <div>
                                <h1 class="fs-xl fw-medium mb-2 mt-6 text-center">{{ __('Installed') }}</h1>
                                <p class="text-center text-muted mb-0">{!! __(':name has been installed.', ['name' => '<span class="fw-medium">'.config('info.software.name').'</span>']) !!}</p>
                            </div>
                        </div>
                    </div>
                </div>

                <a href="{{ route('home') }}" class="btn btn-block btn-primary d-inline-flex align-items-center mt-4 py-2">
