...
<div class="bg-base-1 flex-fill">
    <div class="container-md p-4 mx-auto">
        <div class="row justify-content-center">
            <div class="col-md-12 col-lg-10 col-xl-9">
                ...
                <div class="bg-base-0 border rounded p-12 my-4 position-relative overflow-hidden">
                    @if ($payment->status == 'cancelled')
                        <div class="position-absolute top-0 end-0 bottom-0 start-0 d-flex align-items-center justify-content-center z-1">
                            <div class="fs-8xl fw-bold text-uppercase opacity-50 text-danger rotate-n45">{{ __('Cancelled') }}</div>
                        </div>
                    @endif
                    <div class="row">
                        <div class="col-12 col-sm-6">
                            ...
                            @if($payment->seller->title)
                                <div class="fs-2xl fw-bold tracking-tight">{{ $payment->seller->title }}</div>
                            @endif
                        </div>
                        <div class="col-12 col-sm-6 text-start text-sm-end">
                            <div class="fs-2xl fw-bold tracking-tight text-uppercase text-muted">{{ __('Invoice') }}</div>
                        </div>
                        <div class="col-12 py-4 text-start text-sm-end">
                    ...
                    <div class="row">
                        <div class="col-12 col-sm-6 py-4">
                            <div class="fs-xl fw-medium mb-2">{{ __('Vendor') }}</div>
                        ...
                        <div class="col-12 col-sm-6 py-4">
                            <div class="fs-xl fw-medium mb-2">{{ __('Client') }}</div>
                        ...
                        <div class="col-12">
                            <div class="list-group list-group-flush">
                                ...
                                @foreach(collect($payment->tax_rates) as $taxRate)
                                    <div class="list-group-item px-0">
                                        <div class="row">
                                            <div class="col-9 text-end">{{ $taxRate->name }} ({{ $taxRate->percentage }}% {{ $taxRate->type ? __('excl.') : __('incl.') }})</div>

                                            <div class="col-3">
                                                @if($taxRate->type)
                                                    {{ formatMoney(checkoutExclusiveTax(($payment->interval == 'month' ? $payment->product->amount_month : $payment->product->amount_year), $payment->coupon->percentage ?? 0, $taxRate->percentage, $inclTaxRatesPercentage), $payment->currency) }}
                                                @else
                                                    {{ formatMoney(calculateInclusiveTax(($payment->interval == 'month' ? $payment->product->amount_month : $payment->product->amount_year), $payment->coupon->percentage ?? 0, $taxRate->percentage, $inclTaxRatesPercentage), $payment->currency) }}
                                                @endif
                                                {{ $payment->currency }}
                                            </div>
                                        </div>
                                ...
                                <div class="list-group-item px-0">
                                    <div class="row">
                                        <div class="col-9 text-end fw-bold">{{ __('Total') }}</div>
                                        <div class="col-3 fw-bold">{{ formatMoney($payment->amount, $payment->currency) }} {{ $payment->currency }}</div>
                                    </div>
                                </div>
                            </div>
                        </div>
