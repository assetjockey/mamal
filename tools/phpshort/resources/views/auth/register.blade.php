...
<div class="bg-base-1 d-flex align-items-center flex-fill">
    <div class="container h-full py-16">
        ...
        <div class="text-center d-block d-lg-none">
            <h1 class="fs-3xl fw-semibold  tracking-tight m-0">{{ __('Register') }}</h1>
            <div class="mx-auto mt-4">
                <p class="text-muted fw-normal fs-lg mb-0">{{ __('Join us.') }}</p>
            </div>
        </div>
            ...
            <div class="col-12">
                <div class="card border-0 shadow-sm overflow-hidden">
                    <div class="row no-gutters">
                            ...
                            <div class="card-body p-lg-12">
                                ...
                                <form method="POST" action="{{ route('register') }}" id="registration-form">
                                    ...
                                    @if(config('settings.captcha_driver'))
                                        ...
                                        <x-captcha-button data-callback="{{ (config('settings.captcha_driver') == 'turnstile' ? '' : 'captchaFormSubmit') }}" form-id="registration-form" class="btn btn-block {{ $errors->has(captchaFieldName()) ? 'btn-danger' : 'btn-primary' }} py-2" data-sitekey="{{ config('settings.captcha_site_key') }}" data-theme="{{ (config('settings.dark_mode') == 1 ? 'dark' : 'light') }}">{{ __('Register') }}</x-captcha-button>
                        ...
                        <div class="col-12 col-lg-7 d-none d-lg-flex flex-fill background-size-cover bg-center"
                             style="background-image: url({{ asset('img/register.svg') }})">
                            <div class="card-body p-lg-12 d-flex flex-column flex-fill position-absolute top-0 end-0 bottom-0 start-0">
                                <div class="d-flex align-items-center d-flex flex-fill">
                                    <div class="text-light ms-12">
                                        <div class="fs-3xl fw-bold  tracking-tight m-0">{{ __('Register') }}</div>
