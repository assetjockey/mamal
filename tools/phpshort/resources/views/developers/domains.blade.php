...
<pre class="m-0">{{ route('api.domains.index') }}</pre>
                    </div>
...
--header 'Authorization: Bearer <span class="text-primary">{api_key}</span>'
            ...
            <div class="card border-0 shadow-sm mt-4">
                ...
                <div class="card-body">
                    ...
                    <div class="bg-light text-inverse p-4 rounded d-flex align-items-center mb-4" dir="ltr">
                        <span class="badge badge-success px-2 py-1 me-4">GET</span>
<pre class="m-0">{!! str_replace(':id', '<span class="text-primary">{id}</span>', route('api.domains.show', ['id' => ':id'])) !!}</pre>
                    </div>
...
curl --location --request GET '{!! str_replace(':id', '<span class="text-primary">{id}</span>', route('api.domains.show', ['id' => ':id'])) !!}' \
--header 'Accept: application/json' \
--header 'Authorization: Bearer <span class="text-primary">{api_key}</span>'
</pre>
                </div>
...
<pre class="m-0">{{ route('api.domains.store') }}</pre>
                    </div>
...
--data-urlencode 'name=<span class="text-primary">{name}</span>'
                    ...
                    @include('developers.partials.parameters-list', ['parameters' => [
                        ...
                        [
                            ...
                            'type' => 0,
                            'format' => 'string',
                            'description' => __('Homepage URL') . '.'
                        ],
                        [
                            'name' => 'not_found_url',
                            'type' => 0,
                            'format' => 'string',
                            'description' => __('404 Not Found URL') . '.'
                        ]
                    ]])
                </div>
            </div>
...
<pre class="m-0">{!! str_replace(':id', '<span class="text-primary">{id}</span>', route('api.domains.update', ['id' => ':id'])) !!}</pre>
                    </div>
...
<pre class="bg-light text-inverse p-4 mb-4 rounded text-left" dir="ltr">
curl --location --request PUT '{!! str_replace(':id', '<span class="text-primary">{id}</span>', route('api.domains.update', ['id' => ':id'])) !!}' \
--header 'Content-Type: application/x-www-form-urlencoded' \
--header 'Authorization: Bearer <span class="text-primary">{api_key}</span>'
</pre>
            ...
            <div class="card border-0 shadow-sm mt-4">
                ...
                <div class="card-body">
                    ...
                    <div class="bg-light text-inverse p-4 rounded d-flex align-items-center mb-4" dir="ltr">
                        ...
                        <pre class="m-0">{!! str_replace(':id', '<span class="text-primary">{id}</span>', route('api.domains.destroy', ['id' => ':id'])) !!}</pre>
                    </div>
                    ...
                    <pre class="bg-light text-inverse p-4 mb-0 rounded text-left" dir="ltr">
curl --location --request DELETE '{!! str_replace(':id', '<span class="text-primary">{id}</span>', route('api.domains.destroy', ['id' => ':id'])) !!}' \
--header 'Authorization: Bearer <span class="text-primary">{api_key}</span>'
</pre>
                </div>
            </div>
