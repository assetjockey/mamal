    ...
    <div class="bg-base-1 flex-fill">
        <div class="container h-full py-4 my-4">
            ...
            <div class="row mx-n2 mb-4">
                <div class="col px-2">
                    <h1 class="fs-3xl fw-medium tracking-tight m-0">{{ __('Spaces') }}</h1>
                </div>
            </div>
...
<pre class="m-0">{{ route('api.spaces.index') }}</pre>
                    </div>
...
--header 'Authorization: Bearer <span class="text-primary">{api_key}</span>'
            ...
            <div class="card border-0 shadow-sm mt-4">
                ...
                <div class="card-body">
                    ...
                    <div class="bg-light text-inverse p-4 rounded d-flex align-items-center mb-4" dir="ltr">
                        <span class="badge badge-success px-2 py-1 me-4">GET</span>
<pre class="m-0">{!! str_replace(':id', '<span class="text-primary">{id}</span>', route('api.spaces.show', ['id' => ':id'])) !!}</pre>
                    </div>
...
curl --location --request GET '{!! str_replace(':id', '<span class="text-primary">{id}</span>', route('api.spaces.show', ['id' => ':id'])) !!}' \
--header 'Accept: application/json' \
--header 'Authorization: Bearer <span class="text-primary">{api_key}</span>'
</pre>
                </div>
...
<pre class="m-0">{{ route('api.spaces.store') }}</pre>
                    </div>
...
--data-urlencode 'name=<span class="text-primary">{name}</span>'
                    ...
                    @include('developers.partials.parameters-list', ['parameters' => [
                        ...
                        [
                            ...
                            'type' => 0,
                            'format' => 'integer',
                            'description' => __('Color') . '. ' . __('Possible values are: :values.', ['values' => '<code>' . implode('</code>, <code>', [1, 2, 3, 4, 5, 6]) .'</code>']) . ' ' . __('Defaults to: :value.', ['value' => '<code>1</code>'])
                        ]
                    ]])
                </div>
            </div>
...
<pre class="m-0">{!! str_replace(':id', '<span class="text-primary">{id}</span>', route('api.spaces.update', ['id' => ':id'])) !!}</pre>
                    </div>
                    ...
                    <pre class="bg-light text-inverse p-4 mb-4 rounded text-left" dir="ltr">
curl --location --request PUT '{!! str_replace(':id', '<span class="text-primary">{id}</span>', route('api.spaces.update', ['id' => ':id'])) !!}' \
--header 'Content-Type: application/x-www-form-urlencoded' \
--header 'Authorization: Bearer <span class="text-primary">{api_key}</span>'
</pre>
                    ...
                    @include('developers.partials.parameters-list', ['parameters' => [
                        ...
                        [
                            ...
                            'type' => 0,
                            'format' => 'integer',
                            'description' => __('Color') . '. ' . __('Possible values are: :values.', ['values' => '<code>' . implode('</code>, <code>', [1, 2, 3, 4, 5, 6]) .'</code>'])
                        ]
                    ]])
                </div>
            </div>
...
<pre class="m-0">{!! str_replace(':id', '<span class="text-primary">{id}</span>', route('api.spaces.destroy', ['id' => ':id'])) !!}</pre>
                    </div>
                    ...
                    <pre class="bg-light text-inverse p-4 mb-0 rounded text-left" dir="ltr">
curl --location --request DELETE '{!! str_replace(':id', '<span class="text-primary">{id}</span>', route('api.spaces.destroy', ['id' => ':id'])) !!}' \
--header 'Authorization: Bearer <span class="text-primary">{api_key}</span>'
</pre>
                </div>
            </div>
