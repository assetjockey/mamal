...
<ul class="pricing-features">
    ...
    <?php foreach($features as $feature => $is_enabled): ?>
        ...
        <?php if($feature == 'campaigns_limit'): ?>
            <li>
                <div class="<?= $data->plan_settings->campaigns_limit ? null : 'text-muted' ?>"><?= sprintf(l('global.plan_settings.campaigns_limit'), '<strong>' . ($data->plan_settings->campaigns_limit == -1 ? l('global.unlimited') : nr($data->plan_settings->campaigns_limit)) . '</strong>') ?></div>
        ...
        <?php if($feature == 'notifications_limit'): ?>
            <li>
                <div class="<?= $data->plan_settings->notifications_limit ? null : 'text-muted' ?>"><?= sprintf(l('global.plan_settings.notifications_limit'), '<strong>' . ($data->plan_settings->notifications_limit == -1 ? l('global.unlimited') : nr($data->plan_settings->notifications_limit)) . '</strong>') ?></div>
        ...
        <?php if($feature == 'notifications_impressions_limit'): ?>
            <li>
                <div class="<?= $data->plan_settings->notifications_impressions_limit ? null : 'text-muted' ?>"><?= sprintf(l('global.plan_settings.notifications_impressions_limit'), '<strong>' . ($data->plan_settings->notifications_impressions_limit == -1 ? l('global.unlimited') : nr($data->plan_settings->notifications_impressions_limit, 0, true)) . '</strong>') ?></div>
        ...
        <?php if($feature == 'domains_limit' && settings()->notifications->domains_is_enabled): ?>
            <li>
                <div class="<?= $data->plan_settings->domains_limit ? null : 'text-muted' ?>"><?= sprintf(l('global.plan_settings.domains_limit'), '<strong>' . ($data->plan_settings->domains_limit == -1 ? l('global.unlimited') : nr($data->plan_settings->domains_limit, 0, true)) . '</strong>') ?></div>
        ...
        <?php if($feature == 'affiliate_commission_percentage' && \Altum\Plugin::is_active('affiliate') && settings()->affiliate->is_enabled): ?>
            <li>
                <div class="<?= $data->plan_settings->affiliate_commission_percentage ? null : 'text-muted' ?>"><?= sprintf(l('global.plan_settings.affiliate_commission_percentage'), '<strong>' . nr($data->plan_settings->affiliate_commission_percentage) . '%</strong>') ?></div>
        ...
        <?php if($feature == 'enabled_notifications'): ?>
            ...
            <?php else: ?>
                <li data-toggle="tooltip" title="<?= $enabled_notifications_string ?>">
                    <div class="<?= $enabled_notifications_count ? null : 'text-muted' ?>"><?= sprintf(l('global.plan_settings.enabled_notifications_x'), '<strong>' . nr($enabled_notifications_count) . '</strong>') ?></div>
        ...
        <?php if($feature == 'email_reports_is_enabled' && settings()->notifications->email_reports_is_enabled): ?>
            <li>
                <div class="<?= $data->plan_settings->email_reports_is_enabled ? null : 'text-muted' ?>"><?= settings()->notifications->email_reports_is_enabled ? l('global.plan_settings.email_reports_is_enabled_' . settings()->notifications->email_reports_is_enabled) : l('global.plan_settings.email_reports_is_enabled') ?></div>
        ...
        <?php if(in_array($feature, ['no_ads', 'removable_branding', 'custom_branding', 'custom_css_is_enabled', 'api_is_enabled', 'white_labeling_is_enabled'])): ?>
            <li>
                <div class="<?= $data->plan_settings->{$feature} ? null : 'text-muted' ?>"><?= l('global.plan_settings.' . $feature) ?></div>
