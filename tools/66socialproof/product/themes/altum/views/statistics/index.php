...
<div class="container">
    ...
    <div class="row mb-4">
        <div class="col-12 col-lg d-flex align-items-end mb-3 mb-lg-0 text-truncate">
            <h1 class="h4 m-0 text-truncate"><i class="fas fa-fw fa-xs fa-chart-bar mr-1"></i> <?= l('statistics.header') ?></h1>
        </div>
                    ...
                    <?php while($row = $data->top_pages_result->fetch_object()): ?>
                        ...
                        <tr>
                            <td class="text-nowrap">
                                <img referrerpolicy="no-referrer" src="<?= get_favicon_url_from_domain($row->domain) ?>" class="img-fluid icon-favicon-small mr-2" loading="lazy" />

                                <span title="<?= $row->path ?>"><?= string_truncate($row->path, 64) ?></span>

                                <a href="<?= 'https://' . $row->domain . $row->path ?>" target="_blank" rel="nofollow noopener" class="text-muted ml-2"><i class="fas fa-fw fa-xs fa-external-link-alt"></i></a>
                            </td>
...
<?php \Altum\Event::add_content(fn() => include_view(THEME_PATH . 'views/partials/statistics_reset_modal.php', ['modal_id' => 'campaigns_statistics_reset_modal', 'resource_id' => 'user_id', 'path' => 'statistics/reset']), 'modals'); ?>
