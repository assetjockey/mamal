...
<div class="form-group" data-file-image-input-wrapper data-file-input-wrapper-size-limit="<?= settings()->notifications->image_size_limit ?>" data-file-input-wrapper-size-limit-error="<?= sprintf(l('global.error_message.file_size_limit'), settings()->notifications->image_size_limit) ?>">
    <label for="settings_image"><i class="fas fa-fw fa-sm fa-image text-muted mr-1"></i> <?= l('notification.settings.image') ?></label>
...
<div class="form-group" data-range-counter data-range-counter-suffix="px">
    <label for="settings_image_size"><i class="fas fa-fw fa-arrows-alt-h fa-sm text-muted mr-1"></i> <?= l('notification.settings.image_size') ?></label>
    <input type="range" min="15" max="55" id="settings_image_size" name="image_size" class="form-control-range" value="<?= $data->notification->settings->image_size ?>" />
</div>
...
<script>
    ...
    /* Im Handler */
    $('#settings_image_size').on('change', event => {

        /* Notification Preview Handler */
        $('#notification_preview .altumcode-badge-image').attr('style', 'width:' + $(event.currentTarget).val() + 'px !important; height: ' + $(event.currentTarget).val() + 'px !important;');
