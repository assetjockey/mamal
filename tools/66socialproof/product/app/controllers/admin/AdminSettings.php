...
class AdminSettings extends Controller {
    ...
    public function notifications() {
        ...
        if(!empty($_POST)) {
			...
			$allowed_features = array_keys(require APP_PATH . 'includes/notifications.php');

			/* Validate */
			$_POST['available_notifications'] = array_values(array_filter($_POST['available_notifications'], fn($item) => in_array($item, $allowed_features)));

			/* Preserve the order */
			$available_notifications = array_fill_keys($_POST['available_notifications'], true);

			/* Append missing features at the end with false */
			foreach($allowed_features as $feature) {
				if(!array_key_exists($feature, $available_notifications)) {
					$available_notifications[$feature] = false;
				}
			}
