    ...
    public function index() {
        ...
        if(is_logged_in()) {
            ...
            if(user()->type == 1 && file_exists(APP_PATH . 'languages/admin/' . \Altum\Language::$name . '#' . \Altum\Language::$code . '.php')) {
                ...
                $admin_statistics_pages = [
                    'growth' => l('admin_statistics.growth.menu'),
                    'users' => l('admin_statistics.users.menu'),
                    'users_map' => l('admin_statistics.users_map.menu'),
                    'database' => l('admin_statistics.database.menu'),
                    'local_files' => l('admin_statistics.local_files.menu'),
                    'broadcasts' => l('admin_statistics.broadcasts.menu'),
                    'internal_notifications' => l('admin_internal_notifications.menu'),
                    'notification_handlers' => l('admin_notification_handlers.menu'),
                    'email_reports' => l('admin_statistics.email_reports.menu'),
                    'campaigns' => l('admin_statistics.campaigns.menu'),
                    'notifications' => l('admin_statistics.notifications.menu'),
                    'track_notifications' => l('admin_statistics.track_notifications.menu'),
                    'track_conversions' => l('admin_statistics.track_conversions.menu'),
                    'domains' => l('admin_domains.menu'),
                ];

                if(in_array(settings()->license->type, ['SPECIAL','Extended License', 'extended'])) {
                    $admin_statistics_pages['payments'] = l('admin_statistics.payments.menu');
                    $admin_statistics_pages['redeemed_codes'] = l('admin_statistics.redeemed_codes.menu');

                    if(\Altum\Plugin::is_active('affiliate')) {
                        $admin_statistics_pages['affiliates_commissions'] = l('admin_statistics.affiliates_commissions.menu');
                        $admin_statistics_pages['affiliates_withdrawals'] = l('admin_statistics.affiliates_withdrawals.menu');
                    }
                }

                if(\Altum\Plugin::is_active('teams')) {
                    $admin_statistics_pages['teams'] = l('admin_teams.menu');
                    $admin_statistics_pages['teams_members'] = l('admin_statistics.teams_members.menu');
                }

                if(\Altum\Plugin::is_active('push-notifications')) {
                    $admin_statistics_pages['push_notifications'] = l('admin_push_notifications.menu');
                    $admin_statistics_pages['push_subscribers'] = l('admin_push_subscribers.menu');
                }

                if(\Altum\Plugin::is_active('email-shield')) {
                    $admin_statistics_pages['email_shield'] = l('admin_statistics.email_shield.menu');
                }

                if(\Altum\Plugin::is_active('image-optimizer')) {
                    $admin_statistics_pages['image_optimizer'] = l('admin_statistics.image_optimizer.menu');
                }

                foreach($admin_statistics_pages as $key => $value) {
                    $available_pages[] = [
                        'name' => l('global.menu.admin') . ' - ' . l('admin_statistics.menu') . ' - ' . $value,
                        'url' => 'admin/statistics/' . $key
                    ];
                }
                ...
                $available_pages[] = [
                    'name' => l('global.menu.admin') . ' - ' . l('admin_logs.title'),
                    'url' => 'admin/logs'
                ];
